...
funcs=[extract('inline bool saturn_state::vdp2_roz_window('),extract('inline int saturn_state::get_roz_window_pixel(')]
f='\n'.join(funcs)
...
int main(){saturn_state s;unsigned cases=0;
 for(unsigned n0=0;n0<32;++n0)for(unsigned r0=0;r0<32;++r0)for(bool rbg1:{false,true}){
  ...
  for(int y=0;y<8;++y)for(int x=0;x<16;++x){
   ...
   bool in[]={x>=3&&x<=11&&y>=2&&y<=5,x>=7&&x<=14&&y>=1&&y<=4};
...
for prefix,cfg in [('N0','n0'),('R0','r0')]:
 for suffix,expr in [('W0E',f'{cfg}&1'),('W1E',f'({cfg}>>1)&1'),('W0A',f'({cfg}>>2)&1'),('W1A',f'({cfg}>>3)&1'),('LOG',f'{cfg}>>4')]:setup.append(f's.regs.VDP2_{prefix}{suffix}={expr};')
