...
- [ ] **SND-04 — Qualify SCSP DSP and final mixer. [V/R]**
  ...
  - Signed ADREB correction integrated as native-WIP: add the signed twelve-bit ADRS_REG displacement before TABLE/ring masking. Pinned MiSTer/Ymir/Beetle agree; primary register documentation does not explicitly establish this microinstruction rule.69632 additional actual-method address/latch/read-write cases pass, four address mutants are rejected, and previous native9baba980 passes199/fails48 of247 mapped programs (the original31 all pass). Matching rebuilt native acceptance pending; see `saturn_pending/evidence/scsp-dsp-address/README.md`.
