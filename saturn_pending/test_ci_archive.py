...
with tempfile.TemporaryDirectory() as tmp:
    root=Path(tmp)
    for mode in ('good','digest','size','path','symlink','missing','occupied'):
        ...
        try:unpack_verified(archive,out,digest,size)
            ...
            if mode!='good':raise RuntimeError('bad archive accepted: '+mode)
    if (root/'escape').exists():raise RuntimeError('path escape')
print('7 synthetic CI archive transfer checks passed; no code execution')
