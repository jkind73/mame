...
- choroqpk (car selection)
  ...
  060311E6: MOV SP,R14 ;R14 = 0x60ffba0 / R15 = 0x60ffb9c / [0x60ffb9c] <-
0x60ffba0 060311E8: MOV.L @SP+,R14 ;R14 = 0x60ffb9c / R15 = 0x60ffb9c /
[0x60ffb9c] -> R14 060311EA: RTS ;R14 = 0x60ffba0 / R15 = 0x60ffba0 060311EC:
NOP 06031734: MULS.W R9, R8 ;R14 = 0x60ffba0 / R15 = 0x60ffba0 / EA = 0x60311E4
  on DRC this becomes:
  R14 0x6031b78 (cause of the crash later on), R15 = 0x60ffba4 and EA = 0
- falcom1 Dragon Slayer I (during game loading, slave pitfalls and brings
everything else to death)
- shinreij (selecting options menu, wants specifically a zero in
INTERLEAVE_DURATION but that breaks initial game loading)
...
DEFINE_DEVICE_TYPE(SATURN_DCC, saturn_dcc_device, "saturn_dcc",
                   "Sega Saturn DCC Dual CPU interface (Hitachi 315-5778)")

saturn_dcc_device::saturn_dcc_device(const machine_config &mconfig,
...
void saturn_dcc_device::minit_w(offs_t offset, uint32_t data,
                                uint32_t mem_mask) {
  /* Only a 16-bit write triggers FRT Input Capture on the other SH-2: byte and
     longword writes do not trigger the signal, and the data value is ignored
     either way.  Warning about the width and pulsing regardless meant a title
     writing a longword here got an inter-CPU sync it would never have seen on
     hardware. */
  if (mem_mask != 0x0000ffff && mem_mask != 0xffff0000) {
    logerror("minit_w: ignoring non 16-bit access, mask %08x\n", mem_mask);
    return;
  }
...
void saturn_dcc_device::sinit_w(offs_t offset, uint32_t data,
                                uint32_t mem_mask) {
...
IRQ_CALLBACK_MEMBER(saturn_dcc_device::irq_ack_cb) {
  // stv:colmns97 and stv:wwshin requires correct slave irqs for sound to work
  // properly
