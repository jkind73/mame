...
- [ ] **DSP-02 — Complete DSP DMA and bus interaction. [M/P/R]**
  ...
  - Eight-bit counter / zero-as-256 behavior is now integrated as native WIP after the loader qualified. cd074b71 fails16 of24 real count programs; 20,480 extracted count cases and5,120 program loads pass with the correction. Actual data/program save fixtures now require legal zero-encoded256-word transfers; the earlier60,000-word fixture was stress coverage, not a valid single hardware transfer. See `saturn_pending/evidence/dsp-count/README.md` for the convergent-emulator reference basis and primary-text limitation. The no-PCH rebuild exposed five controller include-order errors, repaired in fc6664a6; the 59-script local batch passes and rebuild35352902888 is pending. A separate unapplied count-selector candidate has 16 canonical native passes/16 unused-bit failures and 131,072 extracted operand/MC-wrap/overlap cases; see `saturn_pending/evidence/dsp-count-operand/README.md`.
