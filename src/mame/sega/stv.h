...
public:
  stv_state(const machine_config &mconfig, device_type type, const char *tag)
      : saturn_state(mconfig, type, tag), m_cart1(*this, "stv_slot1"),
        m_cart2(*this, "stv_slot2"), m_cart3(*this, "stv_slot3"),
        m_cart4(*this, "stv_slot4"), m_rax(*this, "rax"),
        m_protbank(*this, "protbank"), m_eeprom(*this, "eeprom"),
        m_cryptdevice(*this, "315_5881"), m_5838crypt(*this, "315_5838"),
        m_hopper(*this, "hopper"), m_billboard(*this, "billboard"),
        m_ioga(*this, "ioga"), m_ioga_ports(*this, "PORT%c", 'A'),
        m_ioga_counters(*this, "PORTG.%u", 0),
        m_ioga_mahjong{{*this, "P1_KEY%u", 0}, {*this, "P2_KEY%u", 0}},
        m_pdr(*this, "PDR%u", 1), m_cc_digits(*this, "cc_digit%u", 0U) {}
  ...
  // synthesise the factory-default EEPROM image when no dump is available
  void stv_make_default_eeprom() ATTR_COLD;
...
private:
  ...
  load_cart(device_image_interface &image, generic_slot_device *slot);
  DECLARE_DEVICE_IMAGE_LOAD_MEMBER(stv_cart1) {
    return load_cart(image, m_cart1);
  }
  DECLARE_DEVICE_IMAGE_LOAD_MEMBER(stv_cart2) {
    return load_cart(image, m_cart2);
  }
  DECLARE_DEVICE_IMAGE_LOAD_MEMBER(stv_cart3) {
    return load_cart(image, m_cart3);
  }
  DECLARE_DEVICE_IMAGE_LOAD_MEMBER(stv_cart4) {
    return load_cart(image, m_cart4);
  }
  ...
  void batmanfr_sound_comms_w(offs_t offset, uint32_t data,
                              uint32_t mem_mask = ~0);
  ...
  void decathlt_prot_srcaddr_w(offs_t offset, uint32_t data,
                               uint32_t mem_mask = ~0);
...
public:
  ...
  static constexpr feature_type unemulated_features() {
    return feature::CAMERA | feature::PRINTER;
  }
};
