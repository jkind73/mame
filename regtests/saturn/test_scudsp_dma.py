...
struct scudsp_cpu_device {
 enum {DMA_STATE_IDLE,DMA_STATE_WAIT,DMA_STATE_MOVE,LEF=15,EXF=16,ESF=17,T0F=23};
 ...
 uint32_t m_ra=0;bool m_lps_active=false;
 bool m_paused=false,m_step_pending=false;int m_icount=0;bool halt=false;int ddwt=0,ddmv=0;
 unsigned m_suspend_mask=0;
 void suspend(unsigned r,bool){m_suspend_mask|=r;halt=(m_suspend_mask!=0);}
 void resume(unsigned r){m_suspend_mask&=~r;halt=(m_suspend_mask!=0);}
 ...
 int irq_line=-1;void m_out_irq_cb(int n){irq_line=n;}
...
int main(){
 ...
 for(uint32_t upper:{0u,0x100u,0x5500u,0xffff0000u}){
  ...
  scudsp_cpu_device s;s.m_flags=1u<<16;/*EX*/
  ...
  for(unsigned cut:{0u,1u,2u,expected/2+1,expected+1}){
   ...
   scudsp_cpu_device b;b.m_flags=1u<<16;/*EX*/b.m_dma.size=7;b.m_dma.count=13;
 ...
 for(unsigned count:{1u,2u,63u,64u,65u,255u}){
  ...
  for(unsigned cut:{0u,1u,2u,count/2+1,count+1}){
   scudsp_cpu_device a=s;a.m_dma_timer=&a.t;
   for(unsigned i=0;i<cut;++i)a.tick();
   scudsp_cpu_device b;b.m_flags=1u<<16;/*EX*/
  ...
  for(unsigned cut:{0u,1u,2u}){
   scudsp_cpu_device r;r.m_flags=1u<<16;/*EX*/r.m_wa0=0x05a00000/4;r.count_source=count;r.op_dma(op);
   for(unsigned i=0;i<cut;++i)r.tick();
   r.device_reset();
   // device_reset() clears PPAF to its documented initial value 00000000H
   // (ST-097 p.51), so EX=0 and the DSP must be stopped afterwards.
   assert(r.halt&&!(r.m_flags&(1u<<r.EXF))&&!r.m_dma.ex&&!r.ddwt&&!r.ddmv
          &&r.t.delay==attotime::never);
 ...
 for(uint32_t addr:{0x02000000u,0x05800000u,0x06000000u}){
  scudsp_cpu_device s;s.m_flags=1u<<16;/*EX*/s.m_wa0=addr/4;s.op_dma(0xc0001001|(2<<15));s.finish();
 ...
 for(unsigned bank=0;bank<4;++bank){
  scudsp_cpu_device s;s.m_flags=1u<<16;/*EX*/s.m_ra0=0x06000000/4;s.op_dma(0xc0010000|(bank<<8)|65);s.finish();
 ...
 for(unsigned indirect=0;indirect<2;++indirect)for(unsigned count:{1u,2u,63u,255u,256u}){
  scudsp_cpu_device s;s.m_flags=1u<<16;/*EX*/s.m_ra0=0x06010000/4;s.count_source=count;s.m_pc=2;
  ...
  // TOP latches the delay slot (that instruction's own address), so model
  // both the slot and the advanced fetch address.
  s.m_pc=3;s.m_delay=3;s.set_dest_mem_reg_2(12,target);
  assert(s.halt&&s.m_pc==target&&s.m_top==3);
  for(unsigned cut:{0u,1u,2u,count/2+1,count+1}){
   scudsp_cpu_device a=s;a.m_dma_timer=&a.t;
   for(unsigned i=0;i<cut;++i)a.tick();
   scudsp_cpu_device b;b.m_flags=1u<<16;/*EX*/restore(b,a);
 ...
 for(unsigned indirect=0;indirect<2;++indirect){
  scudsp_cpu_device s;s.m_flags=1u<<16;/*EX*/uint32_t addr=alias+0x06010000+mirror*0x100000;
 ...
 for(uint32_t addr:{0x02010000u,0x02a10000u,0x02f10000u,0x05810000u}){
  scudsp_cpu_device s;s.m_flags=1u<<16;/*EX*/s.m_ra0=addr/4;s.op_dma(0xc0000003);s.finish();
 ...
 for(unsigned mode=0;mode<8;++mode){
  scudsp_cpu_device s;s.m_flags=1u<<16;/*EX*/s.m_ra0=addr/4;s.op_dma(0xc0000003|(mode<<15));s.finish();
