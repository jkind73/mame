...
phase=scsp-midi-fifo
python3 saturn_pending/test_scsp_midi_fifo_runtime.py --executable "$ARTIFACT/saturn" \
    --rompath "$ROOT/regtests" --output "$LOG_DIR/$phase" > "$LOG_DIR/$phase.log" 2>&1
grep -q 'SCSP MIDI FIFO: 24 depth/status/drain-window cases passed live' "$LOG_DIR/$phase.log"
for system in saturnjp saturneu stvbios; do
    phase="scsp-midi-fifo-$system-drc"
    python3 saturn_pending/test_scsp_midi_fifo_runtime.py --executable "$ARTIFACT/saturn" \
        --rompath "$ROOT/regtests" --system "$system" --drc \
        --output "$LOG_DIR/$phase" > "$LOG_DIR/$phase.log" 2>&1
    grep -q 'SCSP MIDI FIFO: 24 depth/status/drain-window cases passed live' "$LOG_DIR/$phase.log"
...
phase=scsp-pcm
python3 saturn_pending/test_scsp_pcm_runtime.py --executable "$ARTIFACT/saturn" \
    --rompath "$ROOT/regtests" --output "$LOG_DIR/$phase" > "$LOG_DIR/$phase.log" 2>&1
grep -q 'SCSP PCM: PASS' "$LOG_DIR/$phase.log"
for system in saturnjp saturneu stvbios; do
    phase="scsp-pcm-$system-drc"
    python3 saturn_pending/test_scsp_pcm_runtime.py --executable "$ARTIFACT/saturn" \
        --rompath "$ROOT/regtests" --system "$system" --drc \
        --output "$LOG_DIR/$phase" > "$LOG_DIR/$phase.log" 2>&1
    grep -q 'SCSP PCM: PASS' "$LOG_DIR/$phase.log"
...
phase=scsp-fm
python3 saturn_pending/test_scsp_fm_runtime.py --executable "$ARTIFACT/saturn" \
    --rompath "$ROOT/regtests" --output "$LOG_DIR/$phase" > "$LOG_DIR/$phase.log" 2>&1
grep -q 'SCSP FM: 26 modulation-depth cases match ST-077 Table 4.17' "$LOG_DIR/$phase.log"
for system in saturnjp saturneu stvbios; do
    phase="scsp-fm-$system-drc"
    python3 saturn_pending/test_scsp_fm_runtime.py --executable "$ARTIFACT/saturn" \
        --rompath "$ROOT/regtests" --system "$system" --drc \
        --output "$LOG_DIR/$phase" > "$LOG_DIR/$phase.log" 2>&1
    grep -q 'SCSP FM: 26 modulation-depth cases match ST-077 Table 4.17' "$LOG_DIR/$phase.log"
...
phase=scsp-fx
python3 saturn_pending/test_scsp_fx_runtime.py --executable "$ARTIFACT/saturn" \
    --rompath "$ROOT/regtests" --output "$LOG_DIR/$phase" > "$LOG_DIR/$phase.log" 2>&1
grep -q 'SCSP FX: PASS' "$LOG_DIR/$phase.log"
for system in saturnjp saturneu stvbios; do
    phase="scsp-fx-$system-drc"
    python3 saturn_pending/test_scsp_fx_runtime.py --executable "$ARTIFACT/saturn" \
        --rompath "$ROOT/regtests" --system "$system" --drc \
        --output "$LOG_DIR/$phase" > "$LOG_DIR/$phase.log" 2>&1
    grep -q 'SCSP FX: PASS' "$LOG_DIR/$phase.log"
