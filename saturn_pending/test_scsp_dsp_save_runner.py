...
final='SCSP_DSP_SAVE PASS active=1 late_read=1 replay=exact\n'
...
for bad in [final, ''.join(stages[:-1])+final, ''.join(stages[::-1])+final,
            ...
            good.replace('wrapped=1','wrapped=0'),good+'SCSP_DSP_SAVE FAIL deliberate\n',
...
print('13 SCSP DSP save result-parser controls passed (no emulator execution)')
