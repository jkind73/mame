...
local function image()
    ...
    return {sp:read_u16(0x05a08000),e0,sp:read_u16(base+0xec2),
        sp:read_u32(base+0xe00)&0xffffff,sp:read_u32(base+0xe04)&0xffffff}
end
...
emu.register_frame_done(function()
    ...
    elseif phase=='loaded' and loaded then step(function()
        ...
        local restored=image();for i=1,5 do check('restored_state'..i,restored[i],baseline[i]) end
        local replay=next_sample('replayed');for i=1,5 do check('exact_replay'..i,replay[i],original[i]) end
        if #fails==0 then print('SCSP_DSP_SAVE PASS active=1 late_read=1 replay=exact')
...
def validate_output(text,returncode):
    if (returncode or 'SCSP_DSP_SAVE FAIL' in text or 'LUA ERROR' in text or
        ...
        len(re.findall(r'^SCSP_DSP_SAVE PASS active=1 late_read=1 replay=exact$',text,re.M))!=1):
