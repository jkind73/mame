...
- [ ] **SND-03 — Complete timing, DMA and interrupt qualification. [P/V]**
  ...
  - DMA self-target safety is native-WIP: ignore writes to the DMA control registers, preventing recursive DEXE and keeping cached parameters consistent with visible registers. Primary p.101 explicitly forbids these accesses; this is emulator safety policy, not hardware fidelity or a proven cause of the reported host crash.74 actual-method cases and17 mutants pass; old2f54b074 passes32 ordinary native controls and fails8 safe cached-parameter cases. Matching192-case/four-profile and preceding acceptance pending. Evidence: `saturn_pending/evidence/scsp-dma-safety/README.md`.
