...
- [ ] **SND-03 — Complete timing, DMA and interrupt qualification. [P/V]**
  ...
  - MIDI FIFO depth/status/reset WIP ea928158 is method-qualified with a native negative control: both MIDI buffers are 4 bytes with occupancy tracked separately from the pointers, register 404 reports live MIEMP/MIFULL/MIOVF/MOEMP/MOFULL at bits 8-12 per Figure 4.3 p.28, write-only MOBUF reads 0 per p.35, a full output FIFO rejects writes, input overflow latches MIOVF while keeping queued bytes and retires on drain, device reset clears both FIFOs and the shift registers, and a queued byte leaves at frame completion so the previously qualified output-empty interrupt timing is preserved. 902 actual-method MIDI cases, 80 DMA cases, 966656 IRQ cases, 24 reset cases and 26 compiled mutants pass; old-source and old-binary controls fail cleanly (old 53f73010 binary fails all 24 native FIFO cases with 185 observations, including the four-frame drain-window discriminator). Full 71-script local/CI, build/export and the complete native consumer including 24x4 new FIFO profiles are pending. The frame-completion pop edge, the MIOVF clear edge and fifth-byte handling are reasoned choices, not documented silicon; the Saturn/ST-V drivers bind no MIDI wiring, so input depth/overflow cannot be filled natively. Evidence: `saturn_pending/evidence/scsp-midi-fifo/README.md`.
