...
- [ ] **CPU-03 — Complete and qualify on-chip peripherals. [P/V]**
  ...
  - IMPL-0169 connects the existing NMI assertion event to NMIF latching and DMA stop/start gating, including idle latching; completed payload retains its scheduled completion bookkeeping (syntax-only candidate). Default NMIE0 is the hardware-method scope. Programmable edge selection, AE generation and sub-service stop timing remain open; CPU IRQ delivery and acknowledgement handlers are unchanged.
  - IMPL-0172 removes SH7604's disconnected shadow IRQ-poll latch so on-chip sources request the existing SH-2 interpreter check (syntax-only candidate). Priority, acknowledgement and delay-slot sampling code are unchanged. The inherited latch is already saved; removal of the redundant save entry makes pre0172 SH7604 saves incompatible.
  - IMPL-0173 applies the documented TIER fixed-bit read image (bit0=1, bits6–4=0), retaining the four interrupt enables (syntax-only candidate). All three peers force bit0 high but retain bits6–4; prohibited reserved-bit writes are not a silicon qualification method.
...
- [ ] **SCU-03 — Complete DMA legality and transfer rules. [P/R]**
  ...
  - IMPL-0164 bounds CD-source/C-bus destination heads and tails, retaining buffered source bytes and the aligned four-byte bulk service (syntax-only candidate). Source-side partial-read strobes, exact timing and B-bus placement remain unqualified.
...
- [ ] **DSP-03 — Qualify execution timing and integration. [P/V]**
  ...
  - IMPL-0152 suppresses repeated ENDI source assertions while E remains set, following ST-210 precaution26 (syntax-only implementation candidate). IMPL-0150/0151 do not qualify stepping a program-DMA instruction before its following serialization command; that interaction and END PC readback wording remain open in the handoff.
  - IMPL-0149 separates EX/END execution stopping from device reset and preserves pending slots/DMA across an EX-only stop/restart (implementation candidate, syntax checked only). Reset clears the stored host controls and holds execution through the DSP-owned stop gate. IMPL-0150 adds an ordinary instruction latch, actual fetch-PC readback and LPS fetch holding; it is also a syntax-only implementation candidate, not native acceptance. IMPL-0151 adds masked, stopped-only ES pipeline stepping with saved pending state (syntax-only candidate); exact host/pipeline/DMA edge timing and the recorded loop disagreements remain open.
...
- [ ] **SMPC-03 — Qualify RTC, backup settings and power/reset semantics. [V/R]**
  ...
  - RTC support already exists. IMPL-0153 removes zero-delay ticks at startup/SETTIME and preserves the running timer across machine reset; IMPL-0154 prevents SETSMEM from setting the SETTIME-only STE flag; IMPL-0155 corrects the BCD-century carry at 1999-to2000 (syntax-only implementation candidates). Determine ST-V-specific battery/settings behavior rather than assuming console semantics.
- [ ] **SMPC-04 — Complete peripheral protocol and event routing. [P/V]**
  - Multi-page transport and partial-report file save/load pass live on 234c7abc. Integrated VBlank timeout plus initialized/reset/save-registered H/V edge history after live negative reproduction; native 2781 timeout/edge-save positives pass. Physical slot addressing/FF-empty ports and VBlank-sampled RESB now pass the expanded 79f36021 live gate, including both tap types, empty root ports and real sampled-RESB save/load. IMPL-0159 adds an extended-size report implementation candidate; native extended-report coverage, OPE and wire timing remain open.
  - Direct-port modes, peripheral identification, multitap discovery, packet lengths and handshake timing.
  - IMPL-0159 adds a slot-aware extended-length query, 15/255-byte report framing and unknown-tap ID handling, retaining the existing snapshot/paging flow (syntax-only candidate). Existing accessory choices are unchanged; no registered device currently supplies an extended payload.
...
- [ ] **CD-01 — Complete host command/transfer-state behavior. [P/V]**
  ...
  - Third-review update for `dd21cbcf194`: validator accepts IMPL-0131/0132 and the scaffold repair; native build/`-validate`, all 73 regression scripts (zero skips), the live host fixture (2638 checks) and BIOS save/load/replay succeeded. SCAN cadence is adjudicated in the candidate's favor; Q movement readout remains unadjudicated due to the validator instrument. Destination folded-host/BFUL integration remains open. See `saturn_pending/PROMOTION_STATUS.md` for the pinned report and limits.
