...
- [ ] **DSP-01 — Resolve instruction/flag/control semantics. [P/V/R]**
  ...
  - Control-port S/Z are incorrectly writable despite ST-097 p.51 and both pinned references. An external, not-yet-integrated mask correction passes28672 actual-method UBSan transitions; native ff717b2e passes76/fails36 of112 masked-write programs with all guest flag controls correct. The existing pipeline fixture's invalid host Z injection is replaced by guest ALU setup plus an explicit branch-target marker; all12 revised programs pass on ff717b2e. Prior conditional-flag coverage is superseded. Evidence: `saturn_pending/evidence/dsp-hostflags/README.md`; pause/step and other control semantics remain open.
...
- [ ] **DSP-02 — Complete DSP DMA and bus interaction. [M/P/R]**
  ...
  - C-bus writes now use aligned longword placement, byte strides0/2/4/8/16/32/64/128 for both count forms, and cumulative WA0 updates preserving the saved halfword phase (f1fe8e09, native WIP). Old2e14f275 passes212/fails300 of512 mapped programs; all source-counter controls match. Actual old-binary odd-phase file replay fails both completion images and continuation probes. Corrected actual methods pass6912 UBSan cases with five replay cuts; four mutants fail, existing DMA suites and full-TU syntax pass. The66-script full batch/build and2048-program/four-configuration native gate plus odd-phase save replay are pending. Contract/evidence: `saturn_pending/evidence/dsp-cbus/README.md`. Ymir/Beetle agree on placement/stride but disagree on some final non-HOLD addresses; existing MAME/Beetle cumulative behavior is retained, not declared hardware-proven.
