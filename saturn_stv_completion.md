...
- [ ] **DSP-01 — Resolve instruction/flag/control semantics. [P/V/R]**
  ...
  - Negative OR now follows ST-097 p.95 and both pinned references: S=1,Z=0,C=0 (native WIP). The existing ALU suites expand to222662 actual-method cases and438 native programs, covering every defined flag-changing operation. Old f944ce85 passes390/fails48 programs, solely negative-OR flags/read-clear (all result observations match). The host read-only test no longer expects the legacy impossible S=Z=1 preset. Full69-script/build and1752-program/four-configuration native acceptance are pending. Evidence: `saturn_pending/evidence/dsp-logic/README.md`. The old Croc/Psygnosis comment lacks hardware evidence; those titles' compatibility remains unqualified without their media.
