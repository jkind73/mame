...
public:
  ...
  saturn_scu_device(const machine_config &mconfig, const char *tag,
                    device_t *owner, uint32_t clock);

  template <typename T> void set_hostcpu(T &&tag) {
  ...
  // A-Bus external interrupts: IST bits 16-31, vectors 0x50-0x5f, levels 7
  // (sources 0-3), 4 (sources 4-7) and 1 (sources 8-15).  The CD block
  // drives source 0 with its HIRQ output.
  void abus_external_interrupt(int index, bool state);
  void cd_block_irq_w(int state) { abus_external_interrupt(0, state); }
...
private:
  ...
  enum dma_id : int { DMALV0_ID = 0, DMALV1_ID, DMALV2_ID };
  ...
  enum ist_source_t : uint32_t {
    ...
    IST_DMAILL = 1 << 12,
    IST_VDP1_END = 1 << 13,
    IST_ABUS_EXT0 = 1 << 16 // .. 1 << 31 for the 16 A-Bus sources
  };

  // IST bits 14-15 are unused; ISM bit 15 is a single *active high* enable
  // for all of the A-Bus external interrupts (unlike every other mask bit)
  // and ISM bit 14 isn't writable
  static constexpr uint32_t ISM_ABUS_EXT_ENABLE = 1 << 15;
  static constexpr uint32_t ISM_WRITE_MASK = 0x0000'bfff;

  // background a.k.a. interrupt flag (paused out of higher priority executed)
  // move a.k.a. operation flag (DMA is executing)
  // wait a.k.a. stand by (a starting period where the DMA goes from idle to
  // operating) the move/wait given here are for documentation purposes only,
  // they are impractical and hot for what we need
