...
"""Real file replay while paused during an odd-phase C-bus DMA transfer.
...
subscribers[1]=emu.add_machine_pre_save_notifier(function()
    saved=true;save_clock=emu.time();print('DSP_PAUSE_SAVE saved')
end)
subscribers[2]=emu.add_machine_post_load_notifier(function()
    loaded=true;print('DSP_PAUSE_SAVE loaded')
end)
...
local function finish()
    if #fails==0 then print('DSP_PAUSE_SAVE PASS words=256 paused=1 busy=1 phase=odd replay=exact')
    else for _,f in ipairs(fails) do print('DSP_PAUSE_SAVE FAIL '..f) end end
...
local function step(fn)
    ...
    coroutine.wrap(function()
        ...
        if not ok then print('DSP_PAUSE_SAVE FAIL '..tostring(err));phase='done';emu.unpause();m:exit() end
    end)()
end
emu.register_frame_done(function()
    ...
    if frames>600 and phase~='done' then
        print('DSP_PAUSE_SAVE FAIL bounded save/load wait expired');phase='done';emu.unpause();m:exit();return
    end
    ...
    if phase=='setup' then step(function()
        ...
        check('paused_busy_before_save',sp:read_u32(control)&0x810000,0x800000)
    ...
    elseif phase=='saved' and saved then step(function()
        ...
        emu.unpause();sp:write_u32(control,0x04000000);emu.wait(emu.attotime.from_usec(1))
        check('resume_retains_dma_stall',sp:read_u32(control)&0x810000,0x810000)
        complete();image('original_completion')
    ...
    elseif phase=='loaded' and loaded then step(function()
        ...
        check('paused_busy_restored',sp:read_u32(control)&0x810000,0x800000)
        check('saved_tail_restored',sp:read_u32(base+(count-1)*4),0xdeadbeef)
        emu.unpause();sp:write_u32(control,0x04000000);emu.wait(emu.attotime.from_usec(1))
        check('resume_retains_dma_stall',sp:read_u32(control)&0x810000,0x810000)
        complete();image('replayed_completion');finish()
    end)
    end
end)
print('DSP_PAUSE_SAVE armed')
'''

def validate_output(text, returncode):
    if (returncode or 'DSP_PAUSE_SAVE FAIL' in text or 'LUA ERROR' in text or
            re.findall(r'^DSP_PAUSE_SAVE (saved|mutated|loaded)$',text,re.M)!=['saved','mutated','loaded'] or
            len(re.findall(r'^DSP_PAUSE_SAVE PASS words=256 paused=1 busy=1 phase=odd replay=exact$',text,re.M))!=1):
        raise RuntimeError('DSP save fixture failed:\n'+text[-10000:])

runner.validate_output=validate_output
if __name__=='__main__':
    runner.main('DSP pause save: paused active DMA and independent stall ownership restored')
