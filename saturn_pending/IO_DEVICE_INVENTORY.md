...
| SH7604 FRT clock/compare pins | `ftci_w` rising-edge counter/compare API (IMPL-0025) and `ftoa_wr_callback` / `ftob_wr_callback` logical compare outputs (IMPL-0026, `1168e29a`) | No configured Saturn/ST-V FTCI source, FTO peer or external peripheral added; FTI capture is separate. Physical synchronization/pulse-width margins, drive/standby-HIZ behavior and native IRQ/save qualification remain open. |
