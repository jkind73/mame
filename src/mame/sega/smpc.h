...
class smpc_hle_device : public device_t, public device_rtc_interface {
public:
  // construction/destruction
  smpc_hle_device(const machine_config &mconfig, const char *tag,
  ...
  void set_control_port_tags(T &&tag1, U &&tag2) {
...
private:
  ...
  uint8_t m_oreg[32];
  // Two ports, each with a status byte and at most 15 peripheral reports.
  // Each report can contain an ID, extended length and 255 payload bytes.
  uint8_t m_peripheral_data[2 * (1 + 15 * (2 + 255))]{};
  ...
  const uint32_t m_cmd_table_timing[0x20] = {
      ...
      100 * 1000, 100 * 1000, 100 * 1000, // SYSTEM RESET / ClocK CHaNGe 352 /
                                          // 320
      320 * 1000,                         // INTBACK
      30,         30,         30,         30,
      30, // <unknown>
      70,         40,         30,         30,
      30, // SETTIME / SETSMEM / NMIREQ / RESENAB / RESDISA
      30,         30,         30,         30 // <unknown>
  };
