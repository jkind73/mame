...
The user's objective is the most faithful practical reproduction of Saturn/ST-V hardware in software, with the concrete release goal of earning promotion of the relevant MAME drivers/configurations from MACHINE_NOT_WORKING to working. This is an implementation and integration project, not an open-ended specification audit.
Actively collect and adapt the work already done in the supplied emulators and FPGA implementations to fill MAME's gaps. Research is YOUR responsibility: do not wait for the user to translate the sources into register definitions, algorithms or coding instructions.

REQUIRED SOURCE COLLECTION AND REUSE
Start from the user's source corpus, not from scratch. Known recorded starting points are:
- Sega SDK/manual collection: https://github.com/jkind73/saturnsdk/tree/0fab2c30d6d1aff1a4836352e00a7fc5cd4c7f73 . Includes VDP1 ST-013 and supplements, VDP2 ST-058, SCSP ST-077, SCU ST-097, SMPC ST-169 and sh7604.pdf; also inspect applicable technical notes and supplied Sega/Cassini material.
- Ymir user fork: https://github.com/jkind73/Ymir/tree/6d779960127ced72087a418c1daefc637d0aaa80 . Upstream is https://github.com/StrikerX3/Ymir . Inspect relevant subsystems, not only video.
- MiSTer Saturn: https://github.com/MiSTer-devel/Saturn_MiSTer/tree/a95b085038ace57fa621558d60a7adc7a3c53f78 . Read request/grant and pipeline RTL as well as rendering arithmetic.
- Mednafen user fork: https://github.com/jkind73/mednafen-git/tree/f0ee9d595db68ad5247ba5ac6a8367fdced9c3fc . Saturn implementation is under src/ss/.
- Recover the user's other supplied sources, including Yabause, SaturnRecomp and additional Cassini material, from the repository audit/worklist and recorded source manifests. Their exact revisions are not supplied here: locate and verify them, do not invent URLs or provenance.
These pins establish reproducibility, not a prohibition on newer fixes. Inspect relevant newer work/history when useful and record the exact additional revisions. Missing local caches are not missing knowledge: retrieve available sources. Keep downloads outside tracked source.
For each concrete gap, find existing implementations, compare their hardware assumptions and dependencies, and adapt the best-supported behavior into MAME's device/scheduler/memory architecture. Reuse code when its license permits; preserve attribution/notices and record origins. If direct copying is incompatible, use permissible documentation/behavioral evidence and an independently written implementation, not disguised copied code.
Do not wholesale transplant another emulator, but do not interpret that restriction as a ban on integrating its useful, properly licensed work. Compare timing, state, interrupts and corner cases, not just final pixels or boot success.
Use primary documentation for documented hardware behavior. When it is incomplete or contradictory, inspect the supplied implementations, hardware traces and software use; choose and label the best-supported practical implementation rather than leaving a known functional gap untouched solely because perfect silicon proof is unavailable. Never present a reference-backed approximation as measured hardware truth.
Maintain a per-parent source manifest: URL/revision, document pages or source functions, adopted behavior, differences, licensing, tests and unresolved questions. Exhaust relevant available sources before asking the user to perform research or declaring an information blocker. Do not assume the corpus proves every undocumented detail.

TWO DISTINCT ACCEPTANCE LEVELS
1. Working-driver readiness: actual supported configurations boot and perform their intended functions, with integrated CPU/video/audio/media/input behavior and representative sustained runtime tests. Identify and fix real blockers. A flag edit or a BIOS screenshot is not evidence of readiness.
2. Fidelity completion: finish the assigned documented behavior, remaining timing/edge cases and validation. Record unsupported options and genuine hardware uncertainty honestly.
Prioritize changes that improve both fidelity and real functionality. Do not hold working-driver readiness hostage to proving every undocumented silicon corner or every optional accessory. Conversely, do not call working status complete hardware emulation. Do not stop your assignment merely because readiness is reached; continue its actionable fidelity work.
Agent four must assess MACHINE_NOT_WORKING and remaining imperfect/device flags per affected configuration on the integrated build. Do not remove flags en masse or promise upstream acceptance; maintainers determine upstream promotion. MAME working status need not imply the absence of every imperfection, and a base configuration can work while an optional board remains unsupported.
...
- Prioritize earning working-driver status through real functionality. Evaluate `MACHINE_NOT_WORKING` and remaining imperfect/device flags individually, using sustained configuration-specific evidence; working status is not identical to exhaustive silicon fidelity or automatic upstream acceptance.
