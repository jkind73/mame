...
int main(){saturn_state s;unsigned cases=0;
 ...
 for(unsigned address:{0u,0x40u,0x80u,0x7fffcu,0xffffcu})for(unsigned parameter:{1u,2u})
 for(unsigned bit=0;bit<32;++bit)for(unsigned pattern:{0u,0xffffffffu,0x12345678u}){
  ...
  uint32_t words[24];
  for(unsigned i=0;i<24;++i)s.m_vdp2_vram[(base+i)&0x3ffff]=words[i]=(1u<<bit)^(pattern*(i+1));
