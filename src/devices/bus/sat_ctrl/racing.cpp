...
DEFINE_DEVICE_TYPE(SATURN_WHEEL, saturn_wheel_device, "saturn_racing",
                   "Sega Saturn Racing Wheel")

static INPUT_PORTS_START(saturn_racing) PORT_START("JOY")
                        ...
                        PORT_BIT(0x0400, IP_ACTIVE_LOW, IPT_BUTTON1)
                            PORT_NAME("A") PORT_BIT(0x0200, IP_ACTIVE_LOW,
                                ...
                                PORT_BIT(0x0100, IP_ACTIVE_LOW, IPT_BUTTON2)
                                    PORT_NAME("B")
    // the wheel has no R and L switches, those bits are always reported high
    ...
    PORT_BIT(0x0007, IP_ACTIVE_LOW, IPT_UNUSED)
        ...
        PORT_START("ANALOG_X") PORT_BIT(0xff, 0x80, IPT_AD_STICK_X)
            PORT_MINMAX(0x00, 0xff) PORT_SENSITIVITY(25) PORT_KEYDELTA(200)
                PORT_NAME("Racing Stick X") INPUT_PORTS_END
...
saturn_wheel_device::saturn_wheel_device(const machine_config &mconfig,
...
uint8_t saturn_wheel_device::read_ctrl(uint8_t offset) {
  ...
  default:
    ...
    break;
  case 1:
    // second data byte is 1, X, Y, Z, 1, 1, 1, 1: no R and L switches
    res = (m_joy->read() & 0x77) | 0x88;
    break;
  case 2:
    res = m_anx->read();
    break;
