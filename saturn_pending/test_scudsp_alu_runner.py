...
"""DSP ALU result-parser controls; not emulator qualification."""
from test_scudsp_alu_runtime import validate_output
rows=[f'DSP_ALU case={i} PASS\n' for i in range(1,439)]
final='DSP_ALU PASS cases=438\n'
...
for bad in [final,''.join(rows[:-1])+final,''.join(rows[::-1])+final,
            ...
            good+'DSP_ALU FAIL deliberate\n',good+'LUA ERROR deliberate\n',
            good.replace('cases=438\n','cases=438 extra\n')]:
...
print('12 DSP ALU result-parser controls passed (no emulator execution)')
