...
saturn_rom_device::saturn_rom_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock)
		...
		// DRAM (5Ah/5Ch) and battery RAM (21h-24h) carts report.  Technical Bulletin
		// #47 says that address is shared with "Power Memory", and Technical Bulletin
		// #46 identifies data cartridges by a System ID header inside the cart ROM
		// instead, beginning "SEGASATURN DATA" at offset 00H.  So 0xff is a "no ID"
		// placeholder rather than a hardware value; machine_start() keys the ROM
		// window off it, and saturn_cart_type_r() reporting it for a data cart is
		// harmless because nothing identifies a data cart that way.
