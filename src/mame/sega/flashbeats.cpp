...
class flashbeats_spectrum_device : public device_t,
                                   public device_sound_interface {
public:
  static constexpr int BANDS = 5;

  flashbeats_spectrum_device(const machine_config &mconfig, const char *tag,
...
protected:
  ...
  virtual void sound_stream_update(sound_stream &stream) override {
    ...
    const double release =
        std::exp(-1.0 / (0.050 * m_rate)); // ~50ms RC release
...
private:
  ...
  void recompute_filters() {
    static constexpr double CENTER[BANDS] = {63.0, 250.0, 1000.0, 2500.0,
                                             10000.0};
    static constexpr double Q = 1.0;
    for (int b = 0; b < BANDS; b++) {
      const double w0 =
          2.0 * std::numbers::pi * std::min(CENTER[b] / m_rate, 0.45);
...
DEFINE_DEVICE_TYPE(FLASHBEATS_SPECTRUM, flashbeats_spectrum_device,
                   "flashbeats_spectrum",
                   "Flash Beats spectrum display filter board")

namespace {

flashbeats_spectrum_device::flashbeats_spectrum_device(
    const machine_config &mconfig, const char *tag, device_t *owner,
    uint32_t clock)
    : device_t(mconfig, FLASHBEATS_SPECTRUM, tag, owner, clock),
      device_sound_interface(mconfig, *this), m_stream(nullptr), m_rate(0),
      m_b0{}, m_b2{}, m_a1{}, m_a2{}, m_z1{}, m_z2{}, m_env{} {}
...
class flashbeats_state : public driver_device {
  ...
  static constexpr int LANE_LEN =
      47; // LEDs per lane (even bytes 0..92 of each 0x60 row at 0xa0c000)
...
public:
  flashbeats_state(const machine_config &mconfig, device_type type,
                   const char *tag)
      : driver_device(mconfig, type, tag), m_maincpu(*this, "maincpu"),
        m_scspcpu(*this, "scspcpu"), m_scsp(*this, "scsp"),
        m_spectrum(*this, "spectrum"), m_sound_ram(*this, "sound_ram"),
        m_dsb2(*this, "dsb2"), m_eeprom(*this, "eeprom"),
        m_315_5296(*this, "segaio1"), m_315_5338a(*this, "segaio2"),
        m_dispram(*this, "dispram"), m_dmd(*this, "dmddot%u", 0U),
        m_lane_red(*this, "lane%u_%u_r", 0U, 0U),
        m_lane_green(*this, "lane%u_%u_g", 0U, 0U),
        m_btn_lamp(*this, "btnlamp%u", 0U), m_lamp_flash(*this, "lamp_flash"),
        m_lamp_up(*this, "lamp_up"), m_lamp_dn(*this, "lamp_dn"),
...
private:
  ...
  required_shared_ptr<uint16_t>
      m_dispram; // 0xa00000 display work RAM (incl. lanes @0xa0c000)
  output_finder<DMD_W * DMD_H>
      m_dmd; // 64x16 DMD dots (dmddot0..1023) -> .lay artwork
  output_finder<LANE_COUNT, LANE_LEN> m_lane_red; // red plane -> .lay artwork
  output_finder<LANE_COUNT, LANE_LEN>
      m_lane_green; // green plane -> .lay artwork
  output_finder<10>
      m_btn_lamp; // btnlamp0-9 -> .lay (0-4 = P1 lanes 1-5, 5-9 = P2 lanes 1-5)
  output_finder<>
      m_lamp_flash; // lamp_flash -> .lay (gameplay cue: return on the FLASH)
  output_finder<> m_lamp_up, m_lamp_dn,
      m_lamp_start; // lamp_up/dn/start -> .lay

  uint8_t m_spectrum_mux =
      0; // analog spectrum-display source select latch @0xa18000
};

void flashbeats_state::machine_start() { save_item(NAME(m_spectrum_mux)); }
...
// The two planes are independent; the game routinely sets both for the same
// LED. Each LED is published as TWO independent on/off outputs -- the red die
// (lane<r>_<c>_r) and the green die (lane<r>_<c>_g) -- and the .lay overlays
// them with blend="add", so an LED with both dies lit sums to yellow the same
// way the real diffused bi-color package does.
//
// PLANE WIDTHS DIFFER -> 47 LEDs, adjacent-OR green:
// Measured across all attract/meter frames, the RED plane lights cells 0..46
// (47 cells, 1:1 with the 47 physical LEDs) but the GREEN plane lights cells
// 0..47 (48 cells). The 48-cell green source maps onto the 47 LEDs by OR-ing
// each cell with its neighbour: LED_green[i] = green_cell[i] | green_cell[i+1].
//
...
void flashbeats_state::update_lanes() {
  ...
  for (int lane = 0; lane < LANE_COUNT; lane++) {
    for (int i = 0; i < LANE_LEN; i++) {
      ...
      const bool red =
          disp8[(0xa0c000 + off) & 0xffff] != 0; // P0: 47 cells, 1:1 per LED
      // P1: 48 green cells OR'd down to 47 LEDs (this cell OR the next).
      const bool green = disp8[(0xa0c1e0 + off) & 0xffff] != 0 ||
                         disp8[(0xa0c1e0 + off + 2) & 0xffff] != 0;
...
void flashbeats_state::update_dmd() {
  ...
  for (int y = 0; y < DMD_H; y++)
    for (int x = 0; x < DMD_W; x++)
      m_dmd[y * DMD_W + x] =
          disp8[(DMD_BASE + y * DMD_PITCH + x * 2) & 0xffff] & 0x0f;
}
...
uint8_t flashbeats_state::p6_r() { return (m_eeprom->do_read() << 3); }
...
uint8_t flashbeats_state::spectrum_mux_r() { return m_spectrum_mux; }

void flashbeats_state::spectrum_mux_w(uint8_t data) { m_spectrum_mux = data; }

void flashbeats_state::flashbeats_map(address_map &map) {
  map(0x000000, 0x1fffff).rom().region("maincpu", 0);
  map(0x200000, 0x20ffff).ram();
  map(0x400000, 0x40007f)
      .rw(m_315_5296, FUNC(sega_315_5296_device::read),
          FUNC(sega_315_5296_device::write))
      .umask16(0xff00);
  map(0x600000, 0x60001f)
      .rw("telio", FUNC(te7752_device::read), FUNC(te7752_device::write))
      .umask16(0xff00);
  map(0x800000, 0x80001f)
      .rw(m_315_5338a, FUNC(sega_315_5338a_device::read),
          FUNC(sega_315_5338a_device::write))
      .umask16(0xff00);
  map(0xa00000, 0xa0ffff)
      .ram()
      .share("dispram"); // display work RAM; lanes built at 0xa0c000
  map(0xa10000, 0xa10fff).ram();
  map(0xa18000, 0xa18001)
      .rw(FUNC(flashbeats_state::spectrum_mux_r),
          FUNC(flashbeats_state::spectrum_mux_w))
      .umask16(0xff00);
}
...
void flashbeats_state::main_scsp_map(address_map &map) {
  ...
  // Sample ROM (rom3) exposed to the sound-68000, Model 2 style (see
  // model2_snd: 0x800000-0x9fffff = "samples"). rom3 lives at offset 0x80000 in
  // the "scspcpu" region (after rom4's 0x80000 of code). The firmware stages
  // PCM from here into sound_ram (the SCSP wave/DSP DRAM) itself; the SCSP then
  // plays it from RAM, so the DSP keeps its writable work area (mapping rom3
  // straight into scsp_mem as ROM droned because the DSP read/writes that same
  // space).
...
void flashbeats_state::flashbeats(machine_config &config) {
  ...
  H83007(config, m_maincpu,
         16_MHz_XTAL); // 16 MHz oscillator next to chip, also 16 MHz causes
                       // SCI0 and 1 rates to be 31250 (MIDI)
  ...
  // own async receiver (31250 bps, 8N1 -- matches the H8 SCI1 rate above), so
  // we wire the H8's per-bit TX line straight in, exactly like model2/3 route
  // their i8251 TXD to scsp_device::midi_in. No byte reassembly needed. SCI0 TX
  // -> DSB2 is wired below, next to the DSB2 device.
  ...
  TIMER(config, "lane_timer")
      .configure_periodic(FUNC(flashbeats_state::lane_update_timer),
                          attotime::from_hz(480));
  ...
  te7752.out_port3_cb().set(FUNC(
      flashbeats_state::te7752_port3_w)); // P1 lane lamps (see impl comment)
  te7752.out_port4_cb().set(
      FUNC(flashbeats_state::te7752_port4_w)); // P2 lane lamps + up/dn arrows
                                               // (see impl comment)
  te7752.out_port5_cb().set(
      FUNC(flashbeats_state::te7752_port5_w)); // flash light (see impl comment)
  te7752.out_port6_cb().set(
      FUNC(flashbeats_state::te7752_port6_w)); // start lamp (see impl comment)
  // out_port7 deliberately left unhooked: confirmed continuous clock/strobe
  // toggle unrelated to lamps.

  SPEAKER(config, "speaker", 2).front();

  // ST-077: the SCSP re-sampling frequency is fixed at 44.1 kHz and one
  // sample is 512 master clocks, so the chip must be fed 512 * 44100 =
  // 22.5792 MHz; m_scspcpu above runs at half the SCSP clock, consistent
  // with the Saturn/ST-V sound subsystem this board reuses
  ...
  // digitally -- the game leaves EFSDL=0 on slots 16/17, so an EXTS hookup
  // would mute it). Only the 16:1 SCSP:DSB2 ratio is ground truth; the absolute
...
// Input mappings
// 3 bits seem genuinely dead (scanner never reads them) and left PORT_BIT(...,
// IPT_UNUSED).
static INPUT_PORTS_START(flashbeats)
    // 315-5296 port A -- @0x400000.
    PORT_START("IN_A") PORT_BIT(0x01, IP_ACTIVE_LOW, IPT_BUTTON5) PORT_NAME(
        "P2 Attack 1") PORT_PLAYER(2) PORT_BIT(0x02, IP_ACTIVE_LOW, IPT_BUTTON4)
        PORT_NAME("P2 Attack 2") PORT_PLAYER(2)
            PORT_BIT(0x04, IP_ACTIVE_LOW, IPT_BUTTON3) PORT_NAME("P2 Attack 3")
                PORT_PLAYER(2) PORT_BIT(0x08, IP_ACTIVE_LOW, IPT_BUTTON2)
                    PORT_NAME("P2 Attack 4") PORT_PLAYER(2)
                        PORT_BIT(0x10, IP_ACTIVE_LOW, IPT_BUTTON1)
                            PORT_NAME("P2 Attack 5") PORT_PLAYER(2) PORT_BIT(
                                0xe0, IP_ACTIVE_LOW,
                                IPT_UNUSED) // dead lines (scanner never reads)
    ...
    PORT_START("IN_B") PORT_BIT(0x01, IP_ACTIVE_LOW, IPT_COIN1)
        PORT_BIT(0x02, IP_ACTIVE_LOW, IPT_COIN2)
            PORT_SERVICE_NO_TOGGLE(0x04, IP_ACTIVE_LOW)
                PORT_NAME("Test") // TEST button (menu open/select)
    PORT_BIT(0x08, IP_ACTIVE_LOW, IPT_SERVICE1)
        PORT_NAME("Service") // SERVICE button (credit + menu advance)
    PORT_BIT(0x10, IP_ACTIVE_LOW, IPT_JOYSTICK_UP) PORT_NAME("Select 1 (Up)")
        PORT_PLAYER(1) PORT_BIT(0x20, IP_ACTIVE_LOW, IPT_JOYSTICK_DOWN)
            PORT_NAME("Select 2 (Down)") PORT_PLAYER(1)
                PORT_BIT(0x40, IP_ACTIVE_LOW, IPT_START1) PORT_NAME("Start")
                    PORT_BIT(0x80, IP_ACTIVE_LOW,
                             IPT_UNKNOWN) // unconfirmed, not a dead line

    // 315-5296 port D -- @0x400006.
    // Bits 0-4 quiet in attract - P2's 5 lane buttons (mirrors port A's P1
    // layout). Bit 7 is unknown (unconfirmed, not dead). Bits 5-6 are dead
    // lines (scanner never reads them).
    PORT_START("IN_D") PORT_BIT(0x01, IP_ACTIVE_LOW, IPT_BUTTON1) PORT_NAME(
        "P1 Attack 1") PORT_PLAYER(1) PORT_BIT(0x02, IP_ACTIVE_LOW, IPT_BUTTON2)
        PORT_NAME("P1 Attack 2") PORT_PLAYER(1)
            PORT_BIT(0x04, IP_ACTIVE_LOW, IPT_BUTTON3) PORT_NAME("P1 Attack 3")
                PORT_PLAYER(1) PORT_BIT(0x08, IP_ACTIVE_LOW, IPT_BUTTON4)
                    PORT_NAME("P1 Attack 4") PORT_PLAYER(1)
                        PORT_BIT(0x10, IP_ACTIVE_LOW, IPT_BUTTON5)
                            PORT_NAME("P1 Attack 5") PORT_PLAYER(1) PORT_BIT(
                                0x60, IP_ACTIVE_LOW,
                                IPT_UNUSED) // dead lines (scanner never reads)
    ...
    // (~N & 0x0f).
    PORT_START("DSW1") PORT_DIPNAME(0x0f, 0x0f, DEF_STR(Coinage))
        PORT_DIPLOCATION("SW1:1,2,3,4")
    ...
    // cabinet-documented "ADVERTISE SOUND" (Japanese label: 内部/外部,
    // "internal/ external"). This is a sound-source selector, not a demo-sounds
    // mute - confirmed by a full 24-bit input sweep during attract mode that
    // found no input bit (DIP or otherwise) silences advertise sound. Traced
    // the boot read itself (PC 0x69C-0x6A8): bit 7 does gate a conditional
    ...
    PORT_START("DSW2") PORT_DIPNAME(0x03, 0x03, "Credits To Start (1P)")
        PORT_DIPLOCATION("SW2:1,2") PORT_DIPSETTING(0x03, "1") PORT_DIPSETTING(
            0x02, "2") PORT_DIPSETTING(0x01, "3") PORT_DIPSETTING(0x00, "4")
            PORT_DIPNAME(0x0c, 0x0c, "Credits To Start (VS)") PORT_DIPLOCATION(
                "SW2:3,4") PORT_DIPSETTING(0x0c, "1") PORT_DIPSETTING(0x08, "2")
                PORT_DIPSETTING(0x04, "3") PORT_DIPSETTING(
                    0x00, "4") PORT_DIPNAME(0x30, 0x30, "Credits To Continue")
                    PORT_DIPLOCATION("SW2:5,6") PORT_DIPSETTING(0x30, "1")
                        PORT_DIPSETTING(0x20, "2") PORT_DIPSETTING(0x10, "3")
                            PORT_DIPSETTING(0x00, "4")
                                PORT_DIPUNUSED_DIPLOC(0x40, 0x40, "SW2:7")
                                    PORT_DIPNAME(0x80, 0x80, "Advertise Sound")
                                        PORT_DIPLOCATION("SW2:8")
    ...
    ***************************************************************************/
    ...
    ROM_START(flsbeats) ROM_REGION(0x200000, "maincpu", 0) ROM_LOAD16_WORD_SWAP(
        "epr-21609_rom1.ic18", 0x000000, 0x080000,
        CRC(130a0a62) SHA1(400f24304959547b188ed874653ae2e1e77092fe))

        ROM_REGION(0x280000, "scspcpu", 0) ROM_LOAD16_WORD_SWAP(
            "epr-21610_rom4.ic14", 0x000000, 0x080000,
            CRC(c877e0e6) SHA1(595f143fb3789852a4af9d2920cbaefabecfa45c))
            ROM_LOAD16_WORD_SWAP(
                "epr-21611_rom3.ic4", 0x080000, 0x200000,
                CRC(2f5dc574) SHA1(f0b8d076b0fc8e94582de0ca17ecd5c8b90bedc4))

                ROM_REGION(0x20000, "dsb2:mpegcpu", 0) ROM_LOAD16_WORD_SWAP(
                    "epr-21612.ic2", 0x000000, 0x020000,
                    CRC(6912e1cb)
                        SHA1(3497d6ae0b9be00116a3278f46d738c4c6f26d20))

                    ROM_REGION(0x2000000, "dsb2:mpeg", 0) ROM_LOAD(
                        "mpr-21601_n26_9852k7016.ic18", 0x0000000, 0x400000,
                        CRC(d23e2b7b)
                            SHA1(8c26a740fee0adc4d45d34786b0c28abb105324d))
                        ROM_LOAD(
                            "mpr-21602_n27_9852k7017.ic19", 0x0400000, 0x400000,
                            CRC(e143960b)
                                SHA1(7ace5cae6f2a8868d74d4397a9c1b0a0f6f26c9f))
                            ROM_LOAD(
                                "mpr-21603_n28_9852k7018.ic20", 0x0800000,
                                0x400000,
                                CRC(136b69d8) SHA1(
                                    ad81be6f0383f29306c8d9f21d1e7440172ebf97))
                                ROM_LOAD(
                                    "mpr-21604_n29_9852k7019.ic21", 0x0c00000,
                                    0x400000,
                                    CRC(87b15ea4) SHA1(
                                        eacc0f575926e68a195090370cb9e9e06b38e404))
                                    ROM_LOAD(
                                        "mpr-21605_n30_9852k7023.ic22",
                                        0x1000000, 0x400000,
                                        CRC(fb1a802e) SHA1(
                                            6fa99694973fd3cdd1a0f74a655583aadf5adc8f))
                                        ROM_LOAD(
                                            "mpr-21606_n31_9852k7020.ic23",
                                            0x1400000, 0x400000,
                                            CRC(d9aba5e0) SHA1(
                                                6f3f1b01174d771a56f92aeead998827da97ac22))
                                            ROM_LOAD(
                                                "mpr-21607_n32_9852k7021.ic24",
                                                0x1800000, 0x400000,
                                                CRC(f3dd07c6) SHA1(
                                                    aa9d056e8ff5d2282917a09c42711062b8df989a))
                                                ROM_LOAD(
                                                    "mpr-21608_n33_9852k7022."
                                                    "ic25",
                                                    0x1c00000, 0x400000,
                                                    CRC(be4db836) SHA1(
                                                        93d4cbb3bb299e3cf1dda105670e3923751c28ad))
                                                    ROM_END

} // anonymous namespace

//    YEAR  NAME      PARENT  MACHINE     INPUT       CLASS             INIT
//    MONITOR  COMPANY  FULLNAME       FLAGS
GAME(1998, flsbeats, 0, flashbeats, flashbeats, flashbeats_state, empty_init,
     ROT0, "Sega", "Flash Beats", 0)
