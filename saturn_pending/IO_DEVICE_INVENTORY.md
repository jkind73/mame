...
Scope: **IO-02 / STV-05 / EXP-02**, source inventory at `3d18666d` on
...
|---|---|---|
| SH7604 SCI | Internal-clock async TX/RX callbacks, register/IRQ logic; IMPL-0007/0008/0009/0010 refine status, chained TX, receive errors and MP receive filtering | No Saturn/ST-V machine-config binding of `txd_wr_callback` / `rxd_rd_callback` in `sat_console.cpp` or `stv.cpp`; external SCK and synchronous transfer absent; SCI-specific DMA request/ack routing absent. RX sampling phase and native IRQ/save behavior remain open. |
