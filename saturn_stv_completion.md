...
- [ ] **SND-02 — Qualify the complete SCSP voice engine. [V/R]**
  ...
  - Slot LFO correction (WIP, method-qualified): LFORE was decoded with zero call sites, so the documented p.89 reset/start idiom could not work; it is now held per sample in UpdateSlot before both modulation paths, with the p.37 noise exemption that neither Beetle nor MiSTer models. Separately, the 8.8 phase increment truncated every Table 4.21 setting below 0.68 Hz (LFOF 00H-07H, 0.17-0.55 Hz) to zero, i.e. no modulation at all; the accumulator is now 8.24 with rounded increments, so all 32 documented frequencies oscillate within 1%. 328578 extracted-method cases (all 65536 LFO register words for both mapping and hold, all 32x4x7 step combinations, manual Table 4.21 goldens, one-cycle wrap), UpdateSlot wiring checks and twelve compiled mutants pass; the pre-change source fails. LFO state is not register-readable and no audio capture path exists, so this is method-level evidence only, with no claim that any game's audio is now correct. Pre-change save states are not loadable (phase widened u16 to u32). Evidence: `saturn_pending/evidence/scsp-lfo/README.md`.
