 ...
 *
 *   sh7021.h
 *   Shared Hitachi SH-1 (SH7032/SH7021) family peripherals
 *
 *****************************************************************************/

#ifndef MAME_CPU_SH_SH1_H
#define MAME_CPU_SH_SH1_H

#pragma once

#include "sh2.h"

// Hitachi SH-1 family (SH7032/SH7021) common implementation.  The CPU core is
// the shared SH-2/SH-1 interpreter; everything below it is the on-chip
// peripheral set the family has in common.  Parts differ only in their
// internal RAM/ROM layout, which each subclass installs in its own map.
class sh1_device : public sh2_device
{
public:
	// External interrupt pins IRQ0-IRQ7.  The integrator asserts them through
	// set_input_line(); IPRA/IPRB set the level and ICR the sense mode.
	static constexpr int IRQ0 = 0, IRQ7 = 7;
...
protected:
	sh1_device(const machine_config &mconfig, device_type type, const char *tag, device_t *owner, uint32_t clock,
	...
	// External interrupt pins: the level comes from IPRA/IPRB and the sense
	// mode from ICR, and the SH-1 gives each pin its own vector (64 + n).
	...
	void sh1_peripheral_map(address_map &map) ATTR_COLD;

	// External DMA requests (/DREQ0, /DREQ1)
	bool external_dma_selected(int ch) const;
	void step_external_dma(int ch);
	...
	uint16_t m_ipre = 0;
	uint16_t m_icr = 0;
	int8_t m_irq_pin_state[8] = { 0, 0, 0, 0, 0, 0, 0, 0 };
	bool m_irq_request[8] = { false, false, false, false, false, false, false, false };
	...
	struct
	{
		...
		bool dei = false;   // transfer end interrupt pending
	} m_dma[4];
	uint16_t m_dmaor = 0;   // DMA Operation Register (status flags)
	bool m_dreq[2] = { false, false }; // /DREQ0, /DREQ1 request inputs
	int m_dma_cycles;
	...
	template<int which> TIMER_CALLBACK_MEMBER(sh1_timer_callback);
	template<int which> TIMER_CALLBACK_MEMBER(sh1_sci_callback);
