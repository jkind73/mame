...
p.add_argument('--mutation',choices=('phase','cell','mosaic','two-word'))
...
funcs=[extract(src,sig) for sig in ('rgb_t saturn_state::vdp2_line_color(', 'rgb_t saturn_state::vdp2_dot_pixel(', 'rgb_t saturn_state::vdp2_pattern_pixel(', 'rgb_t saturn_state::vdp2_scroll_pixel(', 'void saturn_state::vdp2_draw_scroll_screen(')]
...
int main(){saturn_state s;auto &t=s.current_tilemap;unsigned cases=0;
 ...
 for(unsigned phase:{0u,0x8000u})for(unsigned variant=0;variant<6;++variant)for(bool line_color:{false,true}){
  t={};t.enabled=1;t.bitmap_enable=bitmap;t.colour_depth=depth;t.tile_size=large;t.pattern_data_size=one;
  ...
  t.mosaic_screen_enabled=variant>=4;t.transparency=variant&1;t.colour_calculation_enabled=variant%3!=0;t.alpha=112;
  ...
  auto pixel=[&](int X,int Y){
   unsigned address,pal=0,bitpos=0,bpd[]={4,8,16,16,32};
   if(bitmap){unsigned W=(t.bitmap_size&2)?1024:512,H=(t.bitmap_size&1)?512:256;
    ...
    uint32_t name=bits(off,0,nb*8);unsigned code=0,flip=0;
   ...
   if(depth==3){unsigned r=raw%32,g=raw/32%32,b=raw/1024%32;return 0xff000000|((r*8+r/4)<<16)|((g*8+g/4)<<8)|(b*8+b/4);}
   if(depth==4)return 0xff000000|((raw%256)<<16)|(raw&0xff00)|((raw>>16)&255);
   if(depth==1)pal=pal/256%8*256;
   if(depth==2)pal=0;
   return s.pal.pen((pal+raw+2*256)%2048);
  };
  ...
  for(int y=1;y<=14;++y){
   ...
   for(int x=0;x<=21;++x){
    ...
    if(!t.colour_calculation_enabled)expected.pix(y,x)=color;
    else{
