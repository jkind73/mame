...
- [ ] **DSP-01 — Resolve instruction/flag/control semantics. [P/V/R]**
  ...
  - Parallel X/Y/D1 accesses now share entry CT addresses, merge increments once after D1, honor explicit CT writes, and suppress same-bank/read-bank D1 RAM writes (native WIP). Qualified280c40e1 passes36/fails108 guest programs with all multiplier controls intact. Corrected actual methods pass729,000 UBSan transitions; native gate adds576 programs/four configurations. `saturn_pending/evidence/dsp-parallel/README.md` explicitly records Ymir/Beetle disagreements on register priority and an immediate-collision CT quirk; those are not hardware-qualified.
