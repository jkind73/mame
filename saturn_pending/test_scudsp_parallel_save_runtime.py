...
"""Actual active parallel-copy loop: CT, RAM and multiplier state file replay."""
import re
...
subscribers[1]=emu.add_machine_pre_save_notifier(function()
    saved=true;save_clock=emu.time();print('DSP_PARALLEL_SAVE saved')
end)
subscribers[2]=emu.add_machine_post_load_notifier(function()
    loaded=true;print('DSP_PARALLEL_SAVE loaded')
end)
local function fill(poison)
    sp:write_u32(addr,0)
    for i=0,63 do sp:write_u32(data,poison and 0xdeadbeef or (0x12340000+i)) end
    for i=1,128 do sp:write_u32(data,0xdeadbeef) end
end
...
local function complete(label)
    ...
    for i=0,63 do check(label..'_word'..i,sp:read_u32(data),0x12340000+i) end
    local product=(0x1234003f*0x1234003f)&0xffffffffffff
    sp:write_u32(addr,128)
    check(label..'_product_low',sp:read_u32(data),product&0xffffffff)
    check(label..'_product_high',sp:read_u32(data),(product>>16)&0xffffffff)
end
local function step(fn)
    phase='busy'
    coroutine.wrap(function()
        local ok,err=pcall(fn)
        if not ok then print('DSP_PARALLEL_SAVE FAIL '..tostring(err));phase='done';emu.unpause();m:exit() end
    end)()
end
emu.register_frame_done(function()
    frames=frames+1;assert(subscribers[1] and subscribers[2])
    if frames>600 and phase~='done' then
        print('DSP_PARALLEL_SAVE FAIL bounded save/load wait expired');phase='done';emu.unpause();m:exit();return
    end
    ...
    if phase=='setup' then step(function()
        ...
        print('DSP_PARALLEL_SAVE observed partial loop')
    ...
    elseif phase=='saved' and saved then step(function()
        ...
        fill(true)
        -- Poison CTs, LOP, RX/RY/product, program and source/destination RAM.
        upload({0x1c0b,0x1d16,0x1e21,0xa8000000,0x2080000,0xf0000000})
        sp:write_u32(addr,0);check('poisoned_source',sp:read_u32(data),0xdeadbeef)
        print('DSP_PARALLEL_SAVE mutated')
    ...
    elseif phase=='loaded' and loaded then step(function()
        ...
        if #fails==0 then print('DSP_PARALLEL_SAVE PASS words=64 replay=exact')
        else for _,f in ipairs(fails) do print('DSP_PARALLEL_SAVE FAIL '..f) end end
        phase='done';m:exit()
    end)
    end
end)
print('DSP_PARALLEL_SAVE armed')
'''
def validate_output(text,returncode):
    if (returncode or 'DSP_PARALLEL_SAVE FAIL' in text or 'LUA ERROR' in text or
            re.findall(r'^DSP_PARALLEL_SAVE (observed partial loop|saved|mutated|loaded)$',text,re.M)!=['observed partial loop','saved','mutated','loaded'] or
            len(re.findall(r'^DSP_PARALLEL_SAVE observed partial loop$',text,re.M))!=1 or
            len(re.findall(r'^DSP_PARALLEL_SAVE PASS words=64 replay=exact$',text,re.M))!=1):
        raise RuntimeError('DSP parallel save fixture failed:\n'+text[-16000:])
runner.validate_output=validate_output
if __name__=='__main__':
    runner.main('DSP parallel save: active copy loop, RAM, counters and multiplier replay restored')
