...
// Wojcieszek, R. Belmont thanks-to: Guru, Fabien, Runik, Charles MacDonald
/**************************************************************************************************
...
TODO:
- aclub: "Error 11 call attendant", bypass by turning on maintenance mode in
game test.
\- Horrible voice pitches (SCU streaming DMA to SCSP with LV2);
\- VDP1 Missing contours for time indicator (flowers), enables CCCR = 0x0067 in
VDP2;
\- will hang anyway when supposed to print report and/or dispense the oil (tries
to use IOGA serial);

- colmns97: has a bit of stuck envelope sound in places (i.e. coin in once at
title screen);

- critcrsh: 2 digits 7-seg LED stuck on hi-score during gameplay, has optional
ticket dispenser; Reference video: https://www.youtube.com/watch?v=O9PyIKdSFnU

- danchih / danchiq: https://mametesters.org/view.php?id=6270 (should be fixed)

- fanzonem: press game service reset button on "ROM has changed", afterwards it
will throw a "Door Open" due of missing sub-board comms

- fhboxers: needs emulating main board CN18 in A-Bus Dummy space for multicab
connection;

- magzun: emulate microphone (which type?) over serial connection (needs
315-5649 mods);

- myfairld: https://mametesters.org/view.php?id=2642 (currently also draws VDP1
very badly)

- smleague / finlarch: it randomly hangs / crashes, it works if you use a
ridiculous set_maximum_quantum() number, might need strict SH-2 synching or it's
actually a m68k comms issue.

- stress: accesses the Sound Memory Expansion Area (0x05a80000-0x05afffff),
unknown purpose if any;
...
- wasafari: eventually prompts for rotating the crank, which is untestable by
service mode. Enables VDP2 RBG0 layer but doesn't draw at all (lack of a
linescroll effect when scrolling sideways looks dubious)

- wwshin: has plenty of VDP2 window glitches for being a simple game;

- yattrmnp: throws "S ERR=305 SATELLITE CHACKER JAM" (sic), alternates with
"DOOR OPEN"
...
#define FIRST_SPEEDUP_SLOT                                                     \
  (2) // in case we remove/alter the BIOS speedups later

#define DUMP_CART_HEADER (0)

/*
Sega 315-5649 IO IC, functional same as 315-5338A, also used in Model 2/3,
integrated into 315-6146 'MIE' MCU, etc I/O overview: Connector (as described in
service manual) PORT-A  1st player inputs   JAMMA (56P) PORT-B  2nd player
inputs   JAMMA (56P) PORT-C  system input        JAMMA (56P) PORT-D  system
output       JAMMA (56P) + CN25 (JST NH 5P) RESERVED OUTPUT 4bit. (?) PORT-E I/O
1               CN32 (JST NH 9P) EXTENSION I/O 8bit. PORT-F  I/O 2 CN21 (JST NH
11P) EXTENSION I/O 8bit. PORT-G  I/O 3               CN20 (JST HN 10P) EXTENSION
INPUT 8bit. (?) PORT-AD AD-Stick inputs?    CN19 (JST NH 12P) A/D INPUT 8Ch.
...
offsets:
    ...
    0x000d PORT-G
    0x000f unused
    0x0011 PORT_DIRECTION (each bit configure above IO ports, 1 - input, 0 -
output)
    ...
    0x001b RS422 FLAG
    0x001d MODE (bit 7 - set PORT-G to counter-mode, bits 0-5 - RS422 satellite
mode and node#)  <Technical Bowling> 0x001f PORT-AD (8ch, write: bits 0-2 - set
channel, read: channel data with autoinc channel number)
*/
...
uint8_t stv_state::ioga_r(offs_t offset) {
  ...
  case 0x07:
    res = m_system_output;
    break; // port D, read-backs value written
  case 0x1b:
    res = 0;
    break; // Serial COM READ status
  case 0x1d:
    res = m_ioga_mode;
    break;
  }
...
uint8_t stv_state::critcrsh_ioga_r(offs_t offset) {
  ...
  case 0x03:
    ...
    res |=
        (ioport("PORTC")->read() & 0x10) ? 0x0 : 0x4; // x/y hit latch actually
    break;
  default:
    res = ioga_r(offset);
    break;
...
void stv_state::critcrsh_ioga_w(offs_t offset, uint8_t data) {
  static uint8_t const bcd2hex[] = {
      0x3f, 0x06, 0x5b, 0x4f, 0x66, 0x6d, 0x7c, 0x07, 0x7f,
      0x67, 0x58, 0x4c, 0x62, 0x49, 0x78, 0x00}; // TODO: chip type unknown
  switch (offset * 2 + 1) {
  case 0x0b:
...
uint8_t stv_state::stvmp_ioga_r(offs_t offset) {
  ...
  case 0x01:
  case 0x03:
    if (m_port_sel & 0x10) // joystick select <<< this is obviously wrong, this
                           // bit only selects PORTE direction
    {
      res = ioga_r(offset);
...
uint32_t stv_state::common_prot_r(offs_t offset) {
  ...
  if (m_abus_protenable & 0x00010000) // protection calculation is activated
  {
    ...
    if (m_a_bus[offset] != 0)
      ...
      return ROM[(0x02fffff0 / 4) + offset];
  }
}
...
void stv_state::common_prot_w(offs_t offset, uint32_t data, uint32_t mem_mask) {
  ...
  if (offset == 0) {
    ...
    if (ACCESSING_BITS_16_31)
      m_cryptdevice->set_addr_low(data >> 16);
    if (ACCESSING_BITS_0_15)
      m_cryptdevice->set_addr_high(data & 0xffff);
...
void stv_state::install_common_protection() {
  ...
  m_maincpu->space(AS_PROGRAM)
      .install_write_handler(
          0x4fffff0, 0x4ffffff,
          write32s_delegate(*this, FUNC(stv_state::common_prot_w)));
}
...
/*
    ST-V EEPROM factory defaults.

    The ST-V BIOS validates a CRC-16/CCITT-FALSE protected settings block in
    the cabinet EEPROM (93C46) and reports a damaged or changed EEPROM when
    it doesn't match.  Very few systems have a dumped EEPROM image, and an
    empty region would leave the BIOS stuck on the error screen, so
    synthesise the factory contents from the cart header instead.

    Layout (big-endian words, matching the ROMREGION16_BE region):
        0x00  "SEGA"
        0x08  CRC-16 over 0x0c..0x41, XORed with the word at 0x42
        0x0f  0x01 when the default settings word is unmodified, else 0x02
        0x19  0x08
        0x1a  settings (rotate/cabinet/player count)
        0x1c  game ID, copied from cart ROM + 0xf40
        0x1e  per-game settings, copied from cart ROM + 0xf48 (8 bytes)
        0x44  mirror of 0x08..0x43
*/
void stv_state::stv_make_default_eeprom() {
  memory_region *const region = memregion("eeprom");
  if (!region || (region->bytes() < 0x80))
    return;

  uint8_t *const data = region->base();

  // only synthesise an image when the region is blank (i.e. no dump was
  // supplied); regions without ROM entries are zero- or 0xff-filled
  bool blank = true;
  for (u32 i = 1; i < 0x80 && blank; i++)
    blank = (data[i] == data[0]);

  if (!blank)
    return;

  std::memset(data, 0xff, 0x80);
  std::memcpy(data + 0x00, "SEGA", 4);

  // orientation comes from the system flags
  const u16 rotate =
      (machine().system().flags & machine_flags::MASK_ORIENTATION) ? 1 : 0;

  // the cabinet type, game ID and per-game defaults live in the cart header
  u8 cab_players = 2;
  u16 game_id = 0xffff;
  u8 game_settings[8];
  std::memset(game_settings, 0xff, sizeof(game_settings));

  memory_region *const cart = memregion("cart");
  if (cart && (cart->bytes() >= 0x1000)) {
    const u8 *const rom = cart->base();

    // locate the mandatory "SEGA ST-V(TITAN)" header, allowing for the
    // alternate layouts some carts use (see the header dump in init_stv)
    const u32 base = ((cart->bytes() > 0x200003) && (rom[0x200000 ^ 3] == 'S'))
                         ? 0x200000
                         : 0;
    const bool shifted = (base == 0) && (rom[0] == 'E');
    const u32 endian = shifted ? 1 : 3;
    const u32 shift = shifted ? 1 : 0;

    auto cart_byte = [&cart, rom, base, endian, shift](u32 addr) -> u8 {
      const u32 offs = ((base + addr) ^ endian) << shift;
      return (offs < cart->bytes()) ? rom[offs] : 0xff;
    };

    const u8 cab = cart_byte(0xf46);
    if ((cab == 0x01) || (cab == 0x03))
      cab_players = 1;
    else if (cab == 0x10)
      cab_players = 3;
    else
    ...
    game_id = (u16(cart_byte(0xf40)) << 8) | u16(cart_byte(0xf41));
    for (u32 i = 0; i < 8; i++)
      game_settings[i] = cart_byte(0xf48 + i);
  }

  const u16 settings =
      (rotate << 12) | ((cab_players - 1) & 3) | (1 << 5) | (1 << 6) | 0x089c;

  data[0x0c] = 0x00;
  data[0x0d] = 0x00;
  data[0x0e] = 0x00;
  data[0x0f] = 0x01 + ((settings != 0x08fd) ? 1 : 0);
  data[0x10] = 0x01;
  data[0x11] = 0x00;
  data[0x12] = 0x01;
  data[0x13] = 0x01;
  std::memset(data + 0x14, 0x00, 5);
  data[0x19] = 0x08;
  data[0x1a] = u8(settings >> 8);
  data[0x1b] = u8(settings & 0xff);
  data[0x1c] = u8(game_id >> 8);
  data[0x1d] = u8(game_id & 0xff);
  std::memcpy(data + 0x1e, game_settings, 8);

  // CRC-16/CCITT-FALSE over 0x0c..0x41, XORed with the (0xffff) word at 0x42
  u16 crc = util::crc16_creator::simple(data + 0x0c, 0x36, 0x5a81);
  crc ^= (u16(data[0x42]) << 8) | u16(data[0x43]);
  data[0x08] = u8(crc >> 8);
  data[0x09] = u8(crc & 0xff);

  // the settings block is mirrored at 0x44
  std::memcpy(data + 0x44, data + 0x08, 0x3c);

  logerror("stv: synthesised factory EEPROM contents (game ID %04x, %u "
           "player(s), rotate %u)\n",
           game_id, cab_players, rotate);
}
...
void stv_state::init_stv() {
  ...
  // do strict overwrite verification - maruchan and rsgun crash after coinup
  // without this. cottonbm needs strict PCREL todo: test what games need this
  // and don't turn it on for them...
  ...
  // (BIOS defaults to no cart mapped, have to enter in game test mode to make
  // it appear in debugger)
#if DUMP_CART_HEADER
...
uint32_t stv_state::magzun_hef_hack_r() {
  if (m_maincpu->pc() == 0x604bf20)
    return 0x00000001; // HWEF

  if (m_maincpu->pc() == 0x604bfbe)
    return 0x00000002; // HREF

  return m_workram_h[0x08e830 / 4];
}

uint32_t stv_state::magzun_rx_hack_r() {
  if (m_maincpu->pc() == 0x604c006)
    return 0x40;
...
void stv_state::init_magzun() {
  ...
  //	m_maincpu->space(AS_PROGRAM).install_read_handler(0x608e830, 0x608e833,
  //read32smo_delegate(*this, FUNC(stv_state::magzun_hef_hack_r)));
  //	m_maincpu->space(AS_PROGRAM).install_read_handler(0x60ff3b4, 0x60ff3b7,
  //read32smo_delegate(*this, FUNC(stv_state::magzun_rx_hack_r)));
...
void stv_state::init_prikura() {
  ...
  */
  ...
  //  m_minit_boost_timeslice = m_sinit_boost_timeslice =
  //  attotime::from_usec(50);
}
...
/* these idle loops might change if the interrupts change / are fixed because i
 * don't really think these are vblank waits... */
...
void stv_state::init_batmanfr() {
  ...
  m_maincpu->space(AS_PROGRAM)
      .install_write_handler(
          0x04800000, 0x04800003,
          write32s_delegate(*this, FUNC(stv_state::batmanfr_sound_comms_w)));
...
uint32_t stv_state::decathlt_prot_r(offs_t offset, uint32_t mem_mask) {
  // needs to be a way to indicate if device is enabled and fall through to
  // cartridge data if not?
  ...
  if (mem_mask & 0xffff0000)
    ret |= (m_5838crypt->data_r() << 16);
  if (mem_mask & 0x0000ffff)
    ret |= m_5838crypt->data_r();
  return ret;
}

void stv_state::decathlt_prot_srcaddr_w(offs_t offset, uint32_t data,
                                        uint32_t mem_mask) {
  int offs = offset * 4;

  m_protbankval = (offs & 0x1800000) >> 23;
  m_protbank->set_entry(
      m_protbankval); // if the protection device is accessed at this address
                      // data is fetched from 0x02000000
...
void stv_state::init_decathlt() {
  m_maincpu->space(AS_PROGRAM)
      .install_write_handler(
          ...
          write32s_delegate(
              *this,
              FUNC(stv_state::decathlt_prot_srcaddr_w))); // set compressed data
                                                          // source address,
                                                          // write data

  // really needs installing over the whole range, with fallbacks to read rom if
  // device is disabled or isn't accessed on given address
  m_maincpu->space(AS_PROGRAM)
      .install_read_handler(
          0x27ffff8, 0x27ffffb,
          read32s_delegate(
              *this,
              FUNC(stv_state::decathlt_prot_r))); // read decompressed data
  m_maincpu->space(AS_PROGRAM)
      .install_read_handler(
          0x2fffff8, 0x2fffffb,
          read32s_delegate(*this, FUNC(stv_state::decathlt_prot_r))); //  ^
  m_maincpu->space(AS_PROGRAM)
      .install_read_handler(
          0x37ffff8, 0x37ffffb,
          read32s_delegate(*this, FUNC(stv_state::decathlt_prot_r))); //  ^
...
void stv_state::init_stv_us() {
  init_stv();
  m_smpc_hle->set_region_code(4); // 4 - USA, configured via jumpers, should be
                                  // added to input ports as DIPSW ?
}
...
void stv_state::stv_mem(address_map &map) {
  map(0x00000000, 0x0007ffff)
      .rom()
      .mirror(0x20080000)
      .region("bios", 0); // bios, mirrored across the 1MB CS0 window
  map(0x00100000, 0x0010007f)
      .mirror(0x2007ff80)
      .m(m_smpc_hle, FUNC(smpc_hle_device::io_map));
  map(0x00180000, 0x0018ffff)
      .rw(FUNC(stv_state::backupram_r), FUNC(stv_state::backupram_w))
      .mirror(0x20070000)
  ...
  map(0x05c80000, 0x05cbffff)
      .mirror(0x40000)
      .rw(FUNC(stv_state::vdp1_framebuffer0_r),
          FUNC(stv_state::vdp1_framebuffer0_w)); // only the back buffer is
                                                 // visible, mirrored across the
                                                 // 512KB window
  ...
  map(0x05f80000, 0x05fbffff)
      .rw(FUNC(stv_state::vdp2_regs_r), FUNC(stv_state::vdp2_regs_w));
...
void stv_state::critcrsh_mem(address_map &map) {
  stv_mem(map);
  map(0x00400000, 0x0040003f)
      .rw(FUNC(stv_state::critcrsh_ioga_r), FUNC(stv_state::critcrsh_ioga_w))
      .umask32(0x00ff00ff);
}
...
void stv_state::stvmp_mem(address_map &map) {
  stv_mem(map);
  map(0x00400000, 0x0040003f)
      .rw(FUNC(stv_state::stvmp_ioga_r), FUNC(stv_state::stvmp_ioga_w))
      .umask32(0x00ff00ff);
}
...
void stv_state::hopper_mem(address_map &map) {
  stv_mem(map);
  map(0x00400000, 0x0040003f)
      .rw(FUNC(stv_state::ioga_r), FUNC(stv_state::hop_ioga_w))
      .umask32(0x00ff00ff);
}
...
void stv_state::stvcd_mem(address_map &map) {
  ...
  map(0x05800000, 0x0589ffff)
      .m("saturn_cd_hle", FUNC(saturn_cd_hle_device::amap));
}
...
uint8_t stv_state::pdr1_input_r() { return (m_pdr[0]->read() & 0x40) | 0x3f; }
...
void stv_state::pdr2_output_w(uint8_t data) {
  m_audiocpu->set_input_line(INPUT_LINE_RESET,
                             (data & 0x10) ? ASSERT_LINE : CLEAR_LINE);
...
void stv_state::stv(machine_config &config) {
  ...
  TIMER(config, "scantimer")
      .configure_scanline(FUNC(stv_state::saturn_scanline), "screen", 0, 1);
  ...
  m_smpc_hle->set_region_code(1); // 1 - Japan, configured via jumpers, should
                                  // be added to input ports as DIPSW ?
  ...
  //  m_smpc_hle->sound_reset_handler().set(FUNC(stv_state::sound_68k_reset_w));
  //  // ST-V games controls reset line via PDR2
  ...
  m_ioga->out_pe_callback().set(
      [this](uint8_t data) { m_billboard->write(data); });
...
void stv_state::batmanfr_sound_comms_w(offs_t offset, uint32_t data,
                                       uint32_t mem_mask) {
  // FIXME
  if (ACCESSING_BITS_16_31)
...
#define STV_CARTSLOT_ADD(_tag, _load)                                          \
  GENERIC_CARTSLOT(config, _tag, generic_plain_slot, "stv_cart")               \
      .set_device_load(FUNC(stv_state::_load));
...
void stv_state::machine_reset() {
  ...
  if (m_cart1)
    m_cart_reg[0] = memregion(
        region_tag.assign(m_cart1->tag()).append(GENERIC_ROM_REGION_TAG));
  else
    m_cart_reg[0] = memregion("cart");
  if (m_cart2)
    m_cart_reg[1] = memregion(
        region_tag.assign(m_cart2->tag()).append(GENERIC_ROM_REGION_TAG));
  else
    m_cart_reg[1] = nullptr;
  if (m_cart3)
    m_cart_reg[2] = memregion(
        region_tag.assign(m_cart3->tag()).append(GENERIC_ROM_REGION_TAG));
  else
    m_cart_reg[2] = nullptr;
  if (m_cart4)
    m_cart_reg[3] = memregion(
        region_tag.assign(m_cart4->tag()).append(GENERIC_ROM_REGION_TAG));
  else
...
stv_state::load_cart(device_image_interface &image, generic_slot_device *slot) {
  uint32_t const size = slot->common_get_size("rom");

  if (!image.loaded_through_softlist())
    return std::make_pair(image_error::UNSUPPORTED,
                          "Cartridges must be loaded from the software list");
...
static INPUT_PORTS_START(stv) PORT_START("PDR1")
    PORT_DIPNAME(0x40, 0x40, "PDR1") // P1 Gun Trigger
    PORT_DIPSETTING(0x40, DEF_STR(Off)) PORT_DIPSETTING(0x00, DEF_STR(On))

        PORT_START("PDR2") PORT_DIPNAME(0x01, 0x01, "PDR2")
            PORT_DIPSETTING(0x01, DEF_STR(Off)) PORT_DIPSETTING(0x00,
                                                                DEF_STR(On))
                PORT_DIPNAME(0x02, 0x02, DEF_STR(Unknown)) // test mode mirror
    PORT_DIPSETTING(0x02, DEF_STR(Off)) PORT_DIPSETTING(0x00, DEF_STR(On))
        PORT_DIPNAME(0x04, 0x04, DEF_STR(Unknown)) // service button mirror
    PORT_DIPSETTING(0x04, DEF_STR(Off)) PORT_DIPSETTING(0x00, DEF_STR(On))
    ...
    PORT_DIPSETTING(0x40, DEF_STR(Off)) PORT_DIPSETTING(0x00, DEF_STR(On))
        ...
        PORT_START("PORTA") STV_PLAYER_INPUTS(1, BUTTON1, BUTTON2, BUTTON3,
                                              BUTTON4)

            PORT_START("PORTB") STV_PLAYER_INPUTS(2, BUTTON1, BUTTON2, BUTTON3,
                                                  BUTTON4)

                PORT_START("PORTC") PORT_BIT(0x01, IP_ACTIVE_LOW, IPT_COIN1)
                    PORT_BIT(0x02, IP_ACTIVE_LOW,
                             IPT_COIN2) PORT_SERVICE_NO_TOGGLE(0x04,
                                                               IP_ACTIVE_LOW)
                        PORT_BIT(0x08, IP_ACTIVE_LOW,
                                 IPT_SERVICE1) PORT_BIT(0x10, IP_ACTIVE_LOW,
                                                        IPT_START1)
                            PORT_BIT(0x20, IP_ACTIVE_LOW,
                                     IPT_START2) PORT_BIT(0x40, IP_ACTIVE_LOW,
                                                          IPT_SERVICE)
                                PORT_NAME("1P Push Switch (Multi Cart Select)")
                                    PORT_CODE(KEYCODE_7) // selects game in
                                                         // multi-cart mode
    PORT_BIT(0x80, IP_ACTIVE_LOW, IPT_SERVICE)
        PORT_NAME("2P Push Switch (Pause)")
            PORT_CODE(KEYCODE_8) // for games that supports it, most of them
                                 // needs to be enabled in game assignment first

    PORT_START("PORTE") STV_PLAYER_INPUTS(3, BUTTON1, BUTTON2, BUTTON3, START)

        PORT_START("PORTF") STV_PLAYER_INPUTS(4, BUTTON1, BUTTON2, BUTTON3,
                                              START)

            PORT_START("PORTG") PORT_BIT(0xff, IP_ACTIVE_LOW, IPT_UNUSED)

                PORT_START("PORTG.0") PORT_BIT(0x0000, IP_ACTIVE_LOW,
                                               IPT_UNUSED) PORT_START("PORTG.1")
                    PORT_BIT(0x0000, IP_ACTIVE_LOW, IPT_UNUSED) PORT_START(
                        "PORTG.2") PORT_BIT(0x0000, IP_ACTIVE_LOW, IPT_UNUSED)
                        PORT_START("PORTG.3") PORT_BIT(0x0000, IP_ACTIVE_LOW,
                                                       IPT_UNUSED)

                            PORT_START("AN0") PORT_BIT(0xff, IP_ACTIVE_LOW,
                                                       IPT_UNUSED)

                                PORT_START("AN1") PORT_BIT(0xff, IP_ACTIVE_LOW,
                                                           IPT_UNUSED)
                                    ...
                                    PORT_START("AN2") PORT_BIT(0xff,
                                        ...
                                        PORT_START("AN3") PORT_BIT(
                                            0xff, IP_ACTIVE_LOW, IPT_UNUSED)

                                            PORT_START("AN4") PORT_BIT(
                                                0xff, IP_ACTIVE_LOW, IPT_UNUSED)

                                                PORT_START("AN5") PORT_BIT(
                                                    0xff, IP_ACTIVE_LOW,
                                                    IPT_UNUSED)

                                                    PORT_START("AN6") PORT_BIT(
                                                        0xff, IP_ACTIVE_LOW,
                                                        IPT_UNUSED)
    ...
    static INPUT_PORTS_START(batmanfr) PORT_INCLUDE(stv)

        PORT_MODIFY("PORTA") PORT_BIT(0x01, IP_ACTIVE_LOW, IPT_UNUSED) PORT_BIT(
            0x02, IP_ACTIVE_LOW,
            IPT_BUTTON1) PORT_PLAYER(1) PORT_NAME("P1 Jump")
            PORT_BIT(0x04, IP_ACTIVE_LOW,
                     IPT_BUTTON2) PORT_PLAYER(1) PORT_NAME("P1 Punch")
                PORT_BIT(0x08, IP_ACTIVE_LOW,
                         IPT_BUTTON3) PORT_PLAYER(1) PORT_NAME("P1 Kick")

                    PORT_MODIFY("PORTB") PORT_BIT(0x01, IP_ACTIVE_LOW,
                                                  IPT_UNUSED)
                        PORT_BIT(0x02, IP_ACTIVE_LOW,
                                 IPT_BUTTON1) PORT_PLAYER(2)
                            PORT_NAME("P2 Jump") PORT_BIT(0x04, IP_ACTIVE_LOW,
                                                          IPT_BUTTON2)
                                PORT_PLAYER(2) PORT_NAME("P2 Punch")
                                    PORT_BIT(0x08, IP_ACTIVE_LOW, IPT_BUTTON3)
                                        PORT_PLAYER(2) PORT_NAME("P2 Kick")

                                            PORT_MODIFY("PORTE")
                                                PORT_BIT(0xff, IP_ACTIVE_LOW,
                                                         IPT_UNUSED)
    ...
    static INPUT_PORTS_START(stv6b) PORT_INCLUDE(stv)

        PORT_MODIFY("PORTA") PORT_BIT(0x08, IP_ACTIVE_LOW, IPT_UNUSED)

            PORT_MODIFY("PORTB") PORT_BIT(0x08, IP_ACTIVE_LOW, IPT_UNUSED)

                PORT_MODIFY("PORTE") PORT_BIT(0xff, IP_ACTIVE_LOW, IPT_UNUSED)

    /* Extra button layout, used by Power Instinct 3, Suikoenbu, Elan Doree,
       Golden Axe Duel & Astra SuperStars */
    PORT_MODIFY("PORTF") PORT_BIT(0x01, IP_ACTIVE_LOW, IPT_BUTTON4)
        PORT_PLAYER(1) PORT_BIT(0x02, IP_ACTIVE_LOW, IPT_BUTTON5) PORT_PLAYER(1)
                        ...
                        PORT_BIT(0x20, IP_ACTIVE_LOW, IPT_BUTTON5)
                            PORT_PLAYER(2)
                                PORT_BIT(0x40, IP_ACTIVE_LOW, IPT_BUTTON6)
                                    PORT_PLAYER(2)
                                        PORT_BIT(0x80, IP_ACTIVE_LOW,
                                                 IPT_UNUSED) INPUT_PORTS_END

    static INPUT_PORTS_START(critcrsh) PORT_INCLUDE(stv)

        PORT_MODIFY("PORTA") PORT_BIT(0xff, IP_ACTIVE_LOW, IPT_UNUSED)

            PORT_MODIFY("PORTB") PORT_BIT(0xff, IP_ACTIVE_LOW, IPT_UNUSED)

                PORT_MODIFY("PORTC") PORT_BIT(0x10, IP_ACTIVE_LOW, IPT_BUTTON1)
                    PORT_NAME("Hammer Hit") PORT_BIT(0x20, IP_ACTIVE_LOW,
                                                     IPT_UNUSED)

                        PORT_MODIFY("PORTE") PORT_BIT(0xff, IP_ACTIVE_LOW,
                                                      IPT_UNUSED)

                            PORT_MODIFY("PORTF") PORT_BIT(0xff, IP_ACTIVE_LOW,
                                                          IPT_UNUSED)

                                PORT_START("LIGHTX") /* mask default type sens
                                                        delta min max */
    PORT_BIT(0x3f, 0x00, IPT_LIGHTGUN_X) PORT_CROSSHAIR(X, 1.0, 0.0, 0)
        PORT_MINMAX(0, 0x3f) PORT_SENSITIVITY(50) PORT_KEYDELTA(1)
            PORT_PLAYER(1)

                PORT_START("LIGHTY") PORT_BIT(0x3f, 0x02, IPT_LIGHTGUN_Y)
                    PORT_CROSSHAIR(Y, 1.0, 0.0, 0) PORT_MINMAX(0x2, 0x2e)
                        PORT_SENSITIVITY(50) PORT_KEYDELTA(1)
                            PORT_PLAYER(1) INPUT_PORTS_END

    /* Same as the regular one, but with an additional & optional mahjong panel
     */
    static INPUT_PORTS_START(stvmp) PORT_INCLUDE(stv)

        PORT_MODIFY("PORTE") PORT_BIT(0xff, IP_ACTIVE_LOW, IPT_UNUSED)

            PORT_MODIFY("PORTF") PORT_BIT(0xff, IP_ACTIVE_LOW, IPT_UNUSED)

    /* Mahjong panel/player 1 side */
    PORT_START("P1_KEY0") PORT_BIT(0x01, IP_ACTIVE_LOW, IPT_MAHJONG_KAN) PORT_BIT(
        0x02, IP_ACTIVE_LOW,
        IPT_START1) PORT_BIT(0x04, IP_ACTIVE_LOW,
                             IPT_UNUSED) PORT_BIT(0x08, IP_ACTIVE_LOW,
                                                  IPT_UNUSED) PORT_BIT(0x10, IP_ACTIVE_LOW, IPT_MAHJONG_E)
        PORT_BIT(0x20, IP_ACTIVE_LOW, IPT_MAHJONG_A) PORT_BIT(
            0x40, IP_ACTIVE_LOW,
            IPT_MAHJONG_M) PORT_BIT(0x80, IP_ACTIVE_LOW, IPT_MAHJONG_I)

            PORT_START("P1_KEY1") PORT_BIT(
                0x01, IP_ACTIVE_LOW,
                IPT_MAHJONG_REACH) PORT_BIT(0x02, IP_ACTIVE_LOW, IPT_UNUSED)
                PORT_BIT(0x04, IP_ACTIVE_LOW, IPT_UNUSED) PORT_BIT(
                    0x08, IP_ACTIVE_LOW,
                    IPT_UNUSED) PORT_BIT(0x10, IP_ACTIVE_LOW, IPT_MAHJONG_F)
                    PORT_BIT(0x20, IP_ACTIVE_LOW, IPT_MAHJONG_B) PORT_BIT(
                        0x40, IP_ACTIVE_LOW,
                        ...
                        PORT_START("P1_KEY2") PORT_BIT(
                            ...
                            PORT_BIT(0x04, IP_ACTIVE_LOW, IPT_UNUSED) PORT_BIT(
                                0x08, IP_ACTIVE_LOW,
                                IPT_UNUSED) PORT_BIT(0x10, IP_ACTIVE_LOW, IPT_MAHJONG_G)
                                PORT_BIT(0x20, IP_ACTIVE_LOW, IPT_MAHJONG_C) PORT_BIT(
                                    0x40, IP_ACTIVE_LOW,
                                    ...
                                    PORT_START("P1_KEY3") PORT_BIT(
                                        ...
                                        PORT_BIT(0x04, IP_ACTIVE_LOW, IPT_UNUSED) PORT_BIT(
                                            0x08, IP_ACTIVE_LOW,
                                            IPT_UNUSED) PORT_BIT(0x10, IP_ACTIVE_LOW, IPT_MAHJONG_H)
                                            PORT_BIT(0x20, IP_ACTIVE_LOW, IPT_MAHJONG_D) PORT_BIT(
                                                0x40, IP_ACTIVE_LOW,
                                                IPT_MAHJONG_PON) PORT_BIT(0x80, IP_ACTIVE_LOW,
                                                                          IPT_MAHJONG_L)

                                                PORT_START("P1_KEY4") PORT_BIT(
                                                    0x01, IP_ACTIVE_LOW,
                                                    IPT_UNUSED)
                                                    PORT_BIT(0x02,
                                                             IP_ACTIVE_LOW,
    ...
    PORT_START("P2_KEY0") PORT_BIT(0x01, IP_ACTIVE_LOW, IPT_MAHJONG_KAN) PORT_PLAYER(2) PORT_BIT(
        0x02, IP_ACTIVE_LOW, IPT_START2) PORT_BIT(0x04, IP_ACTIVE_LOW,
                                                  IPT_UNUSED)
        PORT_BIT(0x08, IP_ACTIVE_LOW, IPT_UNUSED) PORT_BIT(
            0x10, IP_ACTIVE_LOW,
            IPT_MAHJONG_E) PORT_PLAYER(2) PORT_BIT(0x20,
                                                   IP_ACTIVE_LOW, IPT_MAHJONG_A)
            PORT_PLAYER(2) PORT_BIT(0x40, IP_ACTIVE_LOW, IPT_MAHJONG_M) PORT_PLAYER(2) PORT_BIT(
                0x80, IP_ACTIVE_LOW,
                IPT_MAHJONG_I) PORT_PLAYER(2)

                PORT_START("P2_KEY1") PORT_BIT(
                    0x01, IP_ACTIVE_LOW,
                    IPT_MAHJONG_REACH) PORT_PLAYER(2) PORT_BIT(0x02,
                                                               IP_ACTIVE_LOW, IPT_UNUSED)
                    PORT_BIT(0x04, IP_ACTIVE_LOW, IPT_UNUSED) PORT_BIT(
                        0x08, IP_ACTIVE_LOW,
                        IPT_UNUSED) PORT_BIT(0x10, IP_ACTIVE_LOW, IPT_MAHJONG_F)
                        PORT_PLAYER(2) PORT_BIT(0x20, IP_ACTIVE_LOW, IPT_MAHJONG_B) PORT_PLAYER(2) PORT_BIT(
                            0x40, IP_ACTIVE_LOW,
                            IPT_MAHJONG_N) PORT_PLAYER(2) PORT_BIT(0x80,
                                                                   IP_ACTIVE_LOW, IPT_MAHJONG_J) PORT_PLAYER(2)

                            PORT_START("P2_KEY2") PORT_BIT(
                                0x01, IP_ACTIVE_LOW,
                                IPT_MAHJONG_RON) PORT_PLAYER(2) PORT_BIT(0x02, IP_ACTIVE_LOW, IPT_UNUSED)
                                PORT_BIT(0x04, IP_ACTIVE_LOW, IPT_UNUSED) PORT_BIT(
                                    0x08, IP_ACTIVE_LOW,
                                    IPT_UNUSED) PORT_BIT(0x10, IP_ACTIVE_LOW, IPT_MAHJONG_G)
                                    PORT_PLAYER(2) PORT_BIT(
                                        0x20, IP_ACTIVE_LOW,
                                        IPT_MAHJONG_C) PORT_PLAYER(2) PORT_BIT(0x40,
                                                                               IP_ACTIVE_LOW, IPT_MAHJONG_CHI)
                                        PORT_PLAYER(2) PORT_BIT(
                                            0x80, IP_ACTIVE_LOW,
                                            IPT_MAHJONG_K) PORT_PLAYER(2)

                                            PORT_START("P2_KEY3") PORT_BIT(0x01, IP_ACTIVE_LOW, IPT_UNUSED) PORT_BIT(
                                                0x02, IP_ACTIVE_LOW,
                                                IPT_UNUSED) PORT_BIT(0x04, IP_ACTIVE_LOW, IPT_UNUSED)
                                                PORT_BIT(0x08, IP_ACTIVE_LOW, IPT_UNUSED) PORT_BIT(
                                                    0x10, IP_ACTIVE_LOW,
                                                    IPT_MAHJONG_H) PORT_PLAYER(2)
                                                    PORT_BIT(0x20, IP_ACTIVE_LOW, IPT_MAHJONG_D) PORT_PLAYER(
                                                        2) PORT_BIT(0x40, IP_ACTIVE_LOW, IPT_MAHJONG_PON)
                                                        PORT_PLAYER(2) PORT_BIT(
                                                            0x80, IP_ACTIVE_LOW,
    ...
    static INPUT_PORTS_START(myfairld) PORT_INCLUDE(stv)
        ...
        PORT_MODIFY("PORTA") PORT_BIT(0xff, IP_ACTIVE_LOW, IPT_UNUSED)
            ...
            PORT_MODIFY("PORTB") PORT_BIT(0xff, IP_ACTIVE_LOW, IPT_UNUSED)
                ...
                PORT_MODIFY("PORTC") PORT_BIT(
                    0x10, IP_ACTIVE_LOW,
                    IPT_UNUSED) PORT_CONDITION("IO_TYPE", 0x01, EQUALS, 0x00)
                    PORT_BIT(0x10, IP_ACTIVE_LOW, IPT_START1) PORT_CONDITION(
                        "IO_TYPE", 0x01, EQUALS,
                        0x01) PORT_BIT(0x20, IP_ACTIVE_LOW, IPT_UNUSED)

                        PORT_MODIFY(
                            "PORTE") PORT_BIT(0xff, IP_ACTIVE_LOW, IPT_UNUSED)

                            PORT_MODIFY("PORTF") PORT_BIT(
                                0xff, IP_ACTIVE_LOW,
                                IPT_UNUSED)
    ...
    PORT_START("P2_KEY0") PORT_BIT(0xff, IP_ACTIVE_LOW, IPT_UNUSED)
        ...
        PORT_START("P2_KEY1") PORT_BIT(0xff, IP_ACTIVE_LOW, IPT_UNUSED)

            PORT_START("P2_KEY2") PORT_BIT(0xff, IP_ACTIVE_LOW, IPT_UNUSED)

                PORT_START("P2_KEY3") PORT_BIT(0xff, IP_ACTIVE_LOW, IPT_UNUSED)

                    PORT_START("P2_KEY4") PORT_BIT(0xff, IP_ACTIVE_LOW,
                                                   IPT_UNUSED)
                            ...
                            PORT_CONFNAME(0x01, 0x01, "I/O Device type")
                                PORT_CONFSETTING(0x00, "Mahjong Panel")
                                    PORT_CONFSETTING(0x01,
                                                     "Joystick") INPUT_PORTS_END

    /* Micronet layout, routes joystick port to the mux! */
    static INPUT_PORTS_START(vmahjong) PORT_INCLUDE(stv)
        ...
        PORT_MODIFY("PORTA") PORT_BIT(0xff, IP_ACTIVE_LOW, IPT_UNUSED)
            ...
            PORT_MODIFY("PORTB") PORT_BIT(0xff, IP_ACTIVE_LOW, IPT_UNUSED)
                ...
                PORT_MODIFY("PORTC") PORT_BIT(
                    0x10, IP_ACTIVE_LOW,
                    IPT_UNUSED) PORT_BIT(0x20, IP_ACTIVE_LOW, IPT_UNUSED)

                    PORT_MODIFY(
                        "PORTE") PORT_BIT(0xff, IP_ACTIVE_LOW, IPT_UNUSED)

                        PORT_MODIFY(
                            "PORTF") PORT_BIT(0xff, IP_ACTIVE_LOW, IPT_UNUSED)

                            PORT_START("P1_KEY0") PORT_BIT(
                                0x01, IP_ACTIVE_LOW,
                                IPT_MAHJONG_KAN) PORT_BIT(0x02, IP_ACTIVE_LOW,
                                                          IPT_START1)
                                PORT_BIT(0x04, IP_ACTIVE_LOW, IPT_UNUSED) PORT_BIT(
                                    0x08, IP_ACTIVE_LOW,
                                    IPT_UNUSED) PORT_BIT(0x10, IP_ACTIVE_LOW, IPT_MAHJONG_E)
                                    PORT_BIT(0x20, IP_ACTIVE_LOW, IPT_MAHJONG_A) PORT_BIT(
                                        ...
                                        PORT_START("P1_KEY1") PORT_BIT(
                                            0x01, IP_ACTIVE_LOW,
                                            IPT_MAHJONG_REACH) PORT_BIT(0x02, IP_ACTIVE_LOW, IPT_MAHJONG_BET)
                                            PORT_BIT(0x04, IP_ACTIVE_LOW, IPT_UNUSED) PORT_BIT(
                                                0x08, IP_ACTIVE_LOW,
                                                IPT_UNUSED) PORT_BIT(0x10, IP_ACTIVE_LOW, IPT_MAHJONG_F)
                                                PORT_BIT(0x20, IP_ACTIVE_LOW, IPT_MAHJONG_B) PORT_BIT(
    ...
    static INPUT_PORTS_START(patocar) PORT_INCLUDE(stv)

        PORT_MODIFY("PORTA")
            PORT_BIT(0x01, IP_ACTIVE_HIGH, IPT_BUTTON2) // hopper ?
    PORT_BIT(0x02, IP_ACTIVE_HIGH, IPT_CUSTOM)
        PORT_READ_LINE_DEVICE_MEMBER("hopper",
                                     FUNC(ticket_dispenser_device::line_r))
    ...
    PORT_MODIFY("PORTG.0") PORT_BIT(0xffff, 0x0000, IPT_TRACKBALL_X)
        PORT_SENSITIVITY(500) PORT_KEYDELTA(100) PORT_PLAYER(1) PORT_MODIFY(
            "PORTG.1") PORT_BIT(0xffff, 0x0000, IPT_TRACKBALL_Y)
            PORT_SENSITIVITY(500) PORT_KEYDELTA(100) PORT_PLAYER(1)

                PORT_MODIFY("PORTC") PORT_BIT(0x01, IP_ACTIVE_LOW,
                                              IPT_COIN1) PORT_NAME("10Yen")
                    PORT_BIT(0x02, IP_ACTIVE_LOW, IPT_COIN2) PORT_NAME("100Yen")
                        PORT_BIT(0x10, IP_ACTIVE_LOW,
                                 IPT_START1) PORT_NAME("Select Button")
                            PORT_BIT(0x20, IP_ACTIVE_LOW,
                                     IPT_BUTTON1) PORT_NAME("Power Button")
                                PORT_BIT(0xc0, IP_ACTIVE_LOW, IPT_UNUSED)

                                    PORT_MODIFY("PORTE") PORT_BIT(0xff,
                                                                  IP_ACTIVE_LOW,
                                                                  IPT_UNUSED)
    ...
    static INPUT_PORTS_START(micrombc) PORT_INCLUDE(patocar)

        PORT_MODIFY("PORTA") PORT_BIT(0x20, IP_ACTIVE_LOW, IPT_BUTTON1)
            PORT_NAME("Power Button") PORT_BIT(0xdc, IP_ACTIVE_LOW, IPT_UNUSED)

                PORT_MODIFY("PORTB") PORT_BIT(0x0c, IP_ACTIVE_LOW, IPT_BUTTON5)
                    PORT_NAME("Magnet Sensor")

                        PORT_MODIFY("PORTC") PORT_BIT(0x10, IP_ACTIVE_LOW,
                                                      IPT_START1)
                            PORT_NAME("Shoot Button") PORT_BIT(0xe0,
                                                               IP_ACTIVE_LOW,
                                                               IPT_UNUSED)
                                ...
                                PORT_MODIFY("PORTG.0")
                                    PORT_BIT(0x0000, IP_ACTIVE_LOW, IPT_UNUSED)
                                        PORT_MODIFY("PORTG.1")
                                            PORT_BIT(0x0000, IP_ACTIVE_LOW,
                                                     IPT_UNUSED) INPUT_PORTS_END

    static INPUT_PORTS_START(fanzonem) PORT_INCLUDE(stv)
        ...
        PORT_BIT(0x02, IP_ACTIVE_LOW, IPT_JOYSTICK_LEFT)
            PORT_BIT(0x04, IP_ACTIVE_LOW, IPT_JOYSTICK_UP)
                PORT_BIT(0x08, IP_ACTIVE_LOW, IPT_JOYSTICK_DOWN)
                    PORT_BIT(0x30, IP_ACTIVE_HIGH,
                             IPT_UNKNOWN)       // Trigger Sensor 1
    ...
    PORT_BIT(0x01, IP_ACTIVE_LOW, IPT_SERVICE2) PORT_NAME("Test Switch (game)")
        PORT_BIT(0x02, IP_ACTIVE_LOW, IPT_SERVICE3)
            PORT_NAME("Reset Switch (game)")
                PORT_BIT(0x04, IP_ACTIVE_LOW, IPT_SERVICE)
                    PORT_NAME("Upper Door Switch")
                        PORT_BIT(0x08, IP_ACTIVE_LOW, IPT_SERVICE)
                            PORT_NAME("Machine Door Switch")
                                PORT_BIT(0x10, IP_ACTIVE_HIGH,
                                         IPT_UNKNOWN) // Hopper Count Switch
    PORT_BIT(0x60, IP_ACTIVE_HIGH, IPT_UNKNOWN)       // Trigger Sensor 2
    PORT_BIT(0x80, IP_ACTIVE_LOW, IPT_UNUSED) INPUT_PORTS_END

    static INPUT_PORTS_START(wasafari) PORT_INCLUDE(stv)

        PORT_MODIFY("PORTA") PORT_BIT(0xfc, IP_ACTIVE_LOW, IPT_UNUSED)

            PORT_MODIFY("PORTB") PORT_BIT(0xff, IP_ACTIVE_LOW, IPT_UNUSED)

                PORT_MODIFY("PORTC") PORT_BIT(0x30, IP_ACTIVE_LOW, IPT_UNUSED)
                    ...
                    PORT_MODIFY("PORTE")
                        PORT_BIT(0xff, IP_ACTIVE_LOW, IPT_UNUSED)
                            ...
                            PORT_MODIFY("PORTF") PORT_BIT(0xff, IP_ACTIVE_LOW,
                                ...
                                PORT_MODIFY("PORTG")
                                    PORT_BIT(0x01, IP_ACTIVE_LOW, IPT_BUTTON3)
                                        PORT_NAME("Winch") INPUT_PORTS_END

    static INPUT_PORTS_START(wwshin) PORT_INCLUDE(wasafari)

        PORT_MODIFY("PORTG") PORT_BIT(0x01, IP_ACTIVE_LOW, IPT_BUTTON3)

    // NOTE: needs to be initialized in Volume Adjust
    PORT_MODIFY("AN0") PORT_BIT(0xff, 0x00, IPT_PEDAL) PORT_MINMAX(0x00, 0xff)
        PORT_SENSITIVITY(50) PORT_KEYDELTA(60) PORT_NAME("Accelerate")

            PORT_MODIFY("AN1") PORT_BIT(0xff, 0x00, IPT_PEDAL2)
                PORT_MINMAX(0x00, 0xff) PORT_SENSITIVITY(50) PORT_KEYDELTA(60)
                    PORT_NAME("Brake") INPUT_PORTS_END

    static INPUT_PORTS_START(aclub) PORT_INCLUDE(stv)
    ...
    PORT_MODIFY("PORTB") PORT_BIT(0xff, IP_ACTIVE_LOW, IPT_UNUSED)
        ...
        PORT_MODIFY("PORTC") PORT_BIT(0x20, IP_ACTIVE_LOW, IPT_UNUSED)

            PORT_MODIFY("PORTE") PORT_BIT(0xff, IP_ACTIVE_LOW, IPT_UNUSED)

                PORT_MODIFY("PORTF") PORT_BIT(0xff, IP_ACTIVE_LOW, IPT_UNUSED)

                    PORT_MODIFY("PORTG") PORT_BIT(0xff, IP_ACTIVE_LOW,
                                                  IPT_UNUSED) INPUT_PORTS_END

#define ROM_LOAD16_WORD_SWAP_BIOS(bios, name, offset, length, hash)            \
  ROMX_LOAD(name, offset, length, hash,                                        \
            ROM_GROUPWORD | ROM_REVERSE | ROM_BIOS(bios))

/* BIOS rev info found at 0x800 (byte swapped):

epr-17740  - Japan   STVB1.10J0950131  95/01/31 v1.10  - Found on a early board
dated 02/1995 epr-17740a - Japan   STVB1.11J0950220  95/02/20 v1.11 epr-17741a -
USA     STVB1.11U0950220  95/02/20 v1.11 epr-17951a - Japan   STVB1.13J0950425
95/04/25 v1.13 - There's also a 100% identical mask ROM version: mpr-17951a
...
epr-18343  - ?????   STVB1.30S0950727  95/07/27 v1.30 - Special version used on
Sports Fishing 2 (CD based) epr-19730  - Japan   PCD11.13J0970217  97/02/17
v1.13 - Enhanced version for CD based units? epr-20091  - Japan PCS11.13J0970821
97/08/21 v1.13 - Enhanced version for ???

stv110.bin  - Debug  STVB1.10D0950113  95/01/13 v1.10
stv1061.bin - ST-V Dev BIOS (1.061) - Sega 1994, Noted "ST-V Ver 1.061 94/11/25"
on EPROM sticker, Coming from a S-TV SG5001A dev board
...
ROM_LOAD16_WORD_SWAP_BIOS( x, "saturn.bin", 0x000000, 0x080000, CRC(653ff2d8)
SHA1(20994ae7ee177ddaf3a430b010c7620dca000fb4) ) - Saturn EU BIOS
...
#define STV_BIOS                                                               \
  ...
  ROM_LOAD16_WORD_SWAP_BIOS(                                                   \
      0, "epr-23603.ic8", 0x000000, 0x080000,                                  \
      CRC(f688ae60) SHA1(1a31b6b1a4257fcb6ac6a91e67dd798f91505f48))            \
  ROM_SYSTEM_BIOS(1, "jp1", "EPR-20091 (Japan 97/08/21)")                      \
  ROM_LOAD16_WORD_SWAP_BIOS(                                                   \
      1, "epr-20091.ic8", 0x000000, 0x080000,                                  \
      CRC(59ed40f4) SHA1(eff0f54c70bce05ff3a289bf30b1027e1c8cd117))            \
  ROM_SYSTEM_BIOS(2, "jp2", "EPR-19730 (Japan 97/02/17)")                      \
  ROM_LOAD16_WORD_SWAP_BIOS(                                                   \
      2, "epr-19730.ic8", 0x000000, 0x080000,                                  \
      CRC(d0e0889d) SHA1(fae53107c894e0c41c49e191dbe706c9cd6e50bd))            \
  ROM_SYSTEM_BIOS(3, "jp3", "EPR-17951A (Japan 95/04/25)")                     \
  ROM_LOAD16_WORD_SWAP_BIOS(                                                   \
      3, "epr-17951a.ic8", 0x000000, 0x080000,                                 \
      CRC(2672f9d8) SHA1(63cf4a6432f6c87952f9cf3ab0f977aed2367303))            \
  ROM_SYSTEM_BIOS(4, "jp4", "EPR-17740A (Japan 95/02/20)")                     \
  ROM_LOAD16_WORD_SWAP_BIOS(                                                   \
      4, "epr-17740a.ic8", 0x000000, 0x080000,                                 \
      CRC(3e23c81f) SHA1(f9b282fd27693e9891843597b2e1823da3d23c7b))            \
  ROM_SYSTEM_BIOS(5, "jp5", "EPR-17740 (Japan 95/01/31)")                      \
  ROM_LOAD16_WORD_SWAP_BIOS(                                                   \
      5, "epr-17740.ic8", 0x000000, 0x080000,                                  \
      CRC(5c5aa63d) SHA1(06860d96923b81afbc21e0ad32ee19487d8ff6e7))            \
  ROM_SYSTEM_BIOS(6, "euro", "EPR-17954A (Europe 95/04/25)")                   \
  ROM_LOAD16_WORD_SWAP_BIOS(                                                   \
      6, "epr-17954a.ic8", 0x000000, 0x080000,                                 \
      CRC(f7722da3) SHA1(af79cff317e5b57d49e463af16a9f616ed1eee08))            \
  ROM_SYSTEM_BIOS(7, "us", "EPR-17952A (USA 95/04/25)")                        \
  ROM_LOAD16_WORD_SWAP_BIOS(                                                   \
      7, "epr-17952a.ic8", 0x000000, 0x080000,                                 \
      CRC(d1be2adf) SHA1(eaf1c3e5d602e1139d2090a78d7e19f04f916794))            \
  ROM_SYSTEM_BIOS(8, "us1", "EPR-17741a (USA 95/02/20)")                       \
  ROM_LOAD16_WORD_SWAP_BIOS(                                                   \
      8, "epr-17741a.ic8", 0x000000, 0x080000,                                 \
      CRC(4166c663) SHA1(cc41d30de06160083d77e0dc5d69e61fec7fdcb5))            \
  ROM_SYSTEM_BIOS(9, "tw", "EPR-19854 (Taiwan 97/05/15)")                      \
  ROM_LOAD16_WORD_SWAP_BIOS(                                                   \
      9, "epr-19854.ic8", 0x000000, 0x080000,                                  \
      CRC(e09d1f60) SHA1(b55cdcb45b2a5b0b35e352cf7625f0bd659084df))            \
  ROM_SYSTEM_BIOS(10, "tw1", "EPR-17953A (Taiwan 95/04/25)")                   \
  ROM_LOAD16_WORD_SWAP_BIOS(                                                   \
      10, "epr-17953a.ic8", 0x000000, 0x080000,                                \
      CRC(a4c47570) SHA1(9efc73717ec8a13417e65c54344ded9fc25bf5ef))            \
  ROM_SYSTEM_BIOS(11, "tw2", "EPR-17742A (Taiwan 95/02/20)")                   \
  ROM_LOAD16_WORD_SWAP_BIOS(                                                   \
      11, "epr-17742a.ic8", 0x000000, 0x080000,                                \
      CRC(02daf123) SHA1(23185beb1ce9c09b8719e57d1adb7b28c8141fd5))            \
  ROM_SYSTEM_BIOS(12, "debug", "Development (1.10, 95/01/13)")                 \
  ROM_LOAD16_WORD_SWAP_BIOS(                                                   \
      12, "stv110.bin", 0x000000, 0x080000,                                    \
      CRC(3dfeda92) SHA1(8eb33192a57df5f3a1dfb57263054867c6b2db6d))            \
...
ROM_LOAD16_WORD_SWAP(
    "mpr20827.2", 0x0400000, 0x0400000,
    CRC(65cabbb3)
        SHA1(5e7cb090101dc42207a4084465e419f4311b6baf)) // good (was .12)
ROM_LOAD16_WORD_SWAP(
    "mpr20828.3", 0x0800000, 0x0400000,
    CRC(3934d44a)
        SHA1(969406b8bfac43b30f4d732702ca8cffeeefffb9)) // good (was .13)
ROM_LOAD16_WORD_SWAP(
    "mpr20829.4", 0x0c00000, 0x0400000,
    CRC(814308c3)
        SHA1(45c3f551690224c95acd156ae8f8397667927a04)) // good (was .14)
ROM_LOAD16_WORD_SWAP(
    "mpr20830.5", 0x1000000, 0x0400000,
    CRC(ff97fd19)
        SHA1(f37bcdce5f3f522527a44d59f1b8184ef290f829)) // good (was .15)
ROM_LOAD16_WORD_SWAP(
    "mpr20831.6", 0x1400000, 0x0400000,
    CRC(4408e6fb)
        SHA1(d4228cad8a1128e9426dac9ac62e9513a7a0117b)) // good (was .16)
ROM_LOAD16_WORD_SWAP(
    "mpr20826.1", 0x1800000, 0x0400000,
    CRC(bdc4b941)
        SHA1(c5e8b1b186324c2ccab617915f7bdbfe6897ca9f)) // good (was .17)
ROM_LOAD16_WORD_SWAP(
    "mpr20832.8", 0x1c00000, 0x0400000,
    CRC(af1b0985)
        SHA1(d7a0e4e0a8b0556915f924bdde8c3d14e5b3423e)) // good (was .18s)
ROM_LOAD16_WORD_SWAP(
    "mpr20833.9", 0x2000000, 0x0400000,
    CRC(cb6af231)
        SHA1(4a2e5d7c2fd6179c19cdefa84a03f9a34fbb9e70)) // good (was .19s)

// 610-0374-90   1998     317-5040-COM   ST-V      (yes, the 317-5040-COM chip
...
ROM_LOAD16_WORD_SWAP("mpr17970.2", 0x0400000, 0x0400000,
                     CRC(bc4d6f91)
                         SHA1(dcc241dcabea59325decfba3fd5e113c07958422)) // good
ROM_LOAD16_WORD_SWAP("mpr17971.3", 0x0800000, 0x0400000,
                     CRC(c780a3b3)
                         SHA1(99587eea528a6413cacc3e4d3d1dbfff57b03dca)) // good
ROM_LOAD16_WORD_SWAP("mpr17972.4", 0x0c00000, 0x0400000,
                     CRC(8f29815a)
                         SHA1(e86acd8096f2aee5f5e3ddfd3abb4f5c2b11df66)) // good
ROM_LOAD16_WORD_SWAP("mpr17973.5", 0x1000000, 0x0400000,
                     CRC(5f6e0e8b)
...
ROM_LOAD16_WORD_SWAP("mpr19554.2", 0x400000, 0x400000,
                     CRC(5a3ebcac)
                         SHA1(46e3d1cf515a7ff8a8f97e5050b29dbbeb5060c0)) // good
...
ROM_LOAD16_WORD_SWAP("mpr19115.2", 0x0400000, 0x0400000,
                     CRC(6fe06a30)
                         SHA1(dedb90f800bae8fd9df1023eb5bec7fb6c9d0179)) // good
ROM_LOAD16_WORD_SWAP("mpr19116.3", 0x0800000, 0x0400000,
                     CRC(af9e627b)
                         SHA1(a53921c3185a93ec95299bf1c29e744e2fa3b8c0)) // good
ROM_LOAD16_WORD_SWAP("mpr19117.4", 0x0c00000, 0x0400000,
                     CRC(74520ff1)
                         SHA1(16c1acf878664b3bd866c9b94f3695ae892ac12f)) // good
...
ROM_LOAD16_WORD_SWAP("mpr18138.2", 0x0400000, 0x0400000,
                     CRC(f5567049)
                         SHA1(6eb35e4b5fbda39cf7e8c42b6a568bd53a364d6d)) // good
ROM_LOAD16_WORD_SWAP("mpr18139.3", 0x0800000, 0x0400000,
                     CRC(f36b4878)
                         SHA1(e3f63c0046bd37b7ab02fb3865b8ebcf4cf68e75)) // good
ROM_LOAD16_WORD_SWAP("mpr18140.4", 0x0c00000, 0x0400000,
                     CRC(228850a0)
                         SHA1(d83f7fa7df08407fa45a13661393679b88800805)) // good
ROM_LOAD16_WORD_SWAP("mpr18141.5", 0x1000000, 0x0400000,
                     CRC(b51eef36)
                         SHA1(2745cba48dc410d6d31327b956886ec284b9eac3)) // good
ROM_LOAD16_WORD_SWAP("mpr18142.6", 0x1400000, 0x0400000,
                     CRC(cf259541)
...
ROM_LOAD16_WORD_SWAP("mpr18533.2", 0x0400000, 0x0400000,
                     CRC(7181fe51)
                         SHA1(646f95e1a5b64d721e961352cee6fd5adfd031ec)) // good
ROM_LOAD16_WORD_SWAP("mpr18534.3", 0x0800000, 0x0400000,
                     CRC(c87ef125)
                         SHA1(c9ced130faf6dd9e626074b6519615654d8beb19)) // good
ROM_LOAD16_WORD_SWAP("mpr18535.4", 0x0c00000, 0x0400000,
                     CRC(929a64cf)
                         SHA1(206dfc2a46befbcea974df1e27515c5759d88d00)) // good
ROM_LOAD16_WORD_SWAP("mpr18536.5", 0x1000000, 0x0400000,
                     CRC(51b9f64e)
                         SHA1(bfbdfb73d24f26ce1cc5294c23a1712fb9631691)) // good
ROM_LOAD16_WORD_SWAP("mpr18537.6", 0x1400000, 0x0400000,
                     CRC(c364f6a7)
                         SHA1(4db21bcf6ea3e75f9eb34f067b56a417589271c0)) // good
ROM_LOAD16_WORD_SWAP("mpr18532.1", 0x1800000, 0x0400000,
                     CRC(39528643)
                         SHA1(e35f4c35c9eb13e1cdcc26cb2599bb846f2c1af7)) // good
ROM_LOAD16_WORD_SWAP("mpr18539.8", 0x1c00000, 0x0400000,
                     CRC(62b3908c)
                         SHA1(3f00e49beb0e5575cc4250a25c41f04dc91d6ed0)) // good
ROM_LOAD16_WORD_SWAP("mpr18540.9", 0x2000000, 0x0400000,
                     CRC(4c2b59a4)
                         SHA1(4d15503fcff0e9e0d1ed3bac724278102b506da0)) // good

ROM_REGION16_BE(0x80, "eeprom", 0) // preconfigured to 1 player
ROM_LOAD("fhboxers.nv", 0x0000, 0x0080,
         CRC(590fd6da) SHA1(1abe71c62c51a246d854f02cd4cb79fd0d427e88))
ROM_END
...
ROM_LOAD16_WORD_SWAP("mpr20426.2", 0x0400000, 0x0400000,
                     CRC(897d1747)
                         SHA1(f3fb2c4ef8bc2c1658907e822f2ee2b88582afdd)) // good
ROM_LOAD16_WORD_SWAP("mpr20427.3", 0x0800000, 0x0400000,
                     CRC(a488a694)
                         SHA1(80ec81f32e4b5712a607208b2a45cfdf6d5e1849)) // good
ROM_LOAD16_WORD_SWAP("mpr20428.4", 0x0c00000, 0x0400000,
                     CRC(4353b3b6)
                         SHA1(f5e56396b345ff65f57a23f391b77d401f1f58b5)) // good
ROM_LOAD16_WORD_SWAP("mpr20429.5", 0x1000000, 0x0400000,
                     CRC(4f566486)
                         SHA1(5b449288e33f02f2362ebbd515c87ea11cc02633)) // good
ROM_LOAD16_WORD_SWAP("mpr20430.6", 0x1400000, 0x0400000,
                     CRC(d1e11979)
                         SHA1(14405997eefac22c42f0c86dca9411ba1dee9bf9)) // good
ROM_LOAD16_WORD_SWAP("mpr20425.1", 0x1800000, 0x0400000,
                     CRC(67f104c4)
                         SHA1(8e965d2ce554ba8d37254f6bf3931dff4bce1a43)) // good
ROM_LOAD16_WORD_SWAP("mpr20432.8", 0x1c00000, 0x0400000,
                     CRC(79fcdecd)
                         SHA1(df8e7733a51e24196914fc66a024515ee1565599)) // good
ROM_LOAD16_WORD_SWAP("mpr20433.9", 0x2000000, 0x0400000,
                     CRC(82289f29)
                         SHA1(fb6a1015621b1afa3913da162ae71ded6b674649)) // good
ROM_LOAD16_WORD_SWAP("mpr20434.10", 0x2400000, 0x0400000,
                     CRC(85c94afc)
                         SHA1(dfc2f16614bc499747ea87567a21c86e7bddce45)) // good
ROM_LOAD16_WORD_SWAP("mpr20435.11", 0x2800000, 0x0400000,
                     CRC(263a2e48)
                         SHA1(27ef4bf577d240e36dcb6e6a09b9c5f24e59ce8c)) // good
ROM_LOAD16_WORD_SWAP("mpr20436.12", 0x2c00000, 0x0400000,
                     CRC(e3823f49)
                         SHA1(754d48635bd1d4fb01ff665bfe2a71593d92f688)) // good

ROM_REGION16_BE(0x80, "eeprom", 0) // preconfigured to 1 player
ROM_LOAD("findlove.nv", 0x0000, 0x0080,
         CRC(df2fa9f6) SHA1(f05450e1648eafd2ffb52e50769f07bdc75ffb95))
ROM_END
...
ROM_LOAD16_WORD_SWAP("mpr18257.2", 0x0400000, 0x0400000,
                     CRC(137fdf55)
                         SHA1(07a02fe531b3707e063498f5bc9749bd1b4cadb3)) // good
ROM_RELOAD(0x1400000, 0x0400000)
ROM_LOAD16_WORD_SWAP("mpr18258.3", 0x0800000, 0x0400000,
                     CRC(f519c505)
                         SHA1(5cad39314e46b98c24a71f1c2c10c682ef3bdcf3)) // good
ROM_RELOAD(0x1800000, 0x0400000)
ROM_LOAD16_WORD_SWAP("mpr18259.4", 0x0c00000, 0x0400000,
                     CRC(5cabc775)
                         SHA1(84383a4cbe3b1a9dcc6c140cff165425666dc780)) // good
ROM_RELOAD(0x1c00000, 0x0400000)
ROM_LOAD16_WORD_SWAP("mpr18260.5", 0x1000000, 0x0400000,
                     CRC(f5b92082)
...
ROM_LOAD16_WORD_SWAP("mpr17768.2", 0x0400000, 0x0400000,
                     CRC(d6808a7d)
                         SHA1(83a97bbe1160cb45b3bdcbde8adc0d9bae5ded60)) // good
ROM_LOAD16_WORD_SWAP("mpr17769.3", 0x0800000, 0x0400000,
                     CRC(3471dd35)
                         SHA1(24febddfe70984cebc0e6948ad718e0e6957fa82)) // good
ROM_LOAD16_WORD_SWAP("mpr17770.4", 0x0c00000, 0x0400000,
                     CRC(06978a00)
                         SHA1(a8d1333a9f4322e28b23724937f595805315b136)) // good
ROM_LOAD16_WORD_SWAP("mpr17771.5", 0x1000000, 0x0400000,
                     CRC(aea2ea3b)
                         SHA1(2fbe3e10d3f5a3b3099a7ed5b38b93b6e22e19b8)) // good
ROM_LOAD16_WORD_SWAP("mpr17772.6", 0x1400000, 0x0400000,
                     CRC(b3dc0e75)
                         SHA1(fbe2790c84466d186ea3e9d41edfcb7afaf54bea)) // good
ROM_LOAD16_WORD_SWAP("mpr17767.1", 0x1800000, 0x0400000,
                     CRC(9ba1e7b1)
...
ROM_LOAD16_WORD_SWAP("mpr18939.2", 0x0400000, 0x0400000,
                     CRC(ef95a6e6)
                         SHA1(3026c52ad542997d5b0e621b389c0e01240cb486)) // good
ROM_LOAD16_WORD_SWAP("mpr18940.3", 0x0800000, 0x0400000,
                     CRC(cabab4cd)
                         SHA1(b251609573c4b0ccc933188f32226855b25fd9da)) // good
ROM_LOAD16_WORD_SWAP("mpr18941.4", 0x0c00000, 0x0400000,
                     CRC(f4a33a20)
                         SHA1(bf0f33495fb5c9de4ae5036cedda65b3ece217e8)) // good
ROM_LOAD16_WORD_SWAP("mpr18942.5", 0x1000000, 0x0400000,
                     CRC(8dd0a446)
                         SHA1(a75e3552b0fb99e0b253c0906f62fabcf204b735)) // good
ROM_LOAD16_WORD_SWAP("mpr18943.6", 0x1400000, 0x0400000,
                     CRC(d8702a9e)
                         SHA1(960dd3cb0b9eb1f18b8d0bc0da532b600d583ceb)) // good
ROM_LOAD16_WORD_SWAP("mpr18938.1", 0x1800000, 0x0400000,
                     CRC(580ecb83)
...
ROM_LOAD16_WORD_SWAP("mpr18738.2", 0x0400000, 0x0400000,
                     CRC(4b3c175a)
                         SHA1(b6d2438ae1d3d51950a7ed1eaadf2dae45c4e7b1)) // good
ROM_LOAD16_WORD_SWAP("mpr18739.3", 0x0800000, 0x0400000,
                     CRC(eb41fa67)
                         SHA1(d12acebb1df9eafd17aff1841087f5017225e7e7)) // good
ROM_LOAD16_WORD_SWAP("mpr18740.4", 0x0c00000, 0x0200000,
                     CRC(9ca7962f)
                         SHA1(a09e0db2246b34ca7efa3165afbc5ba292a95398)) // good

ROM_REGION16_BE(0x80, "eeprom", 0) // preconfigured to 1 player
ROM_LOAD("kiwames.nv", 0x0000, 0x0080,
         CRC(c7002732) SHA1(57395d256a58ddcedd354ce1f2c458321be40505))
ROM_END
...
ROM_LOAD16_WORD_SWAP("mpr20417.2", 0x0400000, 0x0400000,
                     CRC(636c2a08)
                         SHA1(47986b71d68f6a1852e4e2b03ca7b6e48e83718b)) // good
ROM_LOAD16_WORD_SWAP("mpr20418.3", 0x0800000, 0x0400000,
                     CRC(3f0d9e34)
                         SHA1(2ec81e40ebf689d17b6421820bfb0a1280a8ef25)) // good
ROM_LOAD16_WORD_SWAP("mpr20419.4", 0x0c00000, 0x0400000,
                     CRC(ec969815)
                         SHA1(b59782174051f5717b06f43e57dd8a2a6910d95f)) // good
ROM_LOAD16_WORD_SWAP("mpr20420.5", 0x1000000, 0x0400000,
                     CRC(f2902c88)
                         SHA1(df81e137e8aa4bd37e1d14fce4d593cfd14608f0)) // good
ROM_LOAD16_WORD_SWAP("mpr20421.6", 0x1400000, 0x0400000,
                     CRC(cd0b477c)
                         SHA1(5169cc47fae465b11bc50f5e8410d84c2b2eee42)) // good
ROM_LOAD16_WORD_SWAP("mpr20422.1", 0x1800000, 0x0400000,
                     CRC(66335049)
                         SHA1(59f1968001d1e9fe30990a56309bae18033eee62)) // good
ROM_LOAD16_WORD_SWAP("mpr20423.8", 0x1c00000, 0x0400000,
                     CRC(2bd55832)
                         SHA1(1a1a510f30882d4d726b594a6541a12c552fafb4)) // good
ROM_LOAD16_WORD_SWAP("mpr20443.9", 0x2000000, 0x0400000,
                     CRC(8ac288f5)
...
ROM_LOAD16_WORD_SWAP("mpr18853.2", 0x0400000, 0x0400000,
                     CRC(b9268c97)
                         SHA1(8734e3f0e6b2849d173e3acc9d0308084a4e84fd)) // good
ROM_LOAD16_WORD_SWAP("mpr18854.3", 0x0800000, 0x0400000,
                     CRC(3113c8bc)
                         SHA1(4e4600646ddd1978988d27430ffdf0d1d405b804)) // good
ROM_LOAD16_WORD_SWAP("mpr18855.4", 0x0c00000, 0x0400000,
                     CRC(daf6ad0c)
                         SHA1(2a14a6a42e4eb68abb7a427e43062dfde2d13c5c)) // good
ROM_LOAD16_WORD_SWAP("mpr18856.5", 0x1000000, 0x0400000,
                     CRC(214cef24)
...
ROM_LOAD16_WORD_SWAP("mpr19533.2", 0x0400000, 0x0400000,
                     CRC(17ec54ba)
                         SHA1(d4cdc86926519291cc78980ec513e1cfc677e76e)) // good
ROM_LOAD16_WORD_SWAP("mpr19534.3", 0x0800000, 0x0400000,
                     CRC(820e4781)
                         SHA1(7ea5626ad4e1929a5ec28a99ec12bc364df8f70d)) // good
ROM_LOAD16_WORD_SWAP("mpr19535.4", 0x0c00000, 0x0400000,
                     CRC(94fadfa4)
                         SHA1(a7d0727cf601e00f1ea31e6bf3e591349c3f6030)) // good
ROM_LOAD16_WORD_SWAP("mpr19536.5", 0x1000000, 0x0400000,
                     CRC(5765bc9c)
                         SHA1(b217c292e7cc8ed73a39a3ae7009bc9dd031e376)) // good
ROM_LOAD16_WORD_SWAP("mpr19537.6", 0x1400000, 0x0400000,
                     CRC(8b736686)
                         SHA1(aec347c0f3e5dd8646e85f68d71ca9acc3bf62c3)) // good
ROM_LOAD16_WORD_SWAP("mpr19532.1", 0x1800000, 0x0400000,
                     CRC(985f0c9d)
                         SHA1(de1ad42ef3cf3f4f071e9801696407be7ae29d21)) // good
ROM_LOAD16_WORD_SWAP("mpr19538.8", 0x1c00000, 0x0400000,
                     CRC(915a723e)
                         SHA1(96480441a69d6aad3887ed6f46b0a6bebfb752aa)) // good
ROM_LOAD16_WORD_SWAP("mpr19539.9", 0x2000000, 0x0400000,
                     CRC(72a297e5)
...
ROM_LOAD16_WORD_SWAP("mpr18635.8", 0x1c00000, 0x0400000,
                     CRC(441e1368)
                         SHA1(acb2a7e8d44c2203b8d3c7a7b70e20ffb120bebf)) // good
ROM_RELOAD(0x0400000, 0x0400000)
ROM_LOAD16_WORD_SWAP("mpr18636.9", 0x2000000, 0x0400000,
                     CRC(fff1dd80)
                         SHA1(36b8e1526a4370ae33fd4671850faf51c448bca4)) // good
ROM_RELOAD(0x0800000, 0x0400000)
ROM_LOAD16_WORD_SWAP("mpr18637.10", 0x2400000, 0x0400000,
                     CRC(83aced0f)
                         SHA1(6cd1702b9c2655dc4f56c666607c333f62b09fc0)) // good
ROM_RELOAD(0x0c00000, 0x0400000)
ROM_LOAD16_WORD_SWAP("mpr18638.11", 0x2800000, 0x0400000,
                     CRC(caab531b)
...
This is a non-Sega-manufactured STV cart which works with Japanese and USA
bioses. The cart is mostly the same as the Sega carts, containing not a lot
except some ROMs and logic chips.

On the top side, there are two 27C040 EPROMs and positions for 5 maskROMs, but
only 4 of them are populated. In between the two eproms is an unpopulated
position for a TSOP40 flashROM. On the bottom are locations for 5 maskROMs (none
are populated) and also some logic ICs.
*/
...
ROM_LOAD16_WORD_SWAP("th-e-3.ic3", 0x0800000, 0x0400000,
                     CRC(c9290b44)
                         SHA1(4e51e667edf330cc20e600cf1500f332cc533e20))
ROM_LOAD16_WORD_SWAP("th-e-4.ic4", 0x0c00000, 0x0400000,
                     CRC(c672e40b)
                         SHA1(4ba189de7b8fb5ca45fb15c89cdaf0f860831d52))
ROM_LOAD16_WORD_SWAP("th-e-5.ic5", 0x1000000, 0x0400000,
                     CRC(3914b805)
...
ROM_RELOAD(0x0400000, 0x0400000)
ROM_LOAD("bom210-11.ic3", 0x2000000, 0x0400000,
         CRC(44e5a13e) SHA1(aee3c06662a1d083f3bd01292cf2694132f63533))
ROM_RELOAD(0x0800000, 0x0400000)
ROM_LOAD("bom210-12.ic4", 0x2400000, 0x0400000,
         CRC(deabc701) SHA1(cb313ae9bf6f115682fc76647a999e12c98f6120))
ROM_RELOAD(0x0c00000, 0x0400000)
ROM_LOAD("bom210-13.ic5", 0x2800000, 0x0400000,
...
ROM_LOAD("ic14", 0x0600000, 0x0200000,
         CRC(bcd72105)
             SHA1(fb88a1f2589fef5d9845027646259ec8e781771b)) // ic2 good
ROM_LOAD("ic15", 0x0800000, 0x0200000,
         CRC(8c9c8352)
             SHA1(312e9e6435c6e9a4a19b72e234b087d4f5b069a5)) // ic3 good
ROM_LOAD("ic16", 0x0a00000, 0x0200000,
         CRC(07e11512)
             SHA1(5fb5e93a167aae501bd799aba98f7dbc3762a795)) // ic3 good
ROM_LOAD("ic17", 0x0c00000, 0x0200000,
         CRC(46b7b344)
             SHA1(a325b724e6346f585582bb2581d3063b1c8ddee9)) // ic4 good
ROM_LOAD("ic18", 0x0e00000, 0x0200000,
         CRC(d48404e1)
             SHA1(e4927d17cb19f6825f41b41056b89de224cf97de)) // ic4 good
ROM_LOAD("ic19", 0x1000000, 0x0200000,
         CRC(33d23bb9)
             SHA1(0179438a0b14ad7eb553a24d86fae778671fcc49)) // ic5 good
ROM_LOAD("ic20", 0x1200000, 0x0200000,
         CRC(f8cc1038)
             SHA1(06bcb25eb5df77bd503b0692a96ec66c00976fe9)) // ic5 good
ROM_LOAD("ic21", 0x1400000, 0x0200000,
         CRC(74ceb649)
             SHA1(91b7eb699b7167f890528149178644a74ef3ad17)) // ic6 good
ROM_LOAD("ic22", 0x1600000, 0x0200000,
         CRC(85f31277)
             SHA1(277c28d44cabd9081a8151d6ec27d47b5606435a)) // ic6 good
ROM_LOAD("ic12", 0x1800000, 0x0400000,
         CRC(d5ebc84e)
...
ROM_LOAD16_WORD_SWAP("mpr20544.2", 0x0400000, 0x0400000,
                     CRC(661fff5e)
                         SHA1(41f4ddda7adf004b52cc9a076606a60f31947d19)) // good
ROM_LOAD16_WORD_SWAP("mpr20545.3", 0x0800000, 0x0400000,
                     CRC(8e3a37be)
                         SHA1(a3227cdc4f03bb088e7f9aed225b238da3283e01)) // good
ROM_LOAD16_WORD_SWAP("mpr20546.4", 0x0c00000, 0x0400000,
                     CRC(72020886)
                         SHA1(e80bdeb11b726eb23f2283950d65d55e31a5672e)) // good
ROM_LOAD16_WORD_SWAP("mpr20547.5", 0x1000000, 0x0400000,
                     CRC(8362e397)
                         SHA1(71f13689a60572a04b91417a9a48adfd3bd0f5dc)) // good
ROM_LOAD16_WORD_SWAP("mpr20548.6", 0x1400000, 0x0400000,
                     CRC(e37534d9)
                         SHA1(79988cbb1537ca99fdd0288a86564fe1f714d052)) // good
ROM_LOAD16_WORD_SWAP("mpr20543.1", 0x1800000, 0x0400000,
                     CRC(1f688cdf)
...
ROM_LOAD16_WORD_SWAP("mpr20551.2", 0x0400000, 0x0400000,
                     CRC(9a0c6dd8)
                         SHA1(26600372cc673ce3678945f4b5dc4e3ab31643a4)) // good
ROM_LOAD16_WORD_SWAP("mpr20552.3", 0x0800000, 0x0400000,
                     CRC(5f46b0aa)
                         SHA1(1aa576b15971c0ffb4e08d4802246841b31b6f35)) // good
ROM_LOAD16_WORD_SWAP("mpr20553.4", 0x0c00000, 0x0400000,
                     CRC(c0f8a6b6)
                         SHA1(2038b9231a950450267be0db24b31d8035db79ad)) // good
ROM_LOAD16_WORD_SWAP("mpr20554.5", 0x1000000, 0x0400000,
                     CRC(215fc1f9)
                         SHA1(f042145622ba4bbbcce5f050a4c9eae42cb7adcd)) // good
ROM_LOAD16_WORD_SWAP("mpr20555.6", 0x1400000, 0x0400000,
                     CRC(3f5186a9)
                         SHA1(d613f307ab150a7eae358aa449206af05db5f9d7)) // good
ROM_LOAD16_WORD_SWAP("mpr20550.1", 0x1800000, 0x0400000,
                     CRC(083e1ca8)
                         SHA1(03944dd8fe86f305ca4bd2d71e2140e03798ffc9)) // good
ROM_LOAD16_WORD_SWAP("mpr20556.8", 0x1c00000, 0x0400000,
                     CRC(1fd70c6c)
                         SHA1(d9d2e362d13238216f4f7e10095fb8383bbd91e8)) // good
ROM_LOAD16_WORD_SWAP("mpr20557.9", 0x2000000, 0x0400000,
                     CRC(3c9ba442)
                         SHA1(2e5b795cf4cdc11ab3e4887b2f77c7147c6e3eec)) // good

ROM_REGION16_BE(0x80, "eeprom", 0) // preconfigured to 1 player
ROM_LOAD("seabass.nv", 0x0000, 0x0080,
         CRC(4e7c0944) SHA1(dbba78f6a7f3e7d12d5c4fff36117db82f5f9c01))
ROM_END
...
ROM_LOAD16_WORD_SWAP("mpr18778.8", 0x1c00000, 0x0400000,
                     CRC(25e1300e)
                         SHA1(64f3843f62cee34a47244ad5ee78fb2aa35289e3)) // good
ROM_LOAD16_WORD_SWAP("mpr18779.9", 0x2000000, 0x0400000,
                     CRC(51e2fabd)
                         SHA1(3aa361149af516f16d7d422596ee82014a183c2b)) // good
ROM_LOAD16_WORD_SWAP("mpr18780.10", 0x2400000, 0x0400000,
                     CRC(8cd4dd74)
                         SHA1(9ffec1280b3965d52f643894bdfecdd792028191)) // good
ROM_LOAD16_WORD_SWAP("mpr18781.11", 0x2800000, 0x0400000,
                     CRC(13ee41ae)
                         SHA1(cdbaeac4c90b5ee84233c299612f7f28280a6ba6)) // good
ROM_LOAD16_WORD_SWAP("mpr18782.12", 0x2c00000, 0x0200000,
                     CRC(9be2270a)
...
ROM_LOAD16_WORD_SWAP("mpr19189.2", 0x0400000, 0x0400000,
                     CRC(0b202a3e)
                         SHA1(6691b5af2cacd6092ec03886b78c2565953fa297)) // good
ROM_LOAD16_WORD_SWAP("mpr19190.3", 0x0800000, 0x0400000,
                     CRC(1777ded8)
                         SHA1(dd332ac79f0a6d82b6bde35b795b2845003dd1a5)) // good
ROM_LOAD16_WORD_SWAP("mpr19191.4", 0x0c00000, 0x0400000,
                     CRC(ec6eb07b)
                         SHA1(01fe4832ece8638ea6f4060099d9105fe8092c88)) // good
ROM_LOAD16_WORD_SWAP("mpr19192.5", 0x1000000, 0x0200000,
                     CRC(cb544a1e)
...
ROM_RELOAD_PLAIN(0x0300000, 0x0080000)
ROM_LOAD16_WORD_SWAP(
    "mpr21489.2", 0x0400000, 0x0400000,
    CRC(4c85152b)
        SHA1(78f2f1c31718d5bf631d8813daf9a11ea2a0e451)) // ic2 good (was .12)
ROM_LOAD16_WORD_SWAP(
    "mpr21490.3", 0x0800000, 0x0400000,
    CRC(03da67f8)
        SHA1(02f9ba7549ca552291dc0ff1b631103015838bba)) // ic3 good (was .13)
ROM_LOAD16_WORD_SWAP(
    "mpr21491.4", 0x0c00000, 0x0400000,
    CRC(cf7ee784)
        SHA1(af823df2d60d8ef3d17628b95a04136b807ca095)) // ic4 good (was .14)
ROM_LOAD16_WORD_SWAP(
    "mpr21492.5", 0x1000000, 0x0400000,
    CRC(57753894)
        SHA1(5c51167c158443d02a53d724a5ceb73055876c06)) // ic5 good (was .15)
ROM_LOAD16_WORD_SWAP(
    "mpr21493.6", 0x1400000, 0x0400000,
    CRC(efb2d271)
        SHA1(a591e48206704fbda5fef3ce69ad279da1017ed6)) // ic6 good (was .16)

ROM_REGION16_BE(0x80, "eeprom", 0) // preconfigured to 1 player
ROM_LOAD("sss.nv", 0x0000, 0x0080,
         CRC(3473b2f3) SHA1(6480b4b321af8ee6e967710e74f2556c17bfca97))
...
ROM_LOAD16_WORD_SWAP("mpr17836.2", 0x0400000, 0x0400000,
                     CRC(55e9642d)
                         SHA1(5198291cd1dce0398eb47760db2c19eae99273b0)) // good
ROM_LOAD16_WORD_SWAP("mpr17837.3", 0x0800000, 0x0400000,
                     CRC(13d1e667)
                         SHA1(cd513ceb33cc20032090113b61227638cf3b3998)) // good
ROM_LOAD16_WORD_SWAP("mpr17838.4", 0x0c00000, 0x0400000,
                     CRC(f9e70032)
                         SHA1(8efdbcce01bdf77acfdb293545c59bf224a9c7d2)) // good
ROM_LOAD16_WORD_SWAP("mpr17839.5", 0x1000000, 0x0400000,
                     CRC(1b2762c5)
                         SHA1(5c7d5fc8a4705249a5b0ea64d51dc3dc95d723f5)) // good
ROM_LOAD16_WORD_SWAP("mpr17840.6", 0x1400000, 0x0400000,
                     CRC(0fd4c857)
                         SHA1(42caf22716e834d59e60d45c24f51d95734e63ae)) // good
ROM_LOAD16_WORD_SWAP("mpr17835.1", 0x1800000, 0x0400000,
                     CRC(77f5cb43)
                         SHA1(a4f54bc08d73a56caee5b26bea06360568655bd7)) // good
ROM_LOAD16_WORD_SWAP("mpr17841.8", 0x1c00000, 0x0400000,
                     CRC(f48beffc)
                         SHA1(92f1730a206f4a0abf7fb0ee1210e083a464ad70)) // good
ROM_LOAD16_WORD_SWAP("mpr17842.9", 0x2000000, 0x0400000,
                     CRC(ac8deed7)
...
ROM_LOAD16_WORD_SWAP(
    ...
    CRC(2d930d23)
        SHA1(5fcaf4257f3639cb3aa407d2936f616499a09d97)) // tested as IC2
ROM_LOAD16_WORD_SWAP(
    "mpr20822.13", 0x0800000, 0x0400000,
    CRC(8b33a5e2)
        SHA1(d5689ac8aad63509febe9aa4077351be09b2d8d4)) // tested as IC3
ROM_LOAD16_WORD_SWAP(
    "mpr20823.14", 0x0c00000, 0x0400000,
    CRC(6e6d4e95)
        SHA1(c387d03ba27580c62ac0bf780915fdf41552df6f)) // tested as IC4
ROM_LOAD16_WORD_SWAP(
    "mpr20824.15", 0x1000000, 0x0400000,
    CRC(4cf18a25)
        SHA1(310961a5f114fea8938a3f514dffd5231e910a5a)) // tested as IC5
...
ROM_LOAD16_WORD_SWAP("mpr18916.4", 0x0c00000, 0x0400000,
                     CRC(4aae3ddb)
                         SHA1(b75479e73f1bce3f0c27fbd90820fa51eb1914a6)) // good
ROM_LOAD16_WORD_SWAP("mpr18917.5", 0x1000000, 0x0400000,
                     CRC(edf6edc3)
                         SHA1(478e958f4f10a8126a00c83feca4a55ad6c25503)) // good
ROM_LOAD16_WORD_SWAP("mpr18918.6", 0x1400000, 0x0400000,
                     CRC(d3a95036)
                         SHA1(e300bbbb71fb06027dc539c9bbb12946770ffc95)) // good
ROM_LOAD16_WORD_SWAP("mpr18915.1", 0x1800000, 0x0400000,
                     CRC(09cc38e5)
                         SHA1(4dfe0e2f21f746020ec557e62487aa7558cbc1fd)) // good
ROM_LOAD16_WORD_SWAP("mpr18919.8", 0x1c00000, 0x0400000,
                     CRC(4ac700de)
                         SHA1(b1a8501f1683de380dfa49c9cabbe28bd70a5b26)) // good
ROM_LOAD16_WORD_SWAP("mpr18920.9", 0x2000000, 0x0400000,
                     CRC(0106e36c)
                         SHA1(f7c30dc9fedb9da079dd7d52fdecbeb8721c5dee)) // good
ROM_LOAD16_WORD_SWAP("mpr18921.10", 0x2400000, 0x0400000,
                     CRC(c23d51ad)
                         SHA1(0169b7e2df84e8caa2b349843bd0673f6de2195f)) // good
ROM_LOAD16_WORD_SWAP("mpr18922.11", 0x2800000, 0x0400000,
                     CRC(99d0ab90)
                         SHA1(e9c82a826cc76ffbe2423913645cf5d5ba2506d6)) // good
ROM_LOAD16_WORD_SWAP("mpr18923.12", 0x2c00000, 0x0400000,
                     CRC(30a41ae9)
...
ROM_LOAD16_WORD_SWAP("mpr17946.2", 0x0400000, 0x0400000,
                     CRC(4cb245f7)
                         SHA1(363d9936b27043b5858c956a45736ac05aefc54e)) // good
ROM_LOAD16_WORD_SWAP("mpr17947.3", 0x0800000, 0x0400000,
                     CRC(fef4a9fb)
                         SHA1(1b4bd095962db769da17d3644df10f62d041e914)) // good
ROM_LOAD16_WORD_SWAP("mpr17948.4", 0x0c00000, 0x0400000,
                     CRC(3e2b251a)
                         SHA1(be6191c18727d7cbc6399fd4c1aaae59304af30c)) // good
ROM_LOAD16_WORD_SWAP("mpr17949.5", 0x1000000, 0x0400000,
                     CRC(b2ecea25)
                         SHA1(320c0e7ce34e81e2fe6400cbeb2cb3ca74426cc8)) // good
ROM_LOAD16_WORD_SWAP("mpr17950.6", 0x1400000, 0x0400000,
                     CRC(5b1f981d)
                         SHA1(693b5744d210a2ac8b77e7c8c87f07ca859f8aed)) // good
ROM_LOAD16_WORD_SWAP("mpr17945.1", 0x1800000, 0x0200000,
                     CRC(03ede188)
...
ROM_LOAD16_WORD_SWAP("mpr20110.2", 0x0400000, 0x0400000,
                     CRC(238ef832)
                         SHA1(20fade5730ff8e249a1450c41bfdff6e133f4768)) // good
ROM_LOAD16_WORD_SWAP("mpr20111.3", 0x0800000, 0x0400000,
                     CRC(b0a86f69)
                         SHA1(e66427f70413ad43fccc38423962c5eeda01094f)) // good
ROM_LOAD16_WORD_SWAP("mpr20112.4", 0x0c00000, 0x0400000,
                     CRC(3ba2b49b)
                         SHA1(5ad154a8b774075479d791e29cbaf221d47557fc)) // good
ROM_LOAD16_WORD_SWAP("mpr20113.5", 0x1000000, 0x0400000,
                     CRC(8c858b41)
                         SHA1(d05d2980363c8440863fe2fdb39274de246bd4b9)) // good
ROM_LOAD16_WORD_SWAP("mpr20114.6", 0x1400000, 0x0400000,
                     CRC(b723862c)
                         SHA1(1e0a08669f16fc4cb647124e0c215233ccb98e5a)) // good
ROM_LOAD16_WORD_SWAP("mpr20109.1", 0x1800000, 0x0400000,
                     CRC(c1a713b8)
                         SHA1(a7fefa6e9a1e3aecff5ead41da6fd3aec2ef502a)) // good
ROM_LOAD16_WORD_SWAP("mpr20115.8", 0x1c00000, 0x0400000,
                     CRC(dd01f2ad)
...
ROM_LOAD16_WORD_SWAP("mpr20400.2", 0x0400000, 0x0400000,
                     CRC(1edfbe05)
                         SHA1(b0edd3f3d57408101ae6eb0aec742afbb4d289ca)) // good
ROM_LOAD16_WORD_SWAP("mpr20401.3", 0x0800000, 0x0400000,
                     CRC(99e98937)
                         SHA1(e1b4d12a0b4d0fe97a62fcc085e19cce77657c99)) // good
ROM_LOAD16_WORD_SWAP("mpr20402.4", 0x0c00000, 0x0400000,
                     CRC(4572aa60)
                         SHA1(8b2d76ea8c6e2f472c6ee7c9b6ad6e80e6a1a85a)) // good
ROM_LOAD16_WORD_SWAP("mpr20403.5", 0x1000000, 0x0400000,
                     CRC(26a8e13e)
                         SHA1(07f5564b704598e3c3580d3d620ecc4f14549dbd)) // good
ROM_LOAD16_WORD_SWAP("mpr20404.6", 0x1400000, 0x0400000,
                     CRC(0b70275d)
                         SHA1(47b8672e19c698dc948760f7091f4c6280e728d0)) // good
ROM_LOAD16_WORD_SWAP("mpr20399.1", 0x1800000, 0x0400000,
                     CRC(c178a96e)
                         SHA1(65f4aa05187d48ba8ad4fe75ff6ffe1f8524831d)) // good
ROM_LOAD16_WORD_SWAP("mpr20405.8", 0x1c00000, 0x0400000,
                     CRC(f53337b7)
                         SHA1(09a21f81016ee54f10554ae1f790415d7436afe0)) // good
ROM_LOAD16_WORD_SWAP("mpr20406.9", 0x2000000, 0x0400000,
                     CRC(b677c175)
                         SHA1(d0de7b5a29928036df0bdfced5a8021c0999eb26)) // good
ROM_LOAD16_WORD_SWAP("mpr20407.10", 0x2400000, 0x0400000,
                     CRC(58356050)
...
ROM_LOAD16_WORD_SWAP("mcj-00.2", 0x0400000, 0x0200000,
                     CRC(4eeacd6f)
                         SHA1(104ca230f22cd11cc536b34abd482e54791b4d0f)) // good
ROM_LOAD16_WORD_SWAP("mcj-01.3", 0x0800000, 0x0200000,
                     CRC(365a494b)
                         SHA1(29713dfc83a9ade63ebcc7994d14cd785c4500b9)) // good
ROM_LOAD16_WORD_SWAP("mcj-02.4", 0x0c00000, 0x0200000,
                     CRC(8b8e4931)
                         SHA1(0c94e2ccb72902d7786d1101a3958504f7151077)) // good
ROM_LOAD16_WORD_SWAP("mcj-03.5", 0x1000000, 0x0200000,
                     CRC(9015a0e7)
                         SHA1(8ba8a3723267e631169dc1e06620260fbccce4bd)) // good
ROM_LOAD16_WORD_SWAP("mcj-04.6", 0x1400000, 0x0200000,
                     CRC(9d1beaee)
                         SHA1(c63b61378860319fff2e605c7b9afaa5f1bc4cd2)) // good
ROM_LOAD16_WORD_SWAP("mcj-05.1", 0x1800000, 0x0200000,
                     CRC(a7626a82)
                         SHA1(c12a099132c5b9234a2de5674f3b8ba5fdd35289)) // good
ROM_LOAD16_WORD_SWAP("mcj-06.8", 0x1c00000, 0x0200000,
                     CRC(1ab8e90e)
...
ROM_LOAD16_BYTE("350-mpa1.u16", 0x0000001, 0x0100000,
                CRC(735e23ab) SHA1(133e2284a07a611aed8ada2707248f392f4509aa))
ROM_RELOAD(0x0200001, 0x0100000)
ROM_LOAD16_WORD_SWAP("gfx0.u1", 0x0400000, 0x0400000,
                     CRC(a82d0b7e)
                         SHA1(37a7a177634d51620b1b43e58732987df166c7e6))
ROM_LOAD16_WORD_SWAP("gfx1.u3", 0x0800000, 0x0400000,
                     CRC(a41e55d9)
                         SHA1(b896d3a6c36d325c3cece699da54f340a4512703))
ROM_LOAD16_WORD_SWAP("gfx2.u5", 0x0c00000, 0x0400000,
                     CRC(4c1ebeb7)
                         SHA1(cdd139652d9484ae5837a39c2fd48d0a8d966d43))
ROM_LOAD16_WORD_SWAP("gfx3.u8", 0x1000000, 0x0400000,
                     CRC(f679a3e7)
                         SHA1(db11b033b8bbdd80b81e3bc098bd40ad3a8784f2))
ROM_LOAD16_WORD_SWAP("gfx4.u12", 0x1400000, 0x0400000,
                     CRC(52d95242)
                         SHA1(b554a95933c2be4c72fb4226d3bc4775695da2c1))
ROM_LOAD16_WORD_SWAP("gfx5.u15", 0x1800000, 0x0400000,
                     CRC(e201f830)
                         SHA1(5aa22fcc8f2e153d1abc3aa4050c594b3942ee67))
ROM_LOAD16_WORD_SWAP("gfx6.u18", 0x1c00000, 0x0400000,
                     CRC(c6b381a3)
                         SHA1(46431f1e47c084a0bf85535d35af27471653b008))

ROM_REGION(0x1000000, "rax", 0)
ROM_LOAD("350snda1.u52", 0x000000, 0x080000,
         CRC(9027e7a0) SHA1(678df530838b078964a044ce734776f391654e6c))
ROM_LOAD("snd0.u48", 0x400000, 0x200000,
         CRC(02b1927c) SHA1(08b21d8b31b0f15c59fb5bb7eaf425e6fe04f7b5))
ROM_LOAD("snd1.u49", 0x600000, 0x200000,
         CRC(58b18eda) SHA1(7f3105fe04d9c0cdfd76e3323f623a4d0f7dad06))
ROM_LOAD("snd2.u50", 0x800000, 0x200000,
         CRC(51d626d6) SHA1(0e68b79dcb653dcba48121ca2d4f692f90afa85e))
...
screen with a rubber/plastic hammer. The cab has a large LED display to show how
many 'critters' were hit. The hammer positioning might work like a lightgun and
the 'hitting' may be like pulling the lightgun trigger?

The ROM cart appears to be a re-used Virtua Fighter Remix cart.
On top there is one 27C040 EPROM, EPR-18821 @ IC13
6 maskROMs, MPR-17945 to MPR-17950 (already dumped, known Virtua Fighter Remix
ROMs) On the other side of the PCB are 2 more maskROMs, MPR-18788 @ IC9 and
MPR-18789 @ IC8
...
//  SHA1(849c7fab5b97e043fea3deb8df6cc195ccced0e0) ) // good
ROM_LOAD16_WORD_SWAP("mpr-18789.ic8", 0x1c00000, 0x0400000,
                     CRC(b388616f)
                         SHA1(0b2c5a547c3a6a8fb9f4ca54336cf6dc9adb8c6a)) // good
ROM_LOAD16_WORD_SWAP("mpr-18788.ic9", 0x2000000, 0x0400000,
                     CRC(feae5867)
                         SHA1(7d2e47d5ab18700a246d53fdb7872a905cdac55a)) // good

ROM_REGION16_BE(0x80, "eeprom", 0) // preconfigured to 1 player
ROM_LOAD("critcrsh.nv", 0x0000, 0x0080,
         CRC(3da9860e) SHA1(05b315aa71fcfc4e617266e1c5e4954eccbf7854))
ROM_END
...
Japanese version of Critter Crusher. Like Critter Crusher this is also a
conversion, but this time from a Ejihon Tantei Jimusho cart.
...
//  SHA1(6eb35e4b5fbda39cf7e8c42b6a568bd53a364d6d) ) // good
//  ROM_LOAD16_WORD_SWAP( "mpr18139.3",    0x0800000, 0x0400000, CRC(f36b4878)
//  SHA1(e3f63c0046bd37b7ab02fb3865b8ebcf4cf68e75) ) // good
//  ROM_LOAD16_WORD_SWAP( "mpr18140.4",    0x0c00000, 0x0400000, CRC(228850a0)
//  SHA1(d83f7fa7df08407fa45a13661393679b88800805) ) // good
//  ROM_LOAD16_WORD_SWAP( "mpr18141.5",    0x1000000, 0x0400000, CRC(b51eef36)
//  SHA1(2745cba48dc410d6d31327b956886ec284b9eac3) ) // good
//  ROM_LOAD16_WORD_SWAP( "mpr18142.6",    0x1400000, 0x0400000, CRC(cf259541)
//  SHA1(51e2c8d16506d6074f6511112ec4b6b44bed4886) ) // good
ROM_LOAD16_WORD_SWAP("mpr-18789.ic8", 0x1c00000, 0x0400000,
                     CRC(b388616f)
                         SHA1(0b2c5a547c3a6a8fb9f4ca54336cf6dc9adb8c6a)) // good
...
There's the regular STV main board with dumped BIOS EPR-18343 and on top is a
plug in board (837-11781) containing 2 smaller plug in modules. One is the CD
ROM controller (838-10834) with.... -Hitachi HD6437097 (SH1 variant) (QFP144)
-20MHz Xtal
-Hitachi YGR019B (QFP168)
-HM514260 4M DRAM

The other is an MPEG decoder board (837-10835 171-6814D PC BD MPEG)
containing.... -HM514260 4M DRAM -Hitachi HD814102F (QFP100) -Sega 315-5745
HD814101FE (QFP144) -A curious PLCC44 marked SEGA MPR-17610A-H. The MPR-xxxxx
suggests it's a PLCC mask ROM, but type unknown????)

The CD-ROM unit is an Hitachi CDD-150SG with a custom flat connector (like the
Saturn unit, but you cannot use a Saturn unit on the STV, as it lacks tray
control).
*/
...
ROM_REGION32_BE(0x080000, "bios", 0) /* SH2 code */
ROM_LOAD16_WORD_SWAP(
    "epr18343.bin", 0x000000, 0x080000,
    CRC(48e2eecf) SHA1(
...
ROM_LOAD16_WORD_SWAP("mpr-18273.ic2", 0x0400000, 0x0400000,
                     CRC(6fec0193)
                         SHA1(5bbda289a5ca58c5bf57307360b07f0bb98f7356)) // good
ROM_LOAD16_WORD_SWAP("mpr-18274.ic3", 0x0800000, 0x0400000,
                     CRC(a6d76d23)
                         SHA1(eee8c824eff4485d1b3af93a4fd5b21262eec803)) // good
ROM_LOAD16_WORD_SWAP("mpr-18275.ic4", 0x0c00000, 0x0200000,
                     CRC(7691deca)
                         SHA1(aabb6b098963caf51f66aefa0a97aed7eb86c308)) // good
...
ROM_LOAD16_WORD_SWAP("mpr-19354.ic2", 0x0400000, 0x0400000,
                     CRC(a23822e7)
                         SHA1(10ca5d39dcaaf35b80168a08d8a18d77fba1d2ce)) // good
ROM_LOAD16_WORD_SWAP("mpr-19355.ic3", 0x0800000, 0x0400000,
                     CRC(d70e5ebc)
                         SHA1(2d560f6b6e693b2b91cf5ff5c4f0890cc2176f91)) // good
ROM_LOAD16_WORD_SWAP("mpr-19356.ic4", 0x0c00000, 0x0400000,
                     CRC(3bc43fe9)
                         SHA1(f72b0f3208e2f411f4c9cc76c317a605acd32a67)) // good
ROM_LOAD16_WORD_SWAP("mpr-19357.ic5", 0x1000000, 0x0400000,
                     CRC(aa749370)
                         SHA1(f09b0aa94fcc983419aaf2465dae5c9c64606158)) // good
ROM_LOAD16_WORD_SWAP("mpr-19358.ic6", 0x1400000, 0x0400000,
                     CRC(0969f1ec)
                         SHA1(25b9671f0975172f283458d46e160080dd1d19e9)) // good
ROM_LOAD16_WORD_SWAP("mpr-19359.ic1", 0x1800000, 0x0400000,
                     CRC(b0d06f9c)
                         SHA1(19e04c9c3a0bea5950aba8e1975962fa37722f32)) // good

ROM_REGION(0x1000, "subboard", ROMREGION_ERASE00)
ROM_LOAD("microm", 0x0000, 0x1000,
         NO_DUMP) // we are missing the driver ROM for sub-board (used to
                  // control the mic)

ROM_REGION16_BE(0x80, "eeprom", 0) // preconfigured to 3 players
ROM_LOAD("magzun.nv", 0x0000, 0x0080,
         CRC(42700321) SHA1(1f2ba760c410312539c8677223edcd1cda3b51d4))
ROM_END

// The internal ROM checks on these Atlus Print Club / Name clubs aren't very
// good
//  they test the ROM space, but usually test multiple ROMs under one IC label
//  in the test, and that IC label doesn't even relate to the actual cartridge
//  at all. furthermore several sets use blank roms, and even test that they're
//  present!
...
ROM_LOAD16_WORD_SWAP("mpr-21290.ic2", 0x0400000, 0x0400000,
                     CRC(a49d29f3)
                         SHA1(8f6c26fd9e94a9e03dd0029026d205cf481fe151)) // good
ROM_LOAD16_WORD_SWAP("mpr-21291.ic3", 0x0800000, 0x0400000,
                     CRC(9452ba20)
                         SHA1(8a9ff546901715f99bb911616c74ae30ebd7c6d7)) // good
ROM_LOAD16_WORD_SWAP("mpr-21292.ic4", 0x0c00000, 0x0400000,
                     CRC(f60268e2)
                         SHA1(5c2febb94553a941a68e9611617750a89c82e783)) // good
ROM_LOAD16_WORD_SWAP("mpr-21293.ic5", 0x1000000, 0x0400000,
                     CRC(794946e0)
                         SHA1(7881adb92fbf3efecb88f30834cdd1ed863b0b0e)) // good
ROM_LOAD16_WORD_SWAP("mpr-21294.ic6", 0x1400000, 0x0400000,
                     CRC(550843bb)
                         SHA1(5e278a0ae60f7bee23b5f1ee10b8ce31effc9718)) // good
ROM_LOAD16_WORD_SWAP("mpr-21289.ic1", 0x1800000, 0x0400000,
                     CRC(c2ee8bea)
                         SHA1(5fa0e6b492cc272c33b01adbef9233e6c8098827)) // good
ROM_LOAD16_WORD_SWAP("mpr-21296.ic8", 0x1c00000, 0x0400000,
                     CRC(b825c42a)
                         SHA1(7c6f737b69b9283345f8fa23cb1d110b9b5f3d7e)) // good
ROM_LOAD16_WORD_SWAP("mpr-21297.ic9", 0x2000000, 0x0400000,
                     CRC(4bff7469)
                         SHA1(8a36405a1a292d60b5918e604b460ed98740fae3)) // good
ROM_LOAD16_WORD_SWAP("mpr-21298.ic10", 0x2400000, 0x0400000,
                     CRC(68d07144)
                         SHA1(5021f43d19105b484135ebddd3fa203233096c98)) // good
ROM_LOAD16_WORD_SWAP("mpr-21299.ic11", 0x2800000, 0x0400000,
                     CRC(ecc521c6)
...
ROM_LOAD16_WORD_SWAP("ic22", 0x0200000, 0x0200000,
                     CRC(b4008ed0)
                         SHA1(acb3784acad971eb5f4920760dc23a16330e7bad)) // OK
ROM_LOAD16_WORD_SWAP("ic24", 0x0400000, 0x0200000,
                     CRC(4e894850)
                         SHA1(eb7c3399505a45816701197a45062b9f34e5a3e1)) // OK
ROM_LOAD16_WORD_SWAP("ic26", 0x0600000, 0x0200000,
                     CRC(5b6b023f)
                         SHA1(cf17c5857d85d4326dfe2ce40cf96989f9f78ecd)) // OK
ROM_LOAD16_WORD_SWAP("ic28", 0x0800000, 0x0200000,
                     CRC(b7beab03)
                         SHA1(de703e461a2bdd87b0695bd2f16e4c97d11bcf92)) // OK
ROM_LOAD16_WORD_SWAP("ic30", 0x0a00000, 0x0200000,
                     CRC(a9f81069)
                         SHA1(60d88c7c20178a00d6927c37069ab0c374ebf51e)) // OK
ROM_LOAD16_WORD_SWAP("ic32", 0x0c00000, 0x0200000,
                     CRC(02708d66)
                         SHA1(6881b0b05e55989953a16f6ba503ba891b849c07)) // OK
ROM_LOAD16_WORD_SWAP("ic34", 0x0e00000, 0x0200000,
                     CRC(c79d0537)
                         SHA1(9d595f718ff8f8ff7ca88100f35c589d5f9b4216)) // OK
ROM_LOAD16_WORD_SWAP("ic36", 0x1000000, 0x0200000,
                     CRC(0c9df896)
                         SHA1(4d8c18205e7aa90bfaa677ecff2b65128f2ad47c)) // OK
ROM_LOAD16_WORD_SWAP("ic23", 0x1200000, 0x0200000,
                     CRC(bd922829)
                         SHA1(4c6f988173e439a05a77da043d856e142b0da831)) // OK
ROM_LOAD16_WORD_SWAP("ic25", 0x1400000, 0x0200000,
                     CRC(f77f9e24)
                         SHA1(9a9636114e74c1fd7bd67db8005af02ef6a75ab1)) // OK

ROM_REGION16_BE(0x80, "eeprom", 0) // preconfigured to 1 player
ROM_LOAD("nclubv3.nv", 0x0000, 0x0080,
         CRC(9122a9e9) SHA1(5318994905e005567709c41449547c545182bece))
ROM_END
...
ROM_LOAD16_WORD_SWAP("ic22", 0x0200000, 0x0200000,
                     CRC(80eba7e9)
                         SHA1(d9f90a1aa508451bdd52fddda5ac65adc15f4764)) // OK
ROM_LOAD16_WORD_SWAP("ic24", 0x0400000, 0x0200000,
                     CRC(6bbb1b0c)
                         SHA1(ec4b0d49a3704804818d370458913e6ac443ea3e)) // OK
ROM_LOAD16_WORD_SWAP("ic26", 0x0600000, 0x0200000,
                     CRC(5352ade3)
                         SHA1(a5e060e98bfa7334152602e3568c89ff588926b8)) // OK
ROM_LOAD16_WORD_SWAP("ic28", 0x0800000, 0x0200000,
                     CRC(44f4b15a)
                         SHA1(e9e9e6dad4354e5bbad18efdc8ea0843a087cd59)) // OK
ROM_LOAD16_WORD_SWAP("ic30", 0x0a00000, 0x0200000,
                     CRC(fc4e56e6)
                         SHA1(63ff37eafcf854f33bd20c3c12950834fa10807f)) // OK
ROM_LOAD16_WORD_SWAP("ic32", 0x0c00000, 0x0200000,
                     CRC(6f825b87)
                         SHA1(7a69cc54d4ef42edddeb9fc0a199d0abc93e54df)) // OK
ROM_LOAD16_WORD_SWAP("ic34", 0x0e00000, 0x0200000,
                     CRC(62114a08)
                         SHA1(b44205b217fd982946b8446c7456afd97618a344)) // OK
ROM_LOAD16_WORD_SWAP(
    "ic36", 0x1000000, 0x0200000,
    CRC(9a4109e5) SHA1(
        ba59caac5f5a80fc52c507d8a47f322a380aa9a1)) // FIXED BITS (11111111), but
                                                   // present
ROM_LOAD16_WORD_SWAP(
    "ic23", 0x1200000, 0x0200000,
    CRC(9a4109e5) SHA1(
        ba59caac5f5a80fc52c507d8a47f322a380aa9a1)) // FIXED BITS (11111111), but
                                                   // present
ROM_LOAD16_WORD_SWAP(
...
ROM_REGION16_BE(0x80, "eeprom", 0) // preconfigured to 1 player
ROM_LOAD("nclubv4.nv", 0x0000, 0x0080,
         CRC(7efc9c6a) SHA1(3d44940b7497b07151908a0a6bc2809c7e15f4a8))
ROM_END
...
ROM_LOAD16_WORD_SWAP(
    ...
    BAD_DUMP CRC(cfd5eb54) SHA1(
        878195c70ad931d3015d497d2739a0867ff83cf7)) // tests bad but believed
                                                   // good, flagged so it gets
                                                   // double checked if possible
ROM_LOAD16_WORD_SWAP("ic24", 0x0400000, 0x0200000,
                     CRC(943682eb)
                         SHA1(ad41625244509affe7b6d6caffef58bb7bf75b72))
ROM_LOAD16_WORD_SWAP("ic26", 0x0600000, 0x0200000,
                     CRC(a17bd53a)
                         SHA1(e05079e3bfc7187b70ddc4b48de9e5ffcb19e473))
ROM_LOAD16_WORD_SWAP("ic28", 0x0800000, 0x0200000,
                     CRC(9a597bff)
                         SHA1(8cc265a89bbc42d4e6d55e23a4ed87fc1224c9dc))
ROM_LOAD16_WORD_SWAP("ic30", 0x0a00000, 0x0200000,
                     CRC(3eb020f6)
                         SHA1(6b87a6e7ab9eb820b7661fa6ea8c29bf8ead820d))
ROM_LOAD16_WORD_SWAP("ic32", 0x0c00000, 0x0200000,
                     CRC(ac26d375)
...
ROM_REGION16_BE(0x80, "eeprom", 0) // preconfigured to 1 player
ROM_LOAD("nclubdis.nv", 0x0000, 0x0080,
         CRC(7efc9c6a) SHA1(3d44940b7497b07151908a0a6bc2809c7e15f4a8))

ROM_REGION(0x400, "plds", ROMREGION_ERASE00)
ROM_LOAD("315-6055.ic12", 0x000, 0x117,
         NO_DUMP) // PALCE16V8H-10JC on the front side of the cart
ROM_LOAD("315-6056-01.ic13", 0x200, 0x117,
         NO_DUMP) // PALCE16V8-10JC on the back side of the cart
ROM_END
...
ROM_LOAD16_WORD_SWAP("ic22", 0x0200000, 0x0200000,
                     CRC(5058db21)
                         SHA1(eec908bbfb9ec0fdca0002e69f32c1c030086456)) // OK
ROM_LOAD16_WORD_SWAP("ic24", 0x0400000, 0x0200000,
                     CRC(34090f6d)
                         SHA1(b8bc344ab826d5c9584afb01dba1c720b8dbc74d)) // OK
ROM_LOAD16_WORD_SWAP("ic26", 0x0600000, 0x0200000,
                     CRC(fb073352)
                         SHA1(a5164aa5854ab3095f704ab73b6e4fb9ed0e0785)) // OK
ROM_LOAD16_WORD_SWAP("ic28", 0x0800000, 0x0200000,
                     CRC(530e0ceb)
                         SHA1(8d14eb9dbf253a4563587d256a15492384e7ca5c)) // OK
ROM_LOAD16_WORD_SWAP(
    "ic30", 0x0a00000, 0x0200000,
    CRC(8d89877e) SHA1(7d76d48d64d7ac5411d714a4bb83f37e3e5b8df6)) // 00 Fill, OK

ROM_REGION16_BE(0x80, "eeprom", 0) // preconfigured to 1 player
ROM_LOAD("techbowl.nv", 0x0000, 0x0080,
         CRC(5bebc2b7) SHA1(e189e891e1753059fbaad4ce82ddf191d5e8176a))
ROM_END
...
ROM_LOAD16_WORD_SWAP("ic22", 0x0200000, 0x0200000,
                     CRC(8385bc45)
                         SHA1(0bd60d7560cb2313d68470d0572850a7b8c501fd)) // OK
ROM_LOAD16_WORD_SWAP("ic24", 0x0400000, 0x0200000,
                     CRC(84ecb42f)
                         SHA1(005dee9a0912d4b1b7f5157bc3cde96548c1e348)) // OK
ROM_LOAD16_WORD_SWAP("ic26", 0x0600000, 0x0200000,
                     CRC(869bc19c)
                         SHA1(638125007d331beef567c976ea6f4e7a21acdb64)) // OK
ROM_LOAD16_WORD_SWAP("ic28", 0x0800000, 0x0200000,
                     CRC(0c3db354)
                         SHA1(c4d43da7cea1b4d5ca3ac545afde10344a4a385b)) // OK
ROM_LOAD16_WORD_SWAP("ic30", 0x0a00000, 0x0200000,
                     CRC(03b9eacf)
                         SHA1(d69c10f7613d9f52042dd6cce64e74e2b1ecc2d8)) // OK
ROM_LOAD16_WORD_SWAP("ic32", 0x0c00000, 0x0200000,
                     CRC(62c10626)
                         SHA1(58cb0ca0330fa7a62b277ab0ff84bff65b81bb23)) // OK
ROM_LOAD16_WORD_SWAP(
    "ic34", 0x1000000, 0x0200000,
    CRC(8d89877e) SHA1(7d76d48d64d7ac5411d714a4bb83f37e3e5b8df6)) // 00 Fill. OK
ROM_LOAD16_WORD_SWAP(
    "ic36", 0x1200000, 0x0200000,
    CRC(8d89877e) SHA1(7d76d48d64d7ac5411d714a4bb83f37e3e5b8df6)) // 00 Fill, OK

ROM_REGION16_BE(0x80, "eeprom", 0) // preconfigured to 1 player
ROM_LOAD("micrombc.nv", 0x0000, 0x0080,
...
ROM_LOAD16_WORD_SWAP("pclub2_ic22", 0x0200000, 0x0200000,
                     CRC(d2ceade7)
                         SHA1(a4300322e582f403d9207290f3900e1a72fcb9b9)) // OK
ROM_LOAD16_WORD_SWAP("pclub2_ic24", 0x0400000, 0x0200000,
                     CRC(0e968c2d)
                         SHA1(fbcc7533fcb6b87cd8255fc2d307ae618301ea64)) // OK
ROM_LOAD16_WORD_SWAP("pclub2_ic26", 0x0600000, 0x0200000,
                     CRC(ab51da70)
                         SHA1(85214aa805ffc9de59900dc0cd4e19e5ab756bf7)) // OK
ROM_LOAD16_WORD_SWAP("pclub2_ic28", 0x0800000, 0x0200000,
                     CRC(3a654b2a)
                         SHA1(7398e25836bfbdeab6350759f25c420c3b496172)) // OK
ROM_LOAD16_WORD_SWAP(
    "pclub2_ic30", 0x0a00000, 0x0200000,
    CRC(8d89877e) SHA1(7d76d48d64d7ac5411d714a4bb83f37e3e5b8df6)) // 00 Fill, OK

ROM_REGION16_BE(0x80, "eeprom", 0) // preconfigured to 1 player
ROM_LOAD("pclub2.nv", 0x0000, 0x0080,
         CRC(00d0f04e) SHA1(8b5a3e1c52e34443f83fd4a8948a00cacb5071d0))
...
ROM_LOAD16_WORD_SWAP("pclub2fc_ic22", 0x0200000, 0x0200000,
                     CRC(109c61a5)
                         SHA1(bfb3e014e49064443ef290bc068ffcd459ae132d)) // OK
ROM_LOAD16_WORD_SWAP("pclub2fc_ic24", 0x0400000, 0x0200000,
                     CRC(0ccc5538)
                         SHA1(15133fc9c85f0a384d49841d874a0fe9a76057ce)) // OK
ROM_LOAD16_WORD_SWAP(
    "pclub2fc_ic26", 0x0600000, 0x0200000,
    CRC(8d89877e) SHA1(7d76d48d64d7ac5411d714a4bb83f37e3e5b8df6)) // 00 Fill, OK
ROM_LOAD16_WORD_SWAP("pclub2fc_ic28", 0x0800000, 0x0200000,
                     CRC(ff9643ca)
                         SHA1(3309f970f87324b06cc48add386019f769abcd89)) // OK
ROM_LOAD16_WORD_SWAP("pclub2fc_ic30", 0x0a00000, 0x0200000,
                     CRC(03b9eacf)
                         SHA1(d69c10f7613d9f52042dd6cce64e74e2b1ecc2d8)) // OK

ROM_REGION16_BE(0x80, "eeprom", 0) // preconfigured to 1 player
ROM_LOAD("pclub2fc.nv", 0x0000, 0x0080,
         CRC(c8082326) SHA1(d24ad66eb01a58d3a117a49606003522b6f1feba))
ROM_END
...
ROM_LOAD16_WORD_SWAP("pclb2puf.ic22", 0x0200000, 0x0200000,
                     CRC(a14282f2)
                         SHA1(b96e70693d8e71b090e20efdd3aa6228e7289fa4)) // OK
ROM_LOAD16_WORD_SWAP("pclb2puf.ic24", 0x0400000, 0x0200000,
                     CRC(4fb4dc74)
                         SHA1(1f174512c9cd5420d7f935cbc6b5875836f6e825)) // OK
ROM_LOAD16_WORD_SWAP("pclb2puf.ic26", 0x0600000, 0x0200000,
                     CRC(d20bbfb5)
                         SHA1(5f2768e0e306bd0e3ed9b4e1d234aac8fd7155e6)) // OK
ROM_LOAD16_WORD_SWAP("pclb2puf.ic28", 0x0800000, 0x0200000,
                     CRC(da658ae9)
                         SHA1(24293c2b23b3009956fc05df5177a27415754301)) // OK
ROM_LOAD16_WORD_SWAP("pclb2puf.ic30", 0x0a00000, 0x0200000,
                     CRC(cafc0e6b)
                         SHA1(fa2ac54260336d5dd1ced7ccaf87115511ece1f8)) // OK

ROM_REGION16_BE(0x80, "eeprom", 0) // preconfigured to 1 player
ROM_LOAD("pclub2pf.nv", 0x0000, 0x0080,
         CRC(447bb3bd) SHA1(9fefec09849bfa0c14b49e73ff13e2a538dff511))
ROM_END
...
ROM_LOAD16_WORD_SWAP(
    ...
    CRC(589f6705) SHA1(d10897ab26c3ecdd518087562207de131133646c)) // OK - IC7
ROM_LOAD16_WORD_SWAP(
    "prc297wi_ic24", 0x0400000, 0x0200000,
    CRC(4bd706d1) SHA1(e3c52c63bb93d9fa836c300865423a226bf74586)) // OK - IC2
ROM_LOAD16_WORD_SWAP(
    "prc297wi_ic26", 0x0600000, 0x0200000,
    CRC(417e182a) SHA1(4df04a390523e52e48efcc48891bc54452f351c9)) // OK - IC2
ROM_LOAD16_WORD_SWAP(
    "prc297wi_ic28", 0x0800000, 0x0200000,
    CRC(73da594e) SHA1(936b0af4a32d5b93847bbf2ecfc8d334290059c0)) // OK - IC3
ROM_LOAD16_WORD_SWAP(
    "prc297wi_ic30", 0x0a00000, 0x0200000,
    CRC(03b9eacf) SHA1(d69c10f7613d9f52042dd6cce64e74e2b1ecc2d8)) // OK - IC3
ROM_LOAD16_WORD_SWAP(
    "prc297wi_ic32", 0x0c00000, 0x0200000,
    CRC(20437e93) SHA1(dfd2026bec6b2f418cd1cbfa7266717211d013b6)) // OK - IC4
ROM_LOAD16_WORD_SWAP(
    "prc297wi_ic34", 0x0e00000, 0x0200000,
    CRC(9639b003) SHA1(8f95b024ad19151e1e642d58aa785d14ae3a0661)) // OK - IC4
ROM_LOAD16_WORD_SWAP(
    "prc297wi_ic36", 0x1000000, 0x0200000,
    CRC(dd1b57b6) SHA1(8450355ec6cdc9718f8579f8702f3900f686c3f8)) // OK - IC5
ROM_LOAD16_WORD_SWAP(
    "prc297wi_ic23", 0x1200000, 0x0200000,
    CRC(e3d9d12b) SHA1(28ec3727774ef8a6a241238ad134a5adab8327fd)) // OK - IC5
ROM_LOAD16_WORD_SWAP(
    "prc297wi_ic25", 0x1400000, 0x0200000,
    CRC(71238374) SHA1(0dc534628a98aba508bdef58f8b812908414ae48)) // OK - IC6
ROM_LOAD16_WORD_SWAP(
    "prc297wi_ic27", 0x1600000, 0x0200000,
    CRC(7485a9a2) SHA1(17999a5192f185a27c08c2f05e19c65977b8f84e)) // OK - IC6

ROM_REGION16_BE(0x80, "eeprom", 0) // preconfigured to 1 player
ROM_LOAD("eeprom", 0x0000, 0x0080,
         CRC(9ba58358) SHA1(555ac21321b3051f7083cd72176ddc0fef2d4155))
ROM_END
...
ROM_LOAD16_WORD_SWAP(
    ...
    CRC(1feb3bfe) SHA1(cb79908a13e32c3c00e5892d988088a902d6f874)) // OK - IC7
ROM_LOAD16_WORD_SWAP(
    "prc297wi_ic24", 0x0400000, 0x0200000,
    CRC(4bd706d1) SHA1(e3c52c63bb93d9fa836c300865423a226bf74586)) // OK - IC2
ROM_LOAD16_WORD_SWAP(
...
ROM_LOAD16_WORD_SWAP(
    ...
    CRC(cb0ec98a)
        SHA1(efef536cb3bc71207936b26b87f04641baded10b)) // OK? - tested as IC7?
ROM_LOAD16_WORD_SWAP(
    "prc298sp_ic24", 0x0400000, 0x0200000,
    CRC(645e7e24)
        SHA1(7362b0c4b500639c20ec27002f543a0b4390eaa8)) // OK - tested as IC2
ROM_LOAD16_WORD_SWAP(
    "prc298sp_ic26", 0x0600000, 0x0200000,
    CRC(9d3ad85d)
        SHA1(71fe330594ab58be331aa5311472855be07cb44c)) // OK - tested as IC2
ROM_LOAD16_WORD_SWAP(
    "prc298sp_ic28", 0x0800000, 0x0200000,
    CRC(877e73cc)
        SHA1(dd9928a3fe0ed759611e1b7be8ea10b45084e392)) // OK - tested as IC3
ROM_LOAD16_WORD_SWAP(
    "prc298sp_ic30", 0x0a00000, 0x0200000,
    CRC(03b9eacf)
        SHA1(d69c10f7613d9f52042dd6cce64e74e2b1ecc2d8)) // OK - tested as IC3
ROM_LOAD16_WORD_SWAP(
    "prc298sp_ic32", 0x0c00000, 0x0200000,
    CRC(62c10626)
        SHA1(58cb0ca0330fa7a62b277ab0ff84bff65b81bb23)) // OK - tested as IC4
ROM_LOAD16_WORD_SWAP(
    "prc298sp_ic34", 0x0e00000, 0x0200000,
    CRC(8d89877e)
        SHA1(7d76d48d64d7ac5411d714a4bb83f37e3e5b8df6)) // 00 fill. OK - tested
                                                        // as IC4
ROM_LOAD16_WORD_SWAP(
    "prc298sp_ic36", 0x1000000, 0x0200000,
    CRC(8d89877e)
        SHA1(7d76d48d64d7ac5411d714a4bb83f37e3e5b8df6)) // 00 fill, OK - tested
                                                        // as IC5

ROM_REGION16_BE(0x80, "eeprom", 0) // preconfigured to 1 player
ROM_LOAD("prc298sp.nv", 0x0000, 0x0080,
         CRC(a23dd0f2) SHA1(457282b5d40a17477b95330bba91e05c603f951e))
ROM_END
...
ROM_LOAD16_WORD_SWAP(
    ...
    CRC(9720fe7a)
        SHA1(5b17eee0bd4574c0b2eb5bb64928d37bd87da23f)) // OK - tested as IC7
ROM_LOAD16_WORD_SWAP(
    "pclb298s_ic24", 0x0400000, 0x0200000,
    CRC(380496dc)
        SHA1(60a000cd71553cd23009ffd41b0208153b18d858)) // OK - tested as IC2
ROM_LOAD16_WORD_SWAP(
    "pclb298s_ic26", 0x0600000, 0x0200000,
    CRC(42622126)
        SHA1(cc312b1be51e919013ce55d4c1242a90676157e0)) // OK - tested as IC2
ROM_LOAD16_WORD_SWAP(
    "pclb298s_ic28", 0x0800000, 0x0200000,
    CRC(c03e861a)
        SHA1(e39e1d040651577d088ce88d354c74e96971efaa)) // OK - tested as IC3
ROM_LOAD16_WORD_SWAP(
    "pclb298s_ic30", 0x0a00000, 0x0200000,
    CRC(01844b12)
        SHA1(92d23e54cdfba8c0bdf3d87c52313334e2f903fa)) // OK - tested as IC3

ROM_REGION16_BE(0x80, "eeprom", 0) // preconfigured to 1 player
ROM_LOAD("prc298su.nv", 0x0000, 0x0080,
         CRC(6b81636a) SHA1(c84de7c374c46f92985186834ab023986e2abbd8))
ROM_END
...
ROM_LOAD16_WORD_SWAP("pclbvol6w_ic22", 0x0200000, 0x0200000,
                     CRC(72aa320c)
                         SHA1(09bc30e8cb00a5a4014c44e468cc64f6c3425d92))
ROM_LOAD16_WORD_SWAP("pclbvol6w_ic24", 0x0400000, 0x0200000,
                     CRC(d98371e2)
                         SHA1(813ac5f3c5b57d07cc319c73560bc0719ddcfe6b))
ROM_LOAD16_WORD_SWAP("pclbvol6w_ic26", 0x0600000, 0x0200000,
                     CRC(e6bbe3a5)
                         SHA1(b2f642b8ca0779ad66cfbbadece40f4e3dc41fd1))
ROM_LOAD16_WORD_SWAP("pclbvol6w_ic28", 0x0800000, 0x0200000,
                     CRC(3c330c9b)
                         SHA1(92f8e8d4f43db7c4ce431d17501492a7f8d8a867))
ROM_LOAD16_WORD_SWAP("pclbvol6w_ic30", 0x0a00000, 0x0200000,
                     CRC(67646090)
                         SHA1(ed6402a22acafa0203c587b871edc547f0ec5277))

ROM_REGION16_BE(0x80, "eeprom", 0) // preconfigured to 1 player
ROM_LOAD("pclub26w.nv", 0x0000, 0x0080,
         CRC(448f770d) SHA1(5f966c511c4c8e9d5b2d257c41c2c88a453b4944))
ROM_END
...
ROM_LOAD16_WORD_SWAP("ic22.bin", 0x0200000, 0x0200000,
                     CRC(a88e117d)
                         SHA1(afbc862c558f710b2a87e1326c91881996008055))
...
ROM_LOAD16_WORD_SWAP(
    ...
    CRC(d0412c8d) SHA1(
        4cb3c0ed3176bf86dee00649678a8e868819e92e)) // this is actually blank
                                                   // after 0x199404 but
                                                   // otherwise matches the rom
                                                   // in the parent set, the rom
                                                   // check expects this and
                                                   // fails with the rom from
                                                   // the parent set.
...
ROM_LOAD16_WORD_SWAP(
    ...
    CRC(00a0c702) SHA1(
        f2c4a7a51559f0ade96b8e6337cd1a1d61472de7)) // same as the Warner Bros
                                                   // and Print Club Kome Kome
                                                   // Club versions?!
...
ROM_LOAD16_WORD_SWAP("pclub2v7.ic22", 0x0200000, 0x0200000,
                     CRC(44c8ab27)
                         SHA1(65e2705b2918da32ea40375707df4e148b311159))
ROM_LOAD16_WORD_SWAP("pclub2v7.ic24", 0x0400000, 0x0200000,
                     CRC(24818437)
                         SHA1(5293d45b53680301abaf0b32a62596aaaa2552d6))
ROM_LOAD16_WORD_SWAP("pclub2v7.ic26", 0x0600000, 0x0200000,
                     CRC(076c1d44)
                         SHA1(d597ed4524bb03eb0ef8ada08d49f3dc0fc8136d))
ROM_LOAD16_WORD_SWAP("pclub2v7.ic28", 0x0800000, 0x0200000,
                     CRC(ff9643ca)
                         SHA1(3309f970f87324b06cc48add386019f769abcd89))
ROM_LOAD16_WORD_SWAP("pclub2v7.ic30", 0x0a00000, 0x0200000,
                     CRC(03b9eacf)
                         SHA1(d69c10f7613d9f52042dd6cce64e74e2b1ecc2d8))

ROM_REGION16_BE(0x80, "eeprom", 0) // preconfigured to 1 player
ROM_LOAD("pclub27s.nv", 0x0000, 0x0080,
         CRC(e58c7167) SHA1(d88b1648c5d86a90615a8c6a1bf87bc9e75dc320))
ROM_END
...
ROM_LOAD16_WORD_SWAP(
    ...
    CRC(b78cf122)
        SHA1(10ecdbb04d1ce2c7f98e57260bbd6a09b8a5905d)) // OK (tested as IC7)
ROM_LOAD16_WORD_SWAP(
    "u24.bin", 0x0400000, 0x0200000,
    CRC(aca05d29)
        SHA1(68c487821a912aea867f2f5041f5d926f31e4513)) // OK (tested as IC2)
ROM_LOAD16_WORD_SWAP(
    "u26.bin", 0x0600000, 0x0200000,
    CRC(5591f6e2)
        SHA1(95a28b0a4fc2aeb063902505a4f3613c33f9059a)) // OK (tested as IC2)
ROM_LOAD16_WORD_SWAP(
    "u28.bin", 0x0800000, 0x0200000,
    CRC(0899889b)
        SHA1(e7a77350a2421bbd867681f4c256d3b0d473468d)) // OK (tested as IC3)
ROM_LOAD16_WORD_SWAP(
    "u30.bin", 0x0a00000, 0x0200000,
    CRC(03b9eacf)
        SHA1(d69c10f7613d9f52042dd6cce64e74e2b1ecc2d8)) // OK (tested as IC3)
ROM_LOAD16_WORD_SWAP("u32.bin", 0x0c00000, 0x0200000,
                     CRC(5dc1f4d7)
                         SHA1(dbb04d4c45b3ecf5416f32e34d3cabd4945352fb)) // OK
ROM_LOAD16_WORD_SWAP("u34.bin", 0x0e00000, 0x0200000,
                     CRC(0222734a)
                         SHA1(d65311234100f81aa40a6738f79c61e598da8665)) // OK
ROM_LOAD16_WORD_SWAP("u36.bin", 0x1000000, 0x0200000,
                     CRC(57c30e0f)
                         SHA1(2286ba237edecc0cab1539b30b5f36843c7e276d)) // OK
...
// this is the first one to offer visual display of the frames in the Frame
// Count test menu?
...
ROM_LOAD16_WORD_SWAP(
    ...
    CRC(b7a9bfa4)
        SHA1(d756eb1232fb121ccce6d74feb86becc95335e11)) // OK (tested as IC7)
ROM_LOAD16_WORD_SWAP(
    "ic24.bin", 0x0400000, 0x0200000,
    BAD_DUMP CRC(91f36785)
        SHA1(1cdbb9236e119be37ad782b03e51ba401d0a1dd1)) // BAD? (tested as IC2)
ROM_LOAD16_WORD_SWAP(
    "ic26.bin", 0x0600000, 0x0200000,
    BAD_DUMP CRC(4c1b8823)
        SHA1(8b9f79da973e0331c04da3e776e922aaf2c0e0b7)) // BAD? (tested as IC2)
ROM_LOAD16_WORD_SWAP(
    "ic28.bin", 0x0800000, 0x0200000,
    CRC(594d200f)
        SHA1(9e490b4189348bfde44098dcfbcefb7f070f51fe)) // OK (tested as IC3)
ROM_LOAD16_WORD_SWAP(
    "ic30.bin", 0x0a00000, 0x0200000,
    CRC(03b9eacf)
        SHA1(d69c10f7613d9f52042dd6cce64e74e2b1ecc2d8)) // OK (tested as IC3)
// these aren't tested by the ROM check but can be viewed as 'frame set 2' in
// the Each Game Test -> Frame Count -> Frame Count Display menu they match the
// ones found on pckobe99 except for the final rom, which might be a better dump
// of the one in the pckobe99 romset
ROM_LOAD16_WORD_SWAP(
...
ROM_LOAD16_WORD_SWAP(
    ...
    CRC(524a1c4e)
        SHA1(182dcc9d4808595ec67b4f80a5bf6d25016bf2a7)) // not tested?!
...
ROM_LOAD16_WORD_SWAP("pclb2psi_ic22", 0x0200000, 0x0200000,
                     CRC(caadc660)
                         SHA1(f2e84bee96266bb03d8f9009249c17c27935f82e))
ROM_LOAD16_WORD_SWAP("pclb2psi_ic24", 0x0400000, 0x0200000,
                     CRC(ece82698)
                         SHA1(b17b1ea8adc13c3722067c9854d1b7fdf3917090))
ROM_LOAD16_WORD_SWAP("pclb2psi_ic26", 0x0600000, 0x0200000,
                     CRC(c8a1e335)
                         SHA1(a95ddfc41fdd9f720c11208f45ef5db4bee6cb97))
ROM_LOAD16_WORD_SWAP("pclb2psi_ic28", 0x0800000, 0x0200000,
                     CRC(52f09627)
                         SHA1(e2ffc321bb0f2a650d0c0b39c3ec68226e1ca7f4))
ROM_LOAD16_WORD_SWAP("pclb2psi_ic30", 0x0a00000, 0x0200000,
                     CRC(03b9eacf)
                         SHA1(d69c10f7613d9f52042dd6cce64e74e2b1ecc2d8))

ROM_REGION16_BE(0x80, "eeprom", 0) // preconfigured to 1 player
ROM_LOAD("pclub2pe.nv", 0x0000, 0x0080,
         CRC(447bb3bd) SHA1(9fefec09849bfa0c14b49e73ff13e2a538dff511))
ROM_END
...
ROM_LOAD16_WORD_SWAP("pclb2wb_ic22", 0x0200000, 0x0200000,
                     CRC(12245be7)
                         SHA1(4d6c2c9ca7fe73a9ec490157cdb01a6228dee7f8))
ROM_LOAD16_WORD_SWAP("pclb2wb_ic24", 0x0400000, 0x0200000,
                     CRC(e5d6e11e)
                         SHA1(4af3c646747f76d99482c985f960df2519a85c23))
ROM_LOAD16_WORD_SWAP("pclb2wb_ic26", 0x0600000, 0x0200000,
                     CRC(7ee066f0)
                         SHA1(a7c725ce8e621ed299474dd215174699e097db3f))
ROM_LOAD16_WORD_SWAP("pclb2wb_ic28", 0x0800000, 0x0200000,
                     CRC(9ed59513)
                         SHA1(c8f5ed13be2a91f83c35a7929aaa5751d7843e6e))
ROM_LOAD16_WORD_SWAP("pclb2wb_ic30", 0x0a00000, 0x0200000,
                     CRC(00a0c702)
                         SHA1(f2c4a7a51559f0ade96b8e6337cd1a1d61472de7))

ROM_REGION16_BE(0x80, "eeprom", 0) // preconfigured to 1 player
ROM_LOAD("pclub2wb.nv", 0x0000, 0x0080,
         CRC(0d442eec) SHA1(54dd544e1496e3999d8111eb06abf805b610d77d))
ROM_END
...
ROM_LOAD16_WORD_SWAP("pclbyov1.ic22", 0x0200000, 0x0200000,
                     CRC(769468d1)
                         SHA1(6b7b4b18d275f8c2192ad68840397db29daf0874))
ROM_LOAD16_WORD_SWAP("pclbyov1.ic24", 0x0400000, 0x0200000,
                     CRC(b25885bf)
                         SHA1(290dae5478419bc02e0b0f2045542b745749ff4f))
ROM_LOAD16_WORD_SWAP("pclbyov1.ic26", 0x0600000, 0x0200000,
                     CRC(cefc697f)
                         SHA1(68acfea483f709d4390ab1f7a826f76dd11856a8))
ROM_LOAD16_WORD_SWAP(
    "pclbyov1.ic28", 0x0800000, 0x0200000,
    CRC(3c330c9b) SHA1(
        92f8e8d4f43db7c4ce431d17501492a7f8d8a867)) // same as some other sets
ROM_LOAD16_WORD_SWAP(
    "pclbyov1.ic30", 0x0a00000, 0x0200000,
    CRC(00a0c702) SHA1(
        f2c4a7a51559f0ade96b8e6337cd1a1d61472de7)) // same as some other sets

ROM_REGION16_BE(0x80, "eeprom", 0) // preconfigured to 1 player
ROM_LOAD("pclubyo.nv", 0x0000, 0x0080,
         CRC(2021e0e5) SHA1(31e1107d31f02a259c123c42fe7f0202ad78d774))
...
ROM_LOAD16_WORD_SWAP("pclbyov2.ic22", 0x0200000, 0x0200000,
                     CRC(719a4d27)
                         SHA1(328dfb8debea02e8660e636e953982d381529945))
ROM_LOAD16_WORD_SWAP("pclbyov2.ic24", 0x0400000, 0x0200000,
                     CRC(790dc7b5)
                         SHA1(829ead39930779617a9bef41d8615362ca86c4c7))
ROM_LOAD16_WORD_SWAP("pclbyov2.ic26", 0x0600000, 0x0200000,
                     CRC(12ae1606)
                         SHA1(9534fb2dbf6fd2c258ba2716783cc5bab8bd8dc0))
ROM_LOAD16_WORD_SWAP("pclbyov2.ic28", 0x0800000, 0x0200000,
                     CRC(ff9643ca)
                         SHA1(3309f970f87324b06cc48add386019f769abcd89))
ROM_LOAD16_WORD_SWAP("pclbyov2.ic30", 0x0a00000, 0x0200000,
                     CRC(03b9eacf)
                         SHA1(d69c10f7613d9f52042dd6cce64e74e2b1ecc2d8))

ROM_REGION16_BE(0x80, "eeprom", 0) // preconfigured to 1 player
ROM_LOAD("pclubyo2.nv", 0x0000, 0x0080,
         CRC(2b26a8f7) SHA1(32f34096cac05a37c492ee389ed8e4c02694c268))
ROM_END
...
ROM_LOAD16_WORD_SWAP("pclubsc5.nv", 0x0000, 0x0080,
                     CRC(5a01a17b)
                         SHA1(f9aad386641b640d9e3e96becdd58d08103fb7ff))
...
// 837-12765-09 PCB with populated 317-0231 protection device. Sports an
// AT28C16.
ROM_START(pclubsc6)
STV_BIOS
...
ROM_LOAD16_WORD_SWAP("lh28f016sut-10.ic22", 0x0200000, 0x0200000,
                     CRC(6662b00e)
                         SHA1(8e972bf14f3a4415d0a5108764600a16ed0a17f1)
                             BAD_DUMP)
ROM_LOAD16_WORD_SWAP(
    "lh28f016sut-10.ic24", 0x0400000, 0x0200000,
    CRC(35721f04)
        SHA1(9658c9526a3d1dde89e1b6fd986b3469011813ca)) // same as pclubol
ROM_LOAD16_WORD_SWAP("lh28f016sut-10.ic26", 0x0600000, 0x0200000,
                     CRC(1b621078)
                         SHA1(f0f0a03739420f838ea27a78b2aa39e0b35bc9c2))
ROM_LOAD16_WORD_SWAP("lh28f016sut-10.ic28", 0x0800000, 0x0200000,
                     CRC(295970fc)
                         SHA1(f4dd5b24749469bd60f27442fd491349cb08c7f6))
ROM_LOAD16_WORD_SWAP("lh28f016sut-10.ic30", 0x0a00000, 0x0200000,
                     CRC(2cc5a926)
                         SHA1(f80b26cf2a7e7c5a1011b0eb18d7e79d8ec3c574))
...
ROM_LOAD16_WORD_SWAP("lh28f016sut-10.ic22", 0x0200000, 0x0200000,
                     CRC(4550ccf5)
                         SHA1(fccdb52dd0b0e3c167fedf4200b042a0adef4b88)
                             BAD_DUMP)
ROM_LOAD16_WORD_SWAP("lh28f016sut-10.ic24", 0x0400000, 0x0200000,
                     CRC(b1577a65)
                         SHA1(4a64ecd6e3bf2c5c5d11413b17a4bedd09622a52))
ROM_LOAD16_WORD_SWAP(
    "lh28f016sut-10.ic26", 0x0600000, 0x0200000,
    CRC(5655091e) SHA1(590ef446d4db57cd5bff0a8e10f08549d8df5c91)) // 1xxxxxxxxxxxxxxxxxxxx
                                                                  // = 0x00
ROM_LOAD16_WORD_SWAP("lh28f016sut-10.ic28", 0x0800000, 0x0200000,
                     CRC(fb98a97c)
                         SHA1(3411411580505e6d223912ca9e1e1746c4060a2f))
ROM_LOAD16_WORD_SWAP("lh28f016sut-10.ic30", 0x0a00000, 0x0200000,
                     CRC(b5220345)
                         SHA1(e9fce1a8c7c298d2f571ca70c16023808a95dd24))
...
ROM_START(pclubnbc) // 837-12765-04 (stickered) ROM BD, protection device
                    // (317-0230) present
STV_BIOS
...
ROM_LOAD16_WORD_SWAP("lh28f016sut-10.ic22", 0x0200000, 0x0200000,
                     CRC(b0207086)
                         SHA1(4f6db1799759e87c6b7b86313ea2f2bae6301450))
ROM_LOAD16_WORD_SWAP("lh28f016sut-10.ic24", 0x0400000, 0x0200000,
                     CRC(1241ae77)
                         SHA1(e2b8b769b04e25fa9365f1971289a66c24cd9ea0))
ROM_LOAD16_WORD_SWAP("lh28f016sut-10.ic26", 0x0600000, 0x0200000,
                     CRC(d7ff1134)
                         SHA1(a4c8bec3d03f63fd1e881943b15819fd49d00d19))
ROM_LOAD16_WORD_SWAP("lh28f016sut-10.ic28", 0x0800000, 0x0200000,
                     CRC(de38b841)
                         SHA1(8604bc6459fb167490e9a4ef8a6227da4a6ff052))
ROM_LOAD16_WORD_SWAP("lh28f016sut-10.ic30", 0x0a00000, 0x0200000,
                     CRC(499edabf)
                         SHA1(cad2bd0e8f579c406fdbae9d63397e636191c3d6))
ROM_LOAD16_WORD_SWAP(
    "lh28f016sut-10.ic32", 0x0c00000, 0x0200000,
    CRC(9a4109e5)
        SHA1(ba59caac5f5a80fc52c507d8a47f322a380aa9a1)) // present but empty
...
ROM_LOAD16_WORD_SWAP("lh28f016sut-10.ic22", 0x0200000, 0x0200000,
                     CRC(a8fef41b)
                         SHA1(940a17888614771df7a997c0f474160033427f7f))
ROM_LOAD16_WORD_SWAP("lh28f016sut-10.ic24", 0x0400000, 0x0200000,
                     CRC(5f550c41)
                         SHA1(9a6e8eca437fb5da7a53363021f1acb03b95b75b))
ROM_LOAD16_WORD_SWAP("lh28f016sut-10.ic26", 0x0600000, 0x0200000,
                     CRC(b26d9dba)
                         SHA1(5aad355eebce4d3e0f437d3fd87b401082d275ad))
ROM_LOAD16_WORD_SWAP("lh28f016sut-10.ic28", 0x0800000, 0x0200000,
                     CRC(4b9df566)
                         SHA1(ac339b204d99db49861721a86f4549ab7e855dd5))
ROM_LOAD16_WORD_SWAP("lh28f016sut-10.ic30", 0x0a00000, 0x0200000,
                     CRC(03b9eacf)
                         SHA1(d69c10f7613d9f52042dd6cce64e74e2b1ecc2d8))
ROM_LOAD16_WORD_SWAP(
...
ROM_LOAD16_WORD_SWAP(
    ...
    CRC(8d89877e)
        SHA1(7d76d48d64d7ac5411d714a4bb83f37e3e5b8df6)) // present but empty
ROM_LOAD16_WORD_SWAP(
    "lh28f016sut-10.ic36", 0x1000000, 0x0200000,
    CRC(8d89877e)
        SHA1(7d76d48d64d7ac5411d714a4bb83f37e3e5b8df6)) // present but empty
...
ROM_LOAD16_WORD_SWAP("prc298au_ic22", 0x0200000, 0x0200000,
                     CRC(21a995ce)
                         SHA1(6ee1250becd76bef3aa8044a42e10c3830a609bd)) // OK
ROM_LOAD16_WORD_SWAP("prc298au_ic24", 0x0400000, 0x0200000,
                     CRC(94540f39)
                         SHA1(cee9fff48d177e7502802d366339ed922c212871)) // OK
ROM_LOAD16_WORD_SWAP("prc298au_ic26", 0x0600000, 0x0200000,
                     CRC(8b22c41f)
                         SHA1(371f8b35ed45f695f5ec0c8db2c4b62007bf4782)) // OK
ROM_LOAD16_WORD_SWAP("prc298au_ic28", 0x0800000, 0x0200000,
                     CRC(bf68cec0)
                         SHA1(550138f5110661d69eaff44c0596914a2621c3df)) // OK
ROM_LOAD16_WORD_SWAP("prc298au_ic30", 0x0a00000, 0x0200000,
                     CRC(ae276c06)
                         SHA1(98358860ae9bf7c405ba4f763c7a4bf309ce85e3)) // OK
ROM_LOAD16_WORD_SWAP("prc298au_ic32", 0x0c00000, 0x0200000,
                     CRC(a3fb81f5)
                         SHA1(6c78c97635dd486d2a7c09bc0511267eae6082c4)) // OK
ROM_LOAD16_WORD_SWAP("prc298au_ic34", 0x0e00000, 0x0200000,
                     CRC(04200dc9)
                         SHA1(e40b01d12ccf71e50da7fd0f3000158626e5a98d)) // OK
ROM_LOAD16_WORD_SWAP(
    "prc298au_ic36", 0x1000000, 0x0200000,
    CRC(9a4109e5)
        SHA1(ba59caac5f5a80fc52c507d8a47f322a380aa9a1)) // (blank! - not tested)

ROM_REGION16_BE(0x80, "eeprom", 0) // preconfigured to 1 player
ROM_LOAD("prc298au.nv", 0x0000, 0x0080,
         CRC(b4440ff0) SHA1(bd3c83221ede11c68163df4b52a85856c83f865f))
ROM_END
...
ROM_LOAD16_WORD_SWAP("pclubor_ic22", 0x0200000, 0x0200000,
                     CRC(b25072f7)
                         SHA1(baa674e3d277bee152773b3b0e522677bfedd65c)) // OK
ROM_LOAD16_WORD_SWAP("pclubor_ic24", 0x0400000, 0x0200000,
                     CRC(a4863a0e)
                         SHA1(b092b638b6a8d06776f88aac7e47fff06b4e8221)) // OK
ROM_LOAD16_WORD_SWAP("pclubor_ic26", 0x0600000, 0x0200000,
                     CRC(7f55baf7)
                         SHA1(344f08590484a5ece3a64fe3e2e84a7d386fb8c1)) // OK
ROM_LOAD16_WORD_SWAP("pclubor_ic28", 0x0800000, 0x0200000,
                     CRC(cafd2a7d)
                         SHA1(ff5f6ac7f5745d21db4a9c80eaec6f23d6084f23)) // OK
ROM_LOAD16_WORD_SWAP("pclubor_ic30", 0x0a00000, 0x0200000,
                     CRC(ccf6f885)
                         SHA1(a01279a06cbc0c724c5c03b985f979458a2a2e65)) // OK
ROM_LOAD16_WORD_SWAP("pclubor_ic32", 0x0c00000, 0x0200000,
                     CRC(62e6d1e1)
                         SHA1(5ea2ed73c5c02efbc85cd047983a02947ae829ff)) // OK
ROM_LOAD16_WORD_SWAP("pclubor_ic34", 0x0e00000, 0x0200000,
                     CRC(19cdd167)
                         SHA1(a474134f58101dbbff3a47001ca26a69b03ce830)) // OK
ROM_LOAD16_WORD_SWAP(
    "pclubor_ic36", 0x1000000, 0x0200000,
    CRC(9a4109e5)
        SHA1(ba59caac5f5a80fc52c507d8a47f322a380aa9a1)) // (blank! - not tested)

ROM_REGION16_BE(0x80, "eeprom", 0) // preconfigured to 1 player
ROM_LOAD("pclubor.nv", 0x0000, 0x0080,
         CRC(3ad918c0) SHA1(70b63d78948e2fc26fd95f98b93ed86c6130ed8e))
ROM_END
...
ROM_LOAD16_WORD_SWAP("pclubol_ic22", 0x0200000, 0x0200000,
                     CRC(668b1049)
                         SHA1(a88ab4fbab73f85eb4da8d4792e98fbbefeba5f8)) // OK
ROM_LOAD16_WORD_SWAP("pclubol_ic24", 0x0400000, 0x0200000,
                     CRC(35721f04)
                         SHA1(9658c9526a3d1dde89e1b6fd986b3469011813ca)) // OK
ROM_LOAD16_WORD_SWAP("pclubol_ic26", 0x0600000, 0x0200000,
                     CRC(17b3cc4b)
                         SHA1(226eba06dd7db14fd3d10a1ca687293f2c93db9d)) // OK
ROM_LOAD16_WORD_SWAP("pclubol_ic28", 0x0800000, 0x0200000,
                     CRC(7cfaa530)
                         SHA1(c04742acb692a028021188deea4617c9955db38c)) // OK
ROM_LOAD16_WORD_SWAP("pclubol_ic30", 0x0a00000, 0x0200000,
                     CRC(e1dd7854)
                         SHA1(190b85f4e25d5b98ea893033004a6ee698ade8c4)) // OK
ROM_LOAD16_WORD_SWAP("pclubol_ic32", 0x0c00000, 0x0200000,
                     CRC(f0a3ded7)
                         SHA1(574417b9fb234ea75893dd22c65d2a3c4e3e4aa1)) // OK
ROM_LOAD16_WORD_SWAP("pclubol_ic34", 0x0e00000, 0x0200000,
                     CRC(53aa9821)
                         SHA1(b866e0255297d48fbcab93bc044e198bdff8c6d3)) // OK
ROM_LOAD16_WORD_SWAP(
    "pclubol_ic36", 0x1000000, 0x0200000,
    CRC(9a4109e5)
        SHA1(ba59caac5f5a80fc52c507d8a47f322a380aa9a1)) // (blank! - not tested)

ROM_REGION16_BE(0x80, "eeprom", 0) // preconfigured to 1 player
ROM_LOAD("eeprom", 0x0000, 0x0080,
         CRC(a744ca03) SHA1(f82ca58469356fad8c0fc53d54e7a4714ba43dd3))
ROM_END
...
ROM_LOAD16_WORD_SWAP("pclub2v3_ic22", 0x0200000, 0x0200000,
                     BAD_DUMP CRC(f88347aa)
                         SHA1(3e9ca105edbd6ce11ea4194eb1733785e87f92b2)) // BAD
ROM_LOAD16_WORD_SWAP("pclub2v3_ic24", 0x0400000, 0x0200000,
                     CRC(b5871198)
                         SHA1(10d187eebcca5d70c5ae10d1a144685a96491126)) // OK
ROM_LOAD16_WORD_SWAP("pclub2v3_ic26", 0x0600000, 0x0200000,
                     CRC(d97034ed)
                         SHA1(a7a0f659eefd539b2a1fd70ef394eed30ea54c0c)) // OK
ROM_LOAD16_WORD_SWAP("pclub2v3_ic28", 0x0800000, 0x0200000,
                     CRC(f1421506)
                         SHA1(c384b695338144e5f051134bda73b059b678a7df)) // OK
ROM_LOAD16_WORD_SWAP(
    "pclub2v3_ic30", 0x0a00000, 0x0200000,
    CRC(8d89877e) SHA1(7d76d48d64d7ac5411d714a4bb83f37e3e5b8df6)) // 00 Fill, OK

ROM_REGION16_BE(0x80, "eeprom", 0) // preconfigured to 1 player
ROM_LOAD("pclub2v3.nv", 0x0000, 0x0080,
         CRC(a8a2d30c) SHA1(bdde3d62ff21190a23698058ff66e476a75a09aa))
ROM_END
...
ROM_LOAD16_WORD_SWAP("pclubpok_ic22", 0x0200000, 0x0200000,
                     CRC(48ab8371)
                         SHA1(1c2124afad6bc1f4de2619e6b915f78e91addf05)) // OK
ROM_LOAD16_WORD_SWAP("pclubpok_ic24", 0x0400000, 0x0200000,
                     CRC(9915faea)
                         SHA1(b96f64a8cbb1b9496bb566d0469975fddb4fbe98)) // OK
ROM_LOAD16_WORD_SWAP("pclubpok_ic26", 0x0600000, 0x0200000,
                     CRC(054ad120)
                         SHA1(987e812af099bb0aa5d43a4c10ae8345370a2606)) // OK
ROM_LOAD16_WORD_SWAP("pclubpok_ic28", 0x0800000, 0x0200000,
                     CRC(3a654b2a)
                         SHA1(7398e25836bfbdeab6350759f25c420c3b496172)) // OK
ROM_LOAD16_WORD_SWAP("pclubpok_ic30", 0x0a00000, 0x0200000,
                     CRC(98747bef)
                         SHA1(8d452507a9842a48cd1a7db8fde11a070a6f068b)) // OK

ROM_REGION16_BE(0x80, "eeprom", 0) // preconfigured to 1 player
ROM_LOAD("pclubpok.nv", 0x0000, 0x0080,
         CRC(4ba3f21a) SHA1(898a393fb2fc1961b68b7d4f383d5447e6c010e8))
ROM_END
...
ROM_LOAD16_WORD_SWAP(
    ...
    CRC(9eb4cfd7)
        SHA1(d74b00541419b5cde75409315fdc620a7cb3ac6b)) // OK (tested as IC7)
ROM_LOAD16_WORD_SWAP(
    "pclub2kc_ic24", 0x0400000, 0x0200000,
    CRC(cf3b4080)
        SHA1(1963a45d391f8d591d3995231b26eba56c7896c6)) // OK (tested as IC2)
ROM_LOAD16_WORD_SWAP(
    "pclub2kc_ic26", 0x0600000, 0x0200000,
    CRC(dbdcb1d7)
        SHA1(33bd8c9e351864cdd5f5be2aba62efda1c4a0f4f)) // OK (tested as IC2)
ROM_LOAD16_WORD_SWAP(
    "pclub2kc_ic28", 0x0800000, 0x0200000,
    CRC(3c330c9b)
        SHA1(92f8e8d4f43db7c4ce431d17501492a7f8d8a867)) // OK (tested as IC3)
ROM_LOAD16_WORD_SWAP(
    "pclub2kc_ic30", 0x0a00000, 0x0200000,
    CRC(00a0c702)
        SHA1(f2c4a7a51559f0ade96b8e6337cd1a1d61472de7)) // OK (tested as IC3)

ROM_REGION16_BE(0x80, "eeprom", 0) // preconfigured to 1 player
ROM_LOAD("pclub2kc.nv", 0x0000, 0x0080,
         CRC(064366fe) SHA1(b85489ae19ddc0fbd67b441a6967f10a6cd22d45))
ROM_END
...
ROM_LOAD16_WORD_SWAP(
    ...
    CRC(2faed82a)
        SHA1(035ef25dd974679e46a79e408ee284eca0310557)) // OK (tested as IC7)
ROM_LOAD16_WORD_SWAP(
    "pclb2elk_ic24", 0x0400000, 0x0200000,
    CRC(9cacfb7b)
        SHA1(1c68e1ba077e02ded0f388b4e9ad24998a5d8a48)) // OK (tested as IC2)
ROM_LOAD16_WORD_SWAP(
    "pclb2elk_ic26", 0x0600000, 0x0200000,
    CRC(533a189e)
        SHA1(23a08a9ab02b21d6c75c770f4adc33c6dfe98a4e)) // OK (tested as IC2)
ROM_LOAD16_WORD_SWAP(
    "pclb2elk_ic28", 0x0800000, 0x0200000,
    CRC(1f0c9113)
        SHA1(b65fc9012cb159c374674cde1fd2fb2cf192e7c5)) // OK (tested as IC3)
ROM_LOAD16_WORD_SWAP(
    "pclb2elk_ic30", 0x0a00000, 0x0200000,
    CRC(0e188b8c)
        SHA1(8574f884fbeb2f428913c24e147ee8753305ce86)) // OK (tested as IC3)

ROM_REGION16_BE(0x80, "eeprom", 0) // preconfigured to 1 player
ROM_LOAD("pclb2elk.nv", 0x0000, 0x0080,
         CRC(54c7564f) SHA1(574dcc5e8fe4aac091fee1476347485ed660eddd))
ROM_END
...
ROM_REGION16_BE(0x80, "eeprom", 0) // preconfigured to 1 player
ROM_LOAD("pclove.nv", 0x0000, 0x0080,
         CRC(3c78e3bd) SHA1(6d5fe8545f434b4cc1e8229549adb0a49ac45bd1))
ROM_END
...
ROM_REGION16_BE(0x80, "eeprom", 0) // preconfigured to 1 player
ROM_LOAD("pclove2.nv", 0x0000, 0x0080,
         CRC(93b30600) SHA1(eadba12ec322911823a6873a343e4d9b4089ed93))
ROM_END
...
ROM_REGION16_BE(0x80, "eeprom", 0) // preconfigured to 1 player
ROM_LOAD("eeprom", 0x0000, 0x0080,
         CRC(5aee29d0) SHA1(a1dd443cf2dd9d52ca45ea687eff2f88cae25b85))
ROM_END
...
ROM_REGION16_BE(0x80, "eeprom", 0) // preconfigured to 1 player
ROM_LOAD("eeprom", 0x0000, 0x0080,
         CRC(e41d541b) SHA1(511fe00745787ee5dbd813125ddc3db921d6531e))
ROM_END
...
// have an unusual rom mapping compared to other games, the cartridge is a
// little different too, with a large PALCE16V8H-10 marked 315-6026 For Name
// Club vol. 2, the protection device (317-0229 on both) is checked in the 'each
// game test' menu as 'RCDD2' For the service mode test the game just passes a
// large block of compressed data and checksums the result, it doesn't even look
// like it's passing 100% valid data, just an entire section of ROM, checking
// the result against a pre-calculated checksum.

// The device is accessed by the game when you choose to print, it looks like
// it's decompressing the full-size graphics for the printer rather than
// anything you see onscreen.  It makes quite extensive use of the device, with
// lots of different dictionaries, unlike Decathlete where there are only 2 that
// cover all the data.
...
ROM_LOAD16_WORD_SWAP("ic22", 0x0200000, 0x0200000,
                     CRC(ac23c648)
                         SHA1(4dd099a92ff162082eb24a61a277ca907b3f9892)) // OK
ROM_LOAD16_WORD_SWAP("ic24", 0x0600000, 0x0200000,
                     CRC(a16902e3)
                         SHA1(85c582cb0d02ef028a8ae32688c20a5b5aeeaae8)) // OK
ROM_LOAD16_WORD_SWAP("ic26", 0x0a00000, 0x0200000,
                     CRC(a5eab3f3)
                         SHA1(1b7263639bb8f4aa644cc46133988ef4d2b6c9de)) // OK
ROM_LOAD16_WORD_SWAP("ic28", 0x0e00000, 0x0200000,
                     CRC(34ed677a)
                         SHA1(ff2c4dd8fae33ac618f6e3e28ba71c4ecb4ca88f)) // OK

ROM_REGION16_BE(0x80, "eeprom", 0) // preconfigured to 1 player
ROM_LOAD("nameclub.nv", 0x0000, 0x0080,
         CRC(680a64bc) SHA1(45194bbe4a7e67f0e44f858589881967884f63a6))
ROM_END
...
ROM_REGION32_BE(0x3000000, "cart", ROMREGION_ERASEFF) /* SH2 code */
ROM_LOAD16_WORD_SWAP("nclubv2.ic22", 0x0200000, 0x0200000,
                     CRC(7e81676d)
                         SHA1(fc0f0dcdb4aaf71218d7c1dd0e4ddc5381e8b13b)) // OK
ROM_LOAD16_WORD_SWAP("nclubv2.ic24", 0x0600000, 0x0200000,
                     CRC(1b7637de)
                         SHA1(43c3094f60a6582298a45bad923fef57e98c5b2b)) // OK
ROM_LOAD16_WORD_SWAP("nclubv2.ic26", 0x0a00000, 0x0200000,
                     CRC(630be99d)
                         SHA1(ac7fbaae98b126fad5228b0ebffa91a0f0a94516)) // OK
ROM_LOAD16_WORD_SWAP("nclubv2.ic28", 0x0e00000, 0x0200000,
                     CRC(1a3ca5e2)
                         SHA1(4d3aed51d29c54e71175d828f648c9feb813ac04)) // OK

ROM_REGION16_BE(0x80, "eeprom", 0) // preconfigured to 1 player
ROM_LOAD("nclubv2.nv", 0x0000, 0x0080,
         CRC(96d55fa9) SHA1(b3c821d6cd4ed52d0e20565e12a06d8f81a08dbc))
ROM_END
...
ROM_REGION32_BE(0x3000000, "cart", ROMREGION_ERASE00) // SH2 code
ROM_LOAD16_WORD_SWAP("ic22", 0x0200000, 0x200000,
                     CRC(43f7cafc)
                         SHA1(ac8f8efb752d6357d48090b3152e77fedea90a6f))
ROM_LOAD16_WORD_SWAP("ic24", 0x0400000, 0x200000,
                     CRC(db560e59)
                         SHA1(edaabef7a165a4c309b4a6bac0f6de6bf8fedb34))
ROM_LOAD16_WORD_SWAP("ic26", 0x0600000, 0x200000,
                     CRC(2f411ff1)
                         SHA1(e7bfb6570c0e5b5b64c4291bdc13fe8a62a69382))
ROM_LOAD16_WORD_SWAP("ic28", 0x0800000, 0x200000,
                     CRC(ca5e0446)
                         SHA1(8e1c5c5c35f617129aaf40a1096f409cf7c96dce))
ROM_LOAD16_WORD_SWAP("ic30", 0x0a00000, 0x200000,
                     CRC(2653f758)
                         SHA1(606dcdfe4e1b0fef495a5f62b2414c80405ebeeb))
ROM_LOAD16_WORD_SWAP("ic32", 0x0c00000, 0x200000,
                     CRC(8ebfd947)
                         SHA1(db12bb43aadeb7ca48434286e7ecf2a7c8680e44))
ROM_LOAD16_WORD_SWAP("ic34", 0x0e00000, 0x200000,
                     CRC(22b7180f)
                         SHA1(56aae3c4a9db9dd7443e2f0aa30052fc50b2ab70))
ROM_LOAD16_WORD_SWAP("ic36", 0x1000000, 0x200000,
                     CRC(5b83914c)
...
ROM_REGION32_BE(0x3000000, "cart", ROMREGION_ERASE00) // SH2 code
ROM_LOAD16_WORD_SWAP("13", 0x0000000, 0x080000,
                     CRC(ecd7ac4b)
                         SHA1(9bb4ae5ac236192bb6e927a3b5b75f84264e36aa))
ROM_LOAD16_WORD_SWAP("1", 0x0400000, 0x400000,
                     CRC(d72f1640)
                         SHA1(6a2ef0ca2b525d9ef0a817bb1d99275cf954242e))
ROM_LOAD16_WORD_SWAP("2", 0x0800000, 0x400000,
                     CRC(c2e8aee5)
                         SHA1(f7e404be93f99c8206636dc3391f162fc83abe2f))
ROM_LOAD16_WORD_SWAP("3", 0x0c00000, 0x400000,
                     CRC(cb5b2744)
                         SHA1(18d42bb6ced01e3e9fa1ed66c07bf91e34811510))
ROM_LOAD16_WORD_SWAP("4", 0x1000000, 0x400000,
                     CRC(7eca59b2)
                         SHA1(e03d71e1e9a67dadb2b76dde87f01147afe9ce34))
ROM_LOAD16_WORD_SWAP("5", 0x1400000, 0x400000,
                     CRC(c3450f2b)
                         SHA1(52dbfc8900d569debd92d2dd0d7fc25e133c5fbb))
ROM_LOAD16_WORD_SWAP("6", 0x1800000, 0x400000,
                     CRC(1ac57ed5)
                         SHA1(a34fb91818b586813d5f7ff5885bc07211189751))
ROM_LOAD16_WORD_SWAP("8", 0x1c00000, 0x400000,
                     CRC(acc78f10)
                         SHA1(9081d3006cc2f4daac1a8b99b729ac17bd6fed52))
ROM_LOAD16_WORD_SWAP("9", 0x2000000, 0x400000,
                     CRC(4ffbba8d)
                         SHA1(526b5ba3b874c0ee7ce3fca7631bed1e0b84fea1))
ROM_LOAD16_WORD_SWAP("10", 0x2400000, 0x400000,
                     CRC(4b2a4397)
                         SHA1(e8499e20939f26009f4678b85d3bfb5f02f4ddcb))
ROM_LOAD16_WORD_SWAP("11", 0x2800000, 0x400000,
                     CRC(58877b19)
                         SHA1(fff50cc05ae39a3b8313756d606e27ae2e62af26))
// IC12 is tested but wasn't included in the dump. This causes missing
// backgrounds
...
|----------------------------------------------------------|
Notes:
 IC1-IC12 - MaskROMs (DIP42), 8Mb or 16Mb or 32Mb. Not all positions are
populated IC13 - EPROM (DIP40), 27C4002 315-5930 - GAL16V8D (DIP20) JP1-JP2 -
JUMPERs, ROM size JP2 JP1 8M  2-3 2-3 16M 2-3 1-2 32M 1-2 1-2 JP3-JP4 - JUMPERs
                    JP4 JP3
             MODE0  1-2 1-2
             MODE1  1-2 2-3
             MODE2  2-3 1-2
...
ROM_REGION32_BE(0x3000000, "cart", ROMREGION_ERASEFF) /* SH2 code */
ROM_LOAD16_WORD_SWAP(
    "epr-21122.ic13", 0x0000000, 0x0080000,
    CRC(49f56e32)
        SHA1(7d8bdaaf3a4edd9df90becc3ec5e94a69bb29ffc)) // ST M27C4002-12F1
ROM_LOAD16_WORD_SWAP("mpr-21125.ic02", 0x0400000, 0x0400000,
                     CRC(40f5f119)
                         SHA1(68fc634734ab05b54ff93256259969f16e26807d))
ROM_LOAD16_WORD_SWAP("mpr-21130.ic03", 0x0800000, 0x0400000,
                     CRC(84cb4e9c)
                         SHA1(675464b0fdf80a3d6e39292e56528e906d388d3c))
ROM_LOAD16_WORD_SWAP("mpr-21126.ic04", 0x0c00000, 0x0400000,
                     CRC(36095649)
                         SHA1(cd06471943380d80887e389e04008d78c58d14b3))
ROM_LOAD16_WORD_SWAP("mpr-21131.ic05", 0x1000000, 0x0400000,
                     CRC(b1c860d8)
                         SHA1(474539f86369f10df5f0e51fd5b44febbc846f98))
ROM_LOAD16_WORD_SWAP("mpr-21127.ic06", 0x1400000, 0x0400000,
                     CRC(f55901d7)
                         SHA1(ab80d82b6707929af9cc5615319bc5b336c13f4a))
ROM_LOAD16_WORD_SWAP("mpr-21132.ic07", 0x1800000, 0x0400000,
                     CRC(809b1b0e)
                         SHA1(8404a03624ba735cc796a38a2b2b96bf9e37f5ee))
ROM_LOAD16_WORD_SWAP("mpr-21128.ic08", 0x1c00000, 0x0400000,
                     CRC(c2e2f72e)
                         SHA1(299c60a74caf3ec6641b6369c823059ef2fbdc5e))
ROM_LOAD16_WORD_SWAP("mpr-21133.ic09", 0x2000000, 0x0400000,
                     CRC(2dc21956)
                         SHA1(6fd7ab435127286a7b212e30b1f29a6f3fbd018b))
ROM_LOAD16_WORD_SWAP("mpr-21129.ic10", 0x2400000, 0x0400000,
                     CRC(997d67bd)
                         SHA1(f588b4391982df4afe60fe8bf739cd510f98caa0))
ROM_LOAD16_WORD_SWAP("mpr-21124.ic11", 0x2800000, 0x0400000,
                     CRC(10f073ac)
                         SHA1(8c16d8ad9c435d44a7b4dd327cd44ffd2f7e895c))
ROM_LOAD16_WORD_SWAP("mpr-21123.ic12", 0x2c00000, 0x0400000,
                     CRC(491b9166)
                         SHA1(f1d5e5b7bef4fc99862b1b8178759e96ba4cb29c))

// CGAL MAIN BD 837-12530. This game seems to use it as an I/O board (see
// strings in ROM), though the number of unpopulated spaces and the 'MAIN BD'
// seem to point to it being capable of much more 834-13246-01 YATTERMAN PLUS
// sticker main populated components: MC68360FE25C QUICC with integrated CPU32+
// core, 32 MHz and 20 MHz XTALs, 315-6052 MACH111, 315-5649 I/O custom,
// 315-5296 I/O custom, 2 batteries main unpopulated components (as per PCB
// etches): 68EC000, 315-5687 SCSP, 2x 16M EPROMs, 2x HM514270 RAMs, MB8421,
// GAL16V8, 45.1584 MHz XTAL, TA8221 amplifier, a second 315-5296 I/O custom,
// COM20020 ULANC and much more
ROM_REGION(0x20000, "io_cpu", 0)
ROM_LOAD("epr-21121.bin", 0x00000, 0x20000,
         CRC(d615bce0) SHA1(09c81e3ee1d2ff22c52a20158d6ad09962910c65))
...
ROM_LOAD16_WORD_SWAP(
    "lh28f016sut.ic30", 0x0a00000, 0x0200000,
    CRC(03b9eacf) SHA1(
        d69c10f7613d9f52042dd6cce64e74e2b1ecc2d8)) // same as some other sets
// ic32, ic34 and ic36 are populated but the flashes are empty (0xff filled)

ROM_REGION16_BE(0x80, "eeprom", 0) // preconfigured to 1 player
ROM_LOAD16_WORD_SWAP("tsuribor.nv", 0x0000, 0x0080,
                     CRC(47c7fa6e)
...
// (making a new def for the base 4th button is a YAGNI until we find a game
// that needs it)
...
GAME(1997, pclub2, stvbios, stv, stv, stvpc_state, init_stv, ROT0, "Atlus",
     "Print Club 2 Standard Ver1 (U 970921 V1.000)", MACHINE_NOT_WORKING)
GAME(1999, pclub2v3, stvbios, stv, stv, stvpc_state, init_stv, ROT0, "Atlus",
     "Print Club 2 Vol. 3 (U 990310 V1.000)",
     MACHINE_NOT_WORKING) // Hello Kitty themed
GAME(1999, pclubpok, stvbios, stv, stv, stvpc_state, init_stv, ROT0, "Atlus",
     "Print Club Pokemon B (U 991126 V1.000)", MACHINE_NOT_WORKING)
// Japan sets
GAME(1999, pclub2fc, stvbios, stv, stv, stvpc_state, init_stv, ROT0, "Atlus",
     "Print Club 2 Felix The Cat (Rev. A) (J 970415 V1.100)",
     MACHINE_NOT_WORKING)
GAME(1998, pclub2pf, stvbios, stv, stv, stvpc_state, init_stv, ROT0, "Atlus",
     "Print Club 2 Puffy (J V1.100)",
     MACHINE_NOT_WORKING) // version info is blank
GAME(1998, pclub2mb, stvbios, stv, stv, stvpc_state, init_stv, ROT0, "Atlus",
     "Print Club 2 Mr. Bean (J 980325 V1.000)", MACHINE_NOT_WORKING)
GAME(1997, pclub2pe, stvbios, stv, stv, stvpc_state, init_stv, ROT0, "Atlus",
     "Print Club 2 Pepsiman (J 970618 V1.100)", MACHINE_NOT_WORKING)
GAME(1997, pclub2wb, stvbios, stv, stv, stvpc_state, init_stv, ROT0, "Atlus",
     "Print Club 2 Warner Bros (J 970228 V1.000)", MACHINE_NOT_WORKING)

GAME(1997, pclb2elk, stvbios, stv, stv, stvpc_state, init_stv, ROT0, "Atlus",
     "Print Club 2 Earth Limited Kobe (Print Club Custom) (J 970808 V1.000)",
     MACHINE_NOT_WORKING)
GAME(1997, pckobe99, stvbios, stv, stv, stvpc_state, init_stv, ROT0, "Atlus",
     "Print Club 2 Kobe Luminaire '99 (Print Club Custom 3) (J 991203 V1.000)",
     MACHINE_NOT_WORKING)

GAME(1997, pclub2bb, stvbios, stv, stv, stvpc_state, init_stv, ROT0, "Atlus",
     "Print Club 2 Banpresto Bono Bono", MACHINE_NOT_WORKING)
GAME(1997, pclub2ts, stvbios, stv, stv, stvpc_state, init_stv, ROT0, "Atlus",
     "Print Club 2 Tsubuyaki Shiro (J 970708 V1.100)",
     MACHINE_NOT_WORKING) // internal string TUBUYAKI SHIRO (sic)
GAME(1997, pclub26w, stvbios, stv, stv, stvpc_state, init_stv, ROT0, "Atlus",
     "Print Club 2 Vol. 6 Winter (J 961210 V1.000)",
     MACHINE_NOT_WORKING) // internal string is 'PURIKURA2 97FUYU' (but in
                          // reality it seems to be an end of 96 Winter version)
GAME(1997, pclub26wa, pclub26w, stv, stv, stvpc_state, init_stv, ROT0, "Atlus",
     "Print Club 2 Vol. 6 Winter (J 970121 V1.200)", MACHINE_NOT_WORKING) // ^
GAME(1997, pclub27s, stvbios, stv, stv, stvpc_state, init_stv, ROT0, "Atlus",
     "Print Club 2 Vol. 7 Spring (J 970313 V1.100)", MACHINE_NOT_WORKING)
GAME(1997, prc28su, stvbios, stv, stv, stvpc_state, init_stv, ROT0, "Atlus",
     "Print Club 2 Vol. 8 '97 Summer (J 970616 V1.100)",
     MACHINE_NOT_WORKING) // internal string 97SUMMER
GAME(1997, prc29au, stvbios, stv, stv, stvpc_state, init_stv, ROT0, "Atlus",
     "Print Club 2 Vol. 9 Autumn (J V1.100)",
     MACHINE_NOT_WORKING) // internal string 97AUTUMN, no date code! (all 0)
GAME(1997, prc297wi, stvbios, stv, stv, stvpc_state, init_stv, ROT0, "Atlus",
     "Print Club 2 '97 Winter Ver (J 971017 V1.100, set 1)",
     MACHINE_NOT_WORKING) // internal string is '97WINTER'
GAME(1997, prc297wia, prc297wi, stv, stv, stvpc_state, init_stv, ROT0, "Atlus",
     "Print Club 2 '97 Winter Ver (J 971017 V1.100, set 2)",
     MACHINE_NOT_WORKING) // different program revision, same date code, clearly
                          // didn't get updated properly
GAME(1998, prc298sp, stvbios, stv, stv, stvpc_state, init_stv, ROT0, "Atlus",
     "Print Club 2 '98 Spring Ver (J 971017 V1.100)",
     MACHINE_NOT_WORKING) // again, date doesn't appear to have been updated,
                          // this should be early 98
GAME(1998, prc298su, stvbios, stv, stv, stvpc_state, init_stv, ROT0, "Atlus",
     "Print Club 2 '98 Summer Ver (J 980603 V1.100)", MACHINE_NOT_WORKING) //
GAME(1998, prc298au, stvbios, stv, stv, stvpc_state, init_stv, ROT0, "Atlus",
     "Print Club 2 '98 Autumn Ver (J 980827 V1.000)", MACHINE_NOT_WORKING)
GAME(1998, pclub2sr, stvbios, stv, stv, stvpc_state, init_stv, ROT0, "Atlus",
     "Showa Retro Print Club 2", MACHINE_NOT_WORKING) // bad dump
GAME(2000, prc2ksu, stvbios, stv, stv, stvpc_state, init_stv, ROT0, "Atlus",
     "Print Club 2 2000 Summer (J 000509 V1.000)",
     MACHINE_NOT_WORKING) // internal string 2000_SUMMER

GAME(1999, pclubor, stvbios, stv, stv, stvpc_state, init_stv, ROT0, "Atlus",
     "Print Club Goukakenran (J 991104 V1.000)", MACHINE_NOT_WORKING)
GAME(1999, pclubol, stvbios, stv, stv, stvpc_state, init_stv, ROT0, "Atlus",
     "Print Club Olive (J 980717 V1.000)", MACHINE_NOT_WORKING)
GAME(1997, pclub2kc, stvbios, stv, stv, stvpc_state, init_stv, ROT0, "Atlus",
     "Print Club Kome Kome Club (J 970203 V1.000)", MACHINE_NOT_WORKING)
GAME(1997, pclubyo, stvbios, stv, stv, stvpc_state, init_stv, ROT0, "Atlus",
     "Print Club Yoshimoto V1 (J 970208 V1.000)",
     MACHINE_NOT_WORKING) // Yoshimoto V1 on cart, internal string YOSHIMOTO
                          // KOGYO
GAME(1997, pclubyo2, stvbios, stv, stv, stvpc_state, init_stv, ROT0, "Atlus",
     "Print Club Yoshimoto V2 (J 970422 V1.100)", MACHINE_NOT_WORKING)

GAME(1997, pclove, stvbios, stv_5838, stv, stvpc_state, init_decathlt_nokey,
     ROT0, "Atlus", "Print Club LoveLove (J 970421 V1.000)",
     MACHINE_NOT_WORKING) // uses the same type of protection as decathlete
GAME(1997, pclove2, stvbios, stv_5838, stv, stvpc_state, init_decathlt_nokey,
     ROT0, "Atlus", "Print Club LoveLove Ver 2 (J 970825 V1.000)",
     MACHINE_NOT_WORKING) // ^
GAME(1997, pcpooh2, stvbios, stv_5838, stv, stvpc_state, init_decathlt_nokey,
     ROT0, "Atlus", "Print Club Winnie-the-Pooh Vol. 2 (J 971218 V1.000)",
     MACHINE_NOT_WORKING) // ^
GAME(1998, pcpooh3, stvbios, stv_5838, stv, stvpc_state, init_decathlt_nokey,
     ROT0, "Atlus", "Print Club Winnie-the-Pooh Vol. 3 (J 980406 V1.000)",
     MACHINE_NOT_WORKING) // ^
GAME(1998, pclubsc5, stvbios, stv_5838, stv, stvpc_state, init_decathlt_nokey,
     ROT0, "Atlus", "Print Club Sony Creative Ver.5 (J 980721 V1.000)",
     MACHINE_NOT_WORKING) // ^
GAME(1997, pclubsc6, stvbios, stv_5838, stv, stvpc_state, init_decathlt_nokey,
     ROT0, "Atlus", "Print Club Sony Creative Ver.6 (J 971006 V1.000)",
     MACHINE_NOT_WORKING |
         MACHINE_UNEMULATED_PROTECTION) // ^, earlier than pclubsc5? IC2 bad,
                                        // black screen on boot (hops on illegal
                                        // opcode)
GAME(1998, pclubnbc, stvbios, stv_5838, stv, stvpc_state, init_decathlt_nokey,
     ROT0, "Atlus", "Print Club Nightmare Before Christmas (J 980717 V1.000)",
     MACHINE_NOT_WORKING) // internal string P.CL NIGHTMARE

GAME(1997, aclub, stvbios, stv, aclub, stv_state, init_stv, ROT0, "Sega",
     "Aroma Club (J 970611 V1.000)",
     MACHINE_NOT_WORKING |
         MACHINE_IMPERFECT_SOUND) // technically also printer and "blended oil"
                                  // dispenser

GAME(1998, stress, stvbios, stv, stv, stvpc_state, init_stv, ROT0, "Sega",
     "Stress Busters (J 981020 V1.000)",
     MACHINE_NOT_WORKING | MACHINE_IMPERFECT_GRAPHICS | MACHINE_IMPERFECT_SOUND)

GAME(1996, nameclub, stvbios, stv_5838, stv, stvpc_state, init_decathlt_nokey,
     ROT0, "Sega", "Name Club (J 960315 V1.000)",
     MACHINE_NOT_WORKING) // uses the same type of protection as decathlete
GAME(1996, nclubv2, stvbios, stv_5838, stv, stvpc_state, init_decathlt_nokey,
     ROT0, "Sega", "Name Club Ver.2 (J 960315 V1.000)",
     MACHINE_NOT_WORKING) // ^  (has the same datecode as nameclub, probably
                          // incorrect unless both were released the same day)
GAME(1997, nclubv3, stvbios, stv, stv, stvpc_state, init_nameclv3, ROT0, "Sega",
     "Name Club Ver.3 (J 970723 V1.000)",
     MACHINE_IMPERFECT_SOUND | MACHINE_IMPERFECT_GRAPHICS |
         MACHINE_NOT_WORKING) // no protection
GAME(1997, nclubv4, stvbios, stv, stv, stvpc_state, init_nameclv3, ROT0, "Sega",
     "Name Club Ver.4 (J 971202 V1.000)",
     MACHINE_IMPERFECT_SOUND | MACHINE_IMPERFECT_GRAPHICS |
         MACHINE_NOT_WORKING) // no protection
GAME(1998, nclubdis, stvbios, stv, stv, stvpc_state, init_stv, ROT0, "Sega",
...
/* Gives I/O errors */
GAME(1996, magzun, stvbios, magzun, stv, stv_state, init_magzun, ROT0, "Sega",
     "Magical Zunou Power (J 961031 V1.000)",
     MACHINE_NOT_WORKING | MACHINE_NODEVICE_MICROPHONE)
GAME(1998, yattrmnp, stvbios, stv, stv, stv_state, init_stv, ROT0, "Sega",
     "Yatterman Plus (J 981006 V1.000)",
     MACHINE_NOT_WORKING | MACHINE_IMPERFECT_SOUND |
         MACHINE_IMPERFECT_GRAPHICS) // needs emulation of the medal specific
                                     // hardware
GAME(1998, choroqhr, stvbios, stv, stv, stv_state, init_stv, ROT0,
     "Sega / Takara", "Choro Q Hyper Racing 5 (J 981230 V1.000)",
     MACHINE_NOT_WORKING | MACHINE_IMPERFECT_SOUND | MACHINE_IMPERFECT_GRAPHICS)
GAME(1998, tsuribor, stvbios, stv, stv, stv_state, init_stv, ROT0, "Sega",
     "Tsuribori Taikai (JAE 980605 V1.000)",
     MACHINE_NOT_WORKING | MACHINE_IMPERFECT_SOUND | MACHINE_IMPERFECT_GRAPHICS)
GAME(1999, chalgolf, stvbios, stv, stv, stv_state, init_stv, ROT0, "Sega",
     "Challenge Golf (J 990326 V1.000)",
     MACHINE_NOT_WORKING | MACHINE_IMPERFECT_SOUND | MACHINE_IMPERFECT_GRAPHICS)
GAME(1999, fanzonem, stvbios, stv, fanzonem, stv_state, init_stv, ROT0, "Sega",
     "Fantasy Zone (medal game, REV.A) (J 990202 V1.000)",
     MACHINE_NOT_WORKING) // require SH2's SCI serial port emulated, to
                          // communicate with coin/medal-related I/O board
GAME(2000, sackids, stvbios, stv, stv, stv_state, init_stv, ROT0, "Sega",
     "Soreyuke Anpanman Crayon Kids (J 001026 V1.000)",
     MACHINE_NOT_WORKING | MACHINE_IMPERFECT_SOUND | MACHINE_IMPERFECT_GRAPHICS)
GAME(2001, dfeverg, stvbios, stv, stv, stv_state, init_stv, ROT0, "Sega",
