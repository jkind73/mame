...
- [ ] **CD-03 — Complete a hardware-faithful CD block implementation. [M/P/R]**
  ...
  - Current integration (this round, `b68f89e7`): the host window and the SH-1's YGR register file both had a word-index/byte-offset decode bug, so no host command ever reached the firmware. The window addressing is now folded once in `sat_console_state::cd_reg_offset()` (word index to byte offset, register-block aliases, data-port alias) and the SH-1 side decodes `(offset & 0x0f) << 1`; the contract is published in `saturn_cdblock.h` and `regtests/saturn/handoff/integration.md`. Verified end to end on the dumped YGR019B firmware: its SH-1 runs inside its ROM, raises its own boot HIRQ pattern (read out of the image at `0x1F30` = `0x0BE1`) in the host window, sees a host HIRQ acknowledge, and completes a host command (Get Hardware Info, CR1..CR4 with CR4 last) with CMOK one frame after the host cleared it — `regtests/saturn/test_cd_lle.py` passes with those assertions. The same fix restored 19 of the 23 `test_cdda_runtime.py` checks on the HLE drive path (23 failing on the parent binary, 4 failing now: the open CD-DA/EXTS items). See `regtests/saturn/handoff/integration.md`.
