...
Arbitrary 48-bit overflow boundaries are covered separately by extracted tests.
"""
import re
...
local function test()
    ...
    local case=0
    local function run(op,a,b,sticky)
        case=case+1
        local before=#fails
        ...
        code[#code+1]=op<<26
        code[#code+1]=0x3209;code[#code+1]=0xf0000000
        ...
        elseif op==6 then
            ...
            local right=signed(b);if right<0 then right=right+0x1000000000000 end
            local wide=left+right
            result=wide&0xffffffffffff;carry=wide>0xffffffffffff
            negative=(result&0x800000000000)~=0;overflow=false
        else
...
emu.register_frame_done(function()
    ...
    if frames==180 then coroutine.wrap(function()
        ...
        if #fails==0 then print('DSP_ALU PASS cases=203')
...
def validate_output(text,returncode):
    if (returncode or 'DSP_ALU FAIL' in text or 'LUA ERROR' in text or
            [int(n) for n in re.findall(r'^DSP_ALU case=(\d+) PASS$',text,re.M)]!=list(range(1,204)) or
            len(re.findall(r'^DSP_ALU PASS cases=203$',text,re.M))!=1):
        raise RuntimeError('DSP arithmetic fixture failed:\n'+text[-16000:])
runner.validate_output=validate_output
if __name__=='__main__':
    runner.main('DSP ALU: 203 arithmetic/flag/read-clear programs passed live')
